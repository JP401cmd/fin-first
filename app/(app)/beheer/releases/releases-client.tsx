...
/**
 * Client-deel van /beheer/releases. `currentVersion` komt van de server
 * (package.json via lib/app-version.ts) — de enige versiebron. De notities
 * zelf zijn een statische import; de vitest borgt dat de bovenste notitie
 * dezelfde versie draagt, dus de datum in de kop komt daarvandaan.
 */
export function ReleasesClient({ currentVersion }: { currentVersion: string }) {
  const current = RELEASE_NOTES.find((r) => r.version === currentVersion)
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="mt-2 flex flex-wrap items-center gap-2 text-sm text-[var(--ink-2)]">
          <span className="text-[var(--ink-3)]">Huidige versie</span>
          <VersionBadge version={currentVersion} />
          {current ? (
            <span className="text-[var(--ink-3)]">{formatDate(current.date)}</span>
          ) : (
            <span className="text-[var(--ink-3)]">nog zonder vrijgavenotitie</span>
          )}
        </p>
      </div>
