...
// De kernel-goldens in de kolommen (er is nog maar één motor); alle checks in de uitklap.
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-[var(--border-ed)] pb-4">
        <div>
          ...
          <p className="mt-1 max-w-2xl font-serif text-sm italic text-[var(--ink-3)]">
            Regressietest van de horizon-grafiek op de <strong>horizon-kernel</strong> over alle
            strategie-combinaties op de complete persona. Per combinatie worden de vrijheidsleeftijd
            (±{FIRE_AGE_MARGIN_YEARS} jr) en het doelbedrag (±{(DOELBEDRAG_REL_MARGIN * 100).toFixed(0)}%)
            tegen een golden gevalideerd, plus structurele invarianten.
          </p>
        </div>
        <button
          onClick={run}
      ...
      {result && (
        <div className={`flex flex-wrap items-center gap-4 border px-4 py-3 ${allGreen ? 'border-emerald-200 bg-emerald-50/60' : 'border-red-200 bg-red-50/60'}`}>
          {/* eslint-disable-next-line no-restricted-syntax -- regressietest-status, geen winst/verlies */}
