...
interface GCSettings {
  gocardless_enabled: string
  gocardless_secret_id: string
  gocardless_secret_key: string
  gocardless_environment: string
}

const DEFAULT_SETTINGS: GCSettings = {
...
export default function BeheerGoCardlessPage() {
  const [settings, setSettings] = useState<GCSettings>(DEFAULT_SETTINGS)
  ...
  const fetchSettings = useCallback(async () => {
    try {
      ...
      setSettings({
        gocardless_enabled: data.gocardless_enabled || 'false',
        gocardless_secret_id: data.gocardless_secret_id || '',
        gocardless_secret_key: data.gocardless_secret_key || '',
  ...
  async function handleTestConnection() {
    ...
    setMessage(null)
    try {
      const res = await fetch('/api/admin/gocardless/test-connection', {
        method: 'POST',
      })
      const data = await res.json()
      if (data.success) {
  ...
  const isEnabled = settings.gocardless_enabled === 'true'
  const hasCredentials = settings.gocardless_secret_id !== '' && settings.gocardless_secret_key !== ''
        ...
        <label className="flex items-center gap-3 cursor-pointer">
          <div className="relative">
            <input
              ...
              checked={isEnabled}
              onChange={(e) =>
                setSettings({ ...settings, gocardless_enabled: e.target.checked ? 'true' : 'false' })
              }
              className="sr-only"
            />
        ...
        <div className="flex gap-4">
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              ...
              name="environment"
              value="sandbox"
              checked={settings.gocardless_environment === 'sandbox'}
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              ...
              name="environment"
              value="production"
              checked={settings.gocardless_environment === 'production'}
