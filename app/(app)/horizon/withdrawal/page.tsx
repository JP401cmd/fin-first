...
export default function WithdrawalPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      // Set retirement age based on FIRE projection
      const fire = computeFireProjection(inp)
  ...
  useEffect(() => {
    ...
    setResult(res)
  }, [input, strategy, retirementAge, targetAge])
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-8">
      <h1 className="text-xl font-bold text-zinc-900">Opnamestrategie</h1>
...
function DrawdownChart({ schedule }: { schedule: WithdrawalYear[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ maxHeight: 240 }}>
      ...
      {/* Area fill - gradient from purple to red for depleted */}
      <defs>
