...
export default function ProjectionsPage() {
  ...
  useEffect(() => {
    ...
    setProjection(projectForward(effectiveInput, 480, r))
  }, [input, extraMonthly, workDays, swr, returnRate, inflation])
    ...
    return (
      <div className="mx-auto max-w-6xl px-6 py-12">
        <div className="rounded-xl border border-red-200 bg-red-50 p-6 text-center">
          <p className="text-sm font-medium text-red-700">Kon gegevens niet laden.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-8">
      ...
      <section className="rounded-2xl border border-purple-200 bg-purple-50 p-6 sm:p-8">
        <h1 className="text-xl font-bold text-zinc-900">FIRE Voorspelling</h1>
        <p className="mt-1 text-sm text-zinc-500">Wanneer bereik je financiele vrijheid?</p>
              ...
              <div className="mt-3 flex items-center gap-2">
                ...
                <div className="relative h-4 flex-1 overflow-hidden rounded-full bg-zinc-200">
                  {(() => {
                    ...
                    const expected = fire.fireAge !== null ? Math.round(fire.fireAge) : min
                    const totalRange = max - min || 1
                    const barStart = 0
                    const barEnd = 100
                    const markerPos = ((expected - min) / totalRange) * 100
                    return (
                      <>
