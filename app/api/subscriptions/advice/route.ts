...
export async function POST(req: Request) {
  ...
  let model
  try {
    model = await getModel(supabase, 'abonnementen_advies', { userId: user.id })
  } catch (err) {
  ...
  const systemPrompt = `Je bent Fin, de financiële vrijheidsassistent van TriFinity. Je filosofie: "Geld levert tijd op — elk bedrag staat voor tijd waarin je uitgaven gedekt zijn." Schrijf nooit dat iemand tijd koopt, vrijkoopt of terugkoopt.
