...
interface BelastingSectionProps {
  dailyExpenses: number
}

export function BelastingSection({ dailyExpenses: parentDailyExpenses }: BelastingSectionProps) {
  ...
  const loadBox3Data = useCallback(async () => {
    try {
      ...
      console.error('Error loading Box 3 data:', err)
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  // Use parent daily expenses as fallback if own calculation yields 0
  const effectiveDailyExpenses = dailyExpenses > 0 ? dailyExpenses : parentDailyExpenses

  const input: Box3Input = useMemo(() => ({
    assets,
    debts,
    hasPartner,
    dailyExpenses: effectiveDailyExpenses,
    year,
  }), [assets, debts, hasPartner, effectiveDailyExpenses, year])
    ...
    return (
      <section className="mt-5 sm:mt-8">
        <div className="mb-4 flex items-center gap-2">
          ...
          <h2 className="label-editorial text-[var(--ink-2)]">Box 3 Vermogensbelasting</h2>
        </div>
  ...
  const annualExpenses = Math.round(effectiveDailyExpenses * 365)
  const displayExpenses = annualExpenses > 0 ? annualExpenses : 40000
  const hasRealData = annualExpenses > 0

  return (
    <>
      <section className="mt-5 sm:mt-8" data-testid="belasting-section">
        {/* Section header */}
        <div className="mb-4 flex items-center gap-2">
          <div className="h-5 w-1 rounded-full bg-kern-500" />
          <h2 className="label-editorial text-[var(--ink-2)]">Box 3 Vermogensbelasting</h2>
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 sm:gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          >
            ...
            {result.tax >= 100 && (
              <p className="mt-1 text-xs text-[var(--ink-3)]" data-testid="kpi-vrijheidsdagen-time">
                {formatWithFreedom(result.tax, effectiveDailyExpenses, { includeCurrency: false })}
              </p>
            )}
          </button>
          ...
          >
            ...
            <p className="mt-1 text-xs text-[var(--ink-3)]" data-testid="kpi-heffingsvrij-freedom">
              {formatWithFreedom(result.heffingsvrijVermogen, effectiveDailyExpenses, { format: 'short', includeCurrency: true })}
            </p>
          </button>
        </div>
                      ...
                      {tip.savings > 0 && (
                        <div className="mt-2 flex flex-wrap items-center gap-2" data-testid={`optimization-${tip.id}-savings`}>
                          <span className="rounded-full border border-kern-100 bg-kern-50 px-2 py-0.5 text-[10px] font-medium text-kern-700">
                            Besparing: {tip.savings >= 100 ? formatWithFreedom(tip.savings, effectiveDailyExpenses) : formatCurrency(tip.savings)}
                          </span>
                          {tip.freedomDays > 0 && (
        ...
        <KassabonShell>
          ...
          <div className="mb-2 mt-2 border-b border-dashed border-[var(--border-ed)] pb-2">
            ...
            <div className="flex justify-between py-0.5">
              ...
              <span className="tabular-nums text-[var(--ink)]">{effectiveDailyExpenses > 0 ? formatCurrency(effectiveDailyExpenses) + '/dag' : 'geen data'}</span>
            </div>
          </div>
          <div className="mt-2 flex justify-between border-t-2 border-[var(--ink)] pt-2 font-bold">
