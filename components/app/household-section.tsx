...
export function HouseholdSection() {
  ...
  if (loading) {
    return (
      <section className="mb-10 rounded-2xl border border-teal-200 bg-white p-6 sm:p-8" data-testid="household-section">
    ...
    <section className="mb-10 rounded-2xl border border-teal-200 bg-white p-6 sm:p-8" data-testid="household-section">
              ...
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex gap-2 shrink-0">
                  ...
                  <button
                    ...
                    disabled={accepting === invite.token}
                    className="flex items-center gap-1 rounded-lg border border-zinc-300 px-3 py-1.5 text-xs font-medium text-zinc-600 hover:bg-zinc-50 disabled:opacity-50 transition-colors"
      ...
      {status && !status.has_household && (
        <div data-testid="invite-form">
          <h3 className="text-sm font-semibold text-zinc-700 flex items-center gap-2 mb-3">
          ...
          <div className="space-y-3">
            <div>
              <label htmlFor="householdName" className="mb-1 block text-xs font-medium text-zinc-600">
                Naam huishouden <span className="text-zinc-400">(optioneel)</span>
              </label>
              <input
                id="householdName"
                type="text"
                ...
                className="w-full rounded-lg border border-zinc-300 bg-zinc-50 px-3 py-2 text-sm text-zinc-900 outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500"
                data-testid="household-name-input"
              />
            </div>
            <div>
              <label htmlFor="inviteEmail" className="mb-1 block text-xs font-medium text-zinc-600">
                E-mailadres partner
              </label>
              <input
                id="inviteEmail"
      ...
      {status && status.has_household && (
        <div data-testid="household-active">
          {/* Household info */}
          <div className="mb-5 rounded-xl bg-teal-50/50 border border-teal-100 p-4">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-zinc-800 truncate">
                    ...
                    {member.is_current_user && <span className="text-xs text-zinc-400 ml-1">(jij)</span>}
                  </p>
              ...
              {status.pending_invitations_sent.map((invite) => (
                ...
                >
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-zinc-700 truncate">{invite.invited_email}</p>
                  ...
                  <div className="flex gap-2 shrink-0">
                    <button
                      ...
                      className="flex items-center gap-1 rounded-lg border border-zinc-300 px-2 py-1 text-xs font-medium text-zinc-600 hover:bg-zinc-50 transition-colors"
            ...
            <div className="mb-5" data-testid="invite-partner-section">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  onChange={(e) => setInviteEmail(e.target.value)}
                  placeholder="partner@voorbeeld.nl"
                  className="flex-1 rounded-lg border border-zinc-300 bg-zinc-50 px-3 py-2 text-sm text-zinc-900 outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500"
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                      e.preventDefault()
                      handleInvite()
      ...
      {showLeaveDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="mx-4 w-full max-w-md rounded-xl bg-white p-6 shadow-xl" data-testid="leave-dialog">
            <div className="flex items-start gap-3 mb-4">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-red-100">
                <AlertTriangle className="h-5 w-5 text-red-600" />
              </div>
              <div>
                <h3 className="text-lg font-semibold text-zinc-900">Huishouden verlaten?</h3>
