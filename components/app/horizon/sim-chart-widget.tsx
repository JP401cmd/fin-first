  ...
  return (
    <>
      ...
      <button
        ...
        className="card-editorial overflow-hidden w-full text-left transition-all hover:shadow-[var(--s1)] hover:-translate-y-px"
      ...
      >
        ...
        <div className="p-4 sm:p-6">
          ...
          {!fireReachable && (
            <div className="mb-4 flex items-start gap-2.5 rounded-[var(--r)] border border-dashed border-orange-300 bg-orange-50/60 px-3 py-2.5">
              ...
              <p className="font-sans text-[12px] text-orange-700">
                FIRE niet bereikbaar voor leeftijd {END_AGE} — verhoog je spaarquote of verlaag je uitgaven.
              </p>
            </div>
          )}
            ...
            <div>
              ...
              <span className="font-sans text-[11px] text-[var(--ink-3)]">
                {fireAgeFractional !== null
                  ...
                  : 'niet bereikbaar voor leeftijd 90'}
              </span>
            </div>
          ...
          <p className="mt-3 font-sans text-[10px] text-[var(--ink-4)]">
            {methodLabel(retirementExpenseMethod)} &middot; Simulatie tot leeftijd {END_AGE} &middot; Klik voor jaar-op-jaar tabel
          </p>
        </div>
      </button>
          ...
          <KassabonShell>
            ...
            <div className="mb-3 text-center">
              ...
              <p className="mt-0.5 font-sans text-[10px] text-[var(--ink-3)]">
                jaar-op-jaar simulatie &middot; einddatum leeftijd {END_AGE}
              </p>
            </div>
              ...
              <div className="mb-2 mt-2 border-b border-dashed border-[var(--border-ed)] pb-2">
                ...
                {cashflows.map(cf => (
                  <div key={cf.id} className="flex justify-between py-0.5">
                    ...
                    <span className={`tabular-nums text-[11px] font-medium ${cf.direction === 'income' ? 'text-green-700' : 'text-red-600'}`}>
            ...
            <div className="mt-3 border-t border-dashed border-[var(--border-ed)] pt-2 font-sans text-[11px] leading-relaxed text-[var(--ink-3)]">
              ...
              <p className="mt-1">
                ...
                Opname = max(0, uitgaven − periodieke kasstromen). Portfolio sluit op €0 op leeftijd {END_AGE}.
              </p>
              <p className="mt-1">
            ...
            <div className="overflow-hidden rounded-[var(--r)] border border-[var(--border-ed)] bg-[var(--paper)] p-3">
              ...
              <div className="mt-2 flex flex-wrap gap-3 text-[10px] font-sans text-[var(--ink-4)]">
                <span className="flex items-center gap-1">
                  <span className="inline-block h-2 w-4 rounded-sm bg-green-500/70" /> Opbouwfase
                </span>
                <span className="flex items-center gap-1">
                  <span className="inline-block h-2 w-4 rounded-sm bg-orange-400/70" /> Afbouwfase
                </span>
                <span className="flex items-center gap-1">
                  <span className="inline-block h-0.5 w-4 border-t-2 border-dashed border-green-600" /> Inkomen (AOW/pensioen)
                </span>
                <span className="flex items-center gap-1">
                  <span className="inline-block h-0.5 w-4 border-t-2 border-dashed border-red-600" /> Uitgave (levensgebeurtenis)
                </span>
              </div>
            </div>
          </div>
                ...
                <table className="w-full min-w-[780px] text-[11px] font-mono">
                  <thead>
                    <tr className="border-b border-[var(--border-ed)] bg-[var(--subtle)]">
                      <th className="px-3 py-2 text-left font-sans text-[10px] font-bold uppercase tracking-[0.06em] text-[var(--ink-3)]">
                        Leeftijd
                      </th>
                      <th className="px-3 py-2 text-left font-sans text-[10px] font-bold uppercase tracking-[0.06em] text-[var(--ink-3)]">
                        Fase
                      </th>
                      <th className="px-3 py-2 text-right font-sans text-[10px] font-bold uppercase tracking-[0.06em] text-[var(--ink-3)]">
                        Begin
                      </th>
                      <th className="px-3 py-2 text-right font-sans text-[10px] font-bold uppercase tracking-[0.06em] text-[var(--ink-3)]">
                        Rendement
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-3 py-1.5">
                              <span className={`font-sans text-[10px] font-medium ${isOpbouw ? 'text-green-700' : 'text-orange-600'}`}>
                                {isOpbouw ? 'Opbouw' : 'Afbouw'}
                              </span>
                            </td>
                            <td className="px-3 py-1.5 tabular-nums text-right text-[var(--ink-2)]">
                              {fmt(row.startPortfolio)}
                            </td>
                            <td className="px-3 py-1.5 tabular-nums text-right text-green-700">
                              +{fmt(row.growth)}
                            </td>
                            <td className="px-3 py-1.5 tabular-nums text-right">
                              {isOpbouw ? (
                                <span className="text-green-700">+{fmt(row.savings)}</span>
                              ) : (
                                <span className="text-red-600">−{fmt(row.withdrawal)}</span>
                              )}
                            </td>
                            <td className={`px-3 py-1.5 tabular-nums text-right ${
                              row.cashflowNet > 0 ? 'text-green-700'
                              : row.cashflowNet < 0 ? 'text-red-600'
                              : 'text-[var(--ink-4)]'
                            }`}>
                              {row.cashflowNet !== 0
                            ...
                            <td className={`px-3 py-1.5 tabular-nums text-right font-medium ${
                              cumLifeEvent > 0 ? 'text-green-800'
                              : cumLifeEvent < 0 ? 'text-red-700'
                              : 'text-[var(--ink-4)]'
                            }`}>
                              {cumLifeEvent !== 0
            ...
            {!tableExpanded && (
              <div className="mt-2 grid grid-cols-3 gap-3">
                <div className="rounded-[var(--r-sm)] border border-[var(--border-ed)] bg-[var(--subtle)]/60 p-2.5 text-center">
                  ...
                  <p className="font-mono text-base font-semibold text-green-700">{opbouwRows.length}</p>
                </div>
                <div className="rounded-[var(--r-sm)] border border-[var(--border-ed)] bg-[var(--subtle)]/60 p-2.5 text-center">
                  <p className="font-sans text-[10px] text-[var(--ink-4)]">Afbouwjaren</p>
                  <p className="font-mono text-base font-semibold text-orange-600">{pensioenRows.length}</p>
                </div>
                <div className="rounded-[var(--r-sm)] border border-[var(--border-ed)] bg-[var(--subtle)]/60 p-2.5 text-center">
