...
const { queries, perspectiveData, latestDate } = vi.hoisted(() => ({
...
vi.mock('@/lib/household/perspective-loader', async (importOriginal) => ({
  ...
  loadPerspectiveData: () => Promise.resolve({ ...perspectiveData.value, debts: [], budgets: [] }),
}))
...
describe('CashOverview — bank_accounts in één leesronde', () => {
  it('leest de tabel één keer in de brede modus, mét het tracking-veld', async () => {
    renderOverzicht({ showAllCashAccounts: true })
    await settle()
    expect(tableReads('bank_accounts')).toHaveLength(1)

    const q = tableReads('bank_accounts')[0]
  ...
  it('leest de tabel één keer in de smalle modus, zónder het tracking-veld', async () => {
    renderOverzicht()
    await settle()
    expect(tableReads('bank_accounts')).toHaveLength(1)
