...
export function AlertCard({ spec }: Props) {
  const isUrgent = spec.severity === 'urgent'

  return (
    <BriefingCard href={spec.href} className={isUrgent ? 'border-rose-200' : 'border-amber-200'}>
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-xs text-[var(--ink-3)] mt-1 line-clamp-2">{spec.message}</p>
          {spec.actionLabel && (
            <div className="mt-2 flex items-center gap-1 text-xs font-medium text-[var(--ink-2)]">
              {spec.actionLabel} <ArrowRight className="h-4 w-4" />
            </div>
          )}
        </div>
      </div>
