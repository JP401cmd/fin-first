...
import { Loader2, AlertCircle, Check, PencilLine } from 'lucide-react'
 ...
 *
 * Bewerk-modus: de gebruiker kan per maand één totaalbedrag netto
 * vermogen invoeren/bijwerken (tot 24 maanden terug). Opslaan POST't
 * alleen de gewijzigde maanden; een ingevoerde maand wordt de
 * gezaghebbende stand (en daarmee geen schatting meer).
 *
...
export function NetWorthHistorySheet({
  ...
  // Bewerk-modus + invoervelden (key = 'YYYY-MM').
  ...
  // Fetch bij openen; reset bewerk-/save-state wanneer het modal sluit zodat
  // een heropening schoon begint.
  useEffect(() => {
    if (open) {
      void fetchHistory()
    } else {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      setSaved(true)
      setEditing(false)
      // Herlaad de getoonde historie én ververs de onderliggende /overzicht-
      // grafiek zodat de nieuwe snapshots overal doorwerken.
      await fetchHistory()
      router.refresh()
    } catch {
  ...
  }, [saving, draft, serverEntries, fetchHistory, router])
          ...
          <>
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              <button
                type="button"
                onClick={openEditor}
                className="inline-flex shrink-0 items-center gap-1.5 rounded-[var(--r)] border border-[var(--border-ed)] bg-[var(--paper)] px-3 py-2 text-xs font-medium text-[var(--ink-2)] transition-colors hover:bg-[var(--subtle)]"
              >
