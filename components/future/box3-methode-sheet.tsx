 ...
 *
 * Spiegelt VoorkeurBewerkenSheet: persist via `PUT /api/parameters` met ALLEEN de
 * gewijzigde velden in de body (de route patcht partieel en valideert enum + band met
 * zod), platte `{ error }`-envelope terug naar de UI, `router.refresh()` laat de kernel
 * herrekenen. Geen client-direct schrijfpad naar `profiles` (ADR 0058).
 */
export function Box3MethodeSheet({
  ...
  return (
    <ShellOverlay
      ...
      footer={
        <ModalFooter
          primary={{ label: 'Opslaan', onClick: () => handleSubmit(), loading: saving, disabled: !changed }}
