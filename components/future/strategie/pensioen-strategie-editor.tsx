...
import { normalizePensionType, type LifeEvent } from '@/lib/horizon-data'
...
export function PensioenStrategieEditor({
  ...
  // null = list-view; anders de beginstand van de pot die `PensioenPotBody` bewerkt.
  const [draft, setDraft] = useState<PotDraft | null>(null)
  // Save-state die de pot-body publiceert (alleen relevant in de pot-view).
  const [potActions, setPotActions] = useState<RegelEditActionsState | null>(null)
  // Bezig-vlag van het verwijderen van een pot (opslaan zit in de body).
  ...
  // Bij elke wissel van view de gepubliceerde save-state van een vorige pot-body wissen,
  // zodat de footer nooit even de `save` van een al ontkoppelde body aanroept.
  function openNew() {
    setError(null)
    setPotActions(null)
  ...
  const potBusy = saving || (potActions?.saving ?? false)
  ...
  return (
    ...
    >
      <PensioenPotBody
        ...
        aowAge={aowAge}
        readOnly={readOnly}
        onActionsChange={setPotActions}
        onSaved={handlePotSaved}
        onTerug={backToList}
      />
    </StrategieModalShell>
  )
