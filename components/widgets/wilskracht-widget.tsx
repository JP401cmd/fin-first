...
// freedom_days_impact kan fractioneel zijn; toon hele dagen als heel getal,
// anders één decimaal — voorkomt lange decimalen ("+18.399999") in het
// headline-cijfer. Zelfde logica als de weekregel.
function formatFreedomDays(value: number): string {
...
export const WilskrachtWidget = memo(function WilskrachtWidget({ size, data, href }: WilskrachtWidgetProps) {
  ...
  const progressPct = Math.min(completionRatio, 100)
      ...
      <WidgetShell module="wil" size={size} kicker="Wilskracht" href={href}>
        <div className="flex flex-col items-center justify-center h-full gap-1">
          ...
          <div
            ...
            aria-valuemin={0}
            aria-valuemax={100}
            aria-label={`${completionRatio}% acties afgerond`}
            className="mt-1 h-1.5 w-full max-w-[80px] overflow-hidden rounded-full bg-wil-100"
          >
            <div
              className="h-full rounded-full bg-wil-500"
              style={{ width: hasEntered ? `${progressPct}%` : '0%', transition: barTransition }}
            />
          </div>
        </div>
      </WidgetShell>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-baseline gap-1.5">
              <span className="font-mono text-xl font-bold tabular-nums text-[var(--ink)]">
                +{formatFreedomDays(totalFreedomDaysWon)}
              </span>
            ...
            <div className="mt-1.5 flex items-center gap-2">
              ...
              <span className="text-xs text-wil-700">
                Deze week: +{formatFreedomDays(weeklyFreedomDaysWon)}
              </span>
            </div>
            <div
              ref={barRef}
              ...
              aria-valuemax={100}
              aria-label={`${completionRatio}% acties afgerond`}
              className="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-wil-100"
            >
              <div
                className="h-full rounded-full bg-wil-500"
                style={{ width: hasEntered ? `${progressPct}%` : '0%', transition: barTransition }}
    ...
    <WidgetShell module="wil" size={size} kicker="Wilskracht" href={href}>
      ...
      <div className="mb-4">
        <div
          ...
          aria-valuemax={100}
          aria-label={`${completionRatio}% acties afgerond`}
          className="h-1.5 w-full overflow-hidden rounded-full bg-wil-100"
        >
          <div
            className="h-full rounded-full bg-wil-500"
            style={{ width: hasEntered ? `${progressPct}%` : '0%', transition: barTransition }}
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div>
          ...
          <p className="mt-0.5 font-mono text-lg font-bold tabular-nums text-[var(--ink)]">
            +{formatFreedomDays(data.totalFreedomDaysOpen)}d
          </p>
        </div>
      </div>
    </WidgetShell>
