 ...
 * (lib/expense-rate.ts) of via `getTxAgg12m` → `aggToExpenseRows`, zoals
 * lib/dashboard-data-loader.ts, lib/cashflow-kpis.ts en lib/core-data-loader.ts
 * al doen. Voeg NOOIT een bestand aan de lijst hieronder toe om 'm groen te
 * krijgen — de lijst mag alleen groeien met nieuwe dagtarief-oppervlakken.
 */
...
describe('dagtarief-databron (L10)', () => {
  ...
  it('lib/expense-rate.ts haalt zijn rijen uit het maandaggregaat', () => {
    const src = readFileSync(join(REPO_ROOT, 'lib/expense-rate.ts'), 'utf8')
    expect(src).toContain('aggToExpenseRows')
    // `realOnly: false` = álle negatieve transacties (incl. transfers) — dezelfde
    // grondslag als DashboardData.dailyExpenseRate. Stil filteren zou een tweede
    // grondslag introduceren en precies de drift terugbrengen die L10 opruimt.
    expect(src).toContain('realOnly: false')
  })
