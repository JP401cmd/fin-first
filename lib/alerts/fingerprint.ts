...
/** `serverError:<domein>[/<sub>]:<METHOD>[:<stap>…][ <variant>]` uit
 *  lib/observability/server-error-log.ts — de vormen die de ~213 routes gebruiken. */
const SERVER_ERROR_TAG = /^servererror:[a-z0-9_/-]{1,40}(:[a-z0-9_-]{1,30}){0,3}( [a-z0-9_|-]{1,30})?$/
