...
describe('resolveLabAntwoorden — de drie hefbomen als antwoorden (spec §3)', () => {
  ...
  it('een hele opgeloste leeftijd blijft heel ("61"), een fractie gaat naar het volgende halve jaar', () => {
    expect(resolveLabAntwoorden({ dekking: tekort, solvedFireAge: 61, baseline })[0]).toMatchObject({
      zin: 'Doorwerken tot 61 dekt je plan.',
      actie: { kind: 'stop', stopAge: 61 },
    })
    expect(resolveLabAntwoorden({ dekking: tekort, solvedFireAge: 61.6, baseline })[0].actie).toEqual({ kind: 'stop', stopAge: 62 })
  })
  ...
  it('geen antwoorden zonder tekort, onder now, of zonder stopmoment', () => {
    ...
    expect(resolveLabAntwoorden({ dekking: { ...tekort, stop: null }, solvedFireAge: 61, baseline })).toEqual([])
  ...
  it('zonder hint (of hint ≤ 0) geen bedrag-antwoorden; zonder basisinkomen geen "minder uitgeven"', () => {
    expect(resolveLabAntwoorden({ dekking: { ...tekort, maandHint: null }, solvedFireAge: 61, baseline }).map((x) => x.kind)).toEqual(['doorwerken'])
    expect(resolveLabAntwoorden({ dekking: { ...tekort, maandHint: 0 }, solvedFireAge: 61, baseline }).map((x) => x.kind)).toEqual(['doorwerken'])
    expect(
      resolveLabAntwoorden({ dekking: tekort, solvedFireAge: null, baseline: { ...baseline, monthlyIncome: 0 } }).map((x) => x.kind),
    ).toEqual(['extra_opzij'])
  })
