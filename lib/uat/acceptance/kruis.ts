...
const criteria: AcceptanceCriterion[] = [
  ...
  },
  {
    ...
    kriticiteit: 'KERN',
    persona: 'compleet',
    given: 'Persona Tessa geladen; spaarquote genoteerd op /overzicht en onderaan /overzicht/budget/transacties (waar het grondslagblok sinds ADR 0135 woont).',
  ...
  },
  {
    ...
    then: 'Per domein tonen alle plekken exact dezelfde stoplichtstatus uit één scoringsbron (`loadLeverScores`/`computeLeverScores`, `lib/leverage-status.ts`-semantiek). Een status-omslag beweegt overal tegelijk. Stoplichtkleuren volgen NIET het module-accent (semantiek blijft semantisch). Consistentie: categorische gelijkheid van de status over vier oppervlakken. Sinds 11 aug 2026 leest `loadLeverScores` het Box 1-inkomen op dezelfde budgetgrondslag als het grondslagblok op /overzicht/budget/transacties (ADR 0103) en telt hij losse rekeningen via de canonieke huishoud-gewogen optelling — zonder die twee kon de dot een andere status tonen dan de pagina eronder terwijl beide "uit één bron" heetten te komen. SINDS B-030 (7 sep 2026) geldt hetzelfde voor de BUDGET-hefboom, en dit is de scherpste toets van dit criterium: de hefboomKAART toonde de EFFECTIEVE spaarquote (ADR 0121) terwijl het kompas, de sidebar-dot en de status-duiding-banner op /overzicht/budget de RAUWE 6-maands meting draaiden — hetzelfde domein, twee percentages (25 % vs. 12 %), en een stoplicht dat aan het verkeerde getal hing. `loadLeverScores` levert nu voor detailregel én score `resolveSavingsSource(...).effectiveSavingsRatePct`. Toets dus expliciet: zet de inkomens- of uitgavengrondslag op "handmatig" of "budgetten" (/overzicht/budget/transacties, ADR 0103) en vergelijk daarna het percentage op de Budget-hefboomkaart met dat in de kompas-popover — die moeten gelijk zijn, en de dot/banner moeten mee omslaan als het getal een drempel passeert. Op een transactie/transactie-grondslag verandert er per definitie niets. Bijkomend: de Budget-tegel toont sindsdien óók WAT dat percentage is ("Spaarquote", als `subAmount` in Volledig en `kpiWindow` in Eenvoudig) — er staat geen kaal getal meer tussen drie euro-bedragen.',
    assertion: {
      kind: 'consistency',
      source: 'consistentie-eis: identieke stoplichtstatus per domein op sidebar/hefboomkaart/banner/boxkaart (lib/lever-scores-loader.ts + lib/leverage-status.ts, één bron; de cashflow/budget-hefboom leest sinds B-030 lib/savings-source.ts#resolveSavingsSource i.p.v. computeSavingsRate6m rechtstreeks) + components/overview/overzicht-hero/hefbomen-nav.tsx#CASHFLOW_BASIS_LABEL voor het label bij het percentage.',
    },
  },
  {
  ...
  },
  {
    ...
    then: 'Elke euro→tijd-omrekening gebruikt hetzelfde dagtarief op jaarbasis: dagtarief = jaaruitgaven ÷ 365 (`dailyExpenseRate`, NIET maand ÷ 30). EXACT: maand €3.000 → dagtarief €98,6301/dag; een bedrag gelijk aan de jaaruitgaven (€36.000) → 365 vrijheidsdagen via `calculateFreedomTime` op ditzelfde tarief (delta 0). Één dagtarief, elk oppervlak. ÉN DE BRON, niet alleen de formule (M22): dezelfde €-heffing levert op /overzicht/belasting/box3 hetzelfde aantal vrijheidsdagen als in de Box 3-widget en de optimizer-kansenlijst, want alle drie lezen het tarief uit `lib/expense-rate.ts`. Een oppervlak dat de formule een ándere teller voert (budget-LIMIETEN i.p.v. gerealiseerde uitgaven) of zelf ÷30 doet, faalt dit criterium — ook al klopt de formule. SINDS UR3-08 FASE 2 (6 sep 2026, eigenaarsbesluit C) geldt dat expliciet ook binnen /toekomst: de simulatie-onderbouwing ("Onder de motorkap"), de grafiek-uitleg-walkthrough en de tooltip van de hoofdgrafiek leidden hun koers af uit de PROJECTIE-uitgave (`yearlyMustExpenses`, in wat-als bovendien scenario-aangepast) en toonden daardoor voor hetzelfde bedrag een andere vrijheidstijd dan de pagina eromheen; alle drie consumeren nu `canonicalDailyRate` uit de bundel. Op het scherm toetsbaar: open de simulatiedetails en vergelijk de vrijheidstijd bij "Huidig netto vermogen" met dezelfde omrekening elders op /toekomst — die moeten gelijk zijn, óók nadat een wat-als-schuif de projectie-uitgaven verandert.',
    assertion: {
      kind: 'exact',
  ...
  },
  {
    ...
    kriticiteit: 'BELANGRIJK',
    given:
      'ADR 0129 fase F1 (gedragsbehoudend): `profiles` draagt naast de oude `fire_end_strategy`-kolom nu ook `fire_stop_anchor`/`fire_stop_age` (migratie 20260903140000 + backfill 20260903141000, beide live). Vaste rijfixtures — géén persona-seed, want dit is een parser-/route-identiteit, geen kernel-uitkomst. De kolommen worden inmiddels via drie oppervlakken geschreven — de plan-keuzes in Voorkeuren/de strategie-modal (WF-TOEK-24), de onboarding-stap "Jouw plan" (WF-START-28) en sinds TPR-09 "Maak dit mijn plan" onder de verkenning op de vrijheidsas (WF-TOEK-46), die het VOLLEDIGE gelezen plan terugschrijft met alleen het anker op `age` — maar dit criterium toetst bewust de parser-/routelaag rechtstreeks (`parseFirePlan`, de PUT-handler van `/api/fire-settings`), niet een browserklik. `monthly_savings_override` is sinds ADR 0141 uit de route verdwenen (niet gelezen, niet geschreven) en hoort niet bij het plan-contract.',
    when:
