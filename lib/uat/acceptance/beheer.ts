...
const criteria: AcceptanceCriterion[] = [
  ...
  },
  {
    ...
    assertion: {
      kind: 'ui-only',
      source:
        'app/(app)/beheer/vragenlijsten/page.tsx + components/app/beheer/vragenlijsten/verspreiding-sheet.tsx + regel-editor.tsx; app/(app)/beheer/waardestromen/page.tsx + components/app/beheer/waardestromen/waardestromen-editor.tsx + lib/waardestromen.ts; app/(app)/beheer/gebruikersgroepen/page.tsx + components/app/beheer/gebruikersgroepen/gebruikersgroepen-beheer.tsx + lib/gebruikersgroepen.ts; API /api/admin/questionnaires(/[id])(/verspreiding), /api/admin/waardestromen, /api/admin/user-groups + lib/questionnaires/verspreiding.ts (schema, samenvatting); invullen: components/app/chat/vragenlijst/vragenlijst-view.tsx + /api/questionnaires/[id]/{session,respond} — inhoudsbeheer, geen cijfermatige uitkomst',
    },
  },
  {
  ...
  },
  {
    ...
    then:
      'De tellingen komen via service-role COUNT (head-only) rechtstreeks uit de bron-tabellen en moeten daarmee overeenkomen (A=B); een onbeschikbare service-client toont tellingen null/onbekend; probe/contracten tonen de status per dienst. Een HTTP 429 telt sinds 26 sep 2026 NIET als storing: de dienst heet dan "begrensd" (eigen toestand, geen groen vinkje en geen rood), en de beheerpagina en het dagelijkse meldkanaal tellen storingen via dezelfde `summarizeProbes()`, dus ze kunnen het daarover niet oneens zijn. Een consistentietoets op de tellingen, geen los na te rekenen cijfer.',
    assertion: {
      kind: 'consistency',
      source:
        'app/(app)/beheer/integraties/… + app/api/admin/integraties/health/route.ts + lib/integrations/health-probe.ts (summarizeProbes) + lib/architecture/integrations-model.ts — tellingen = service-role COUNT op de bron-tabellen (A=B), consistentietoets',
    },
  },
  {
