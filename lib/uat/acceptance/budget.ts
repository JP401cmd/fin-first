...
const criteria: AcceptanceCriterion[] = [
  ...
  },
  {
    ...
    kriticiteit: 'BELANGRIJK',
    given: 'In de sleepmodus (bereikbaar vanuit de Budget Hub, het rekeningdetail op /overzicht/bezittingen/cash en de import-flow) staat tussen de pills "Eigen rekening" en "Overslaan" een +-knop (aria-label "Nieuw budget toevoegen"); een transactie wacht in de ring.',
  ...
  },
  {
    ...
    titel: 'Je budgetten als grondslag voor inkomen en uitgaven: welke posten meetellen, realisatie vóór plan, één historiedeler (ADR 0103 + 0138)',
    kriticiteit: 'KERN',
    given:
    ...
    when:
      '`computeBudgetBasis(budgets, \'expense\' | \'income\', excludedIds, { realized })` bepaalt de selecteerbare posten en hun jaarbedrag — eerst zonder realisatie-venster (alles op plan), daarna met een venster van twaalf AFGESLOTEN maanden dat eindigt op 2026-08, met `historyMonths = 6` (de gebruiker heeft zes afgesloten maanden transactiehistorie), waarin "Supermarkt" (budget uit 2020) €2.400 uitgaven draagt en een in maart 2026 aangemaakt budget €600.',
    then:
      'SELECTIENIVEAU — de lijst staat op het niveau waar het BEDRAG ONTSTAAT: hoofdbudgetten zonder kinderen plus de kinderen van hoofdbudgetten die kinderen hebben. "Boodschappen" is dus zelf géén post (zijn €9.999 is een kop boven zijn kinderen en zou dubbel tellen), "Wonen"/"Supermarkt"/"Markt" wél → 3 posten. DRAGENDE INVARIANT: de uitgavengrondslag bestaat uitsluitend uit `budget_type=\'expense\'` — de savings-pot valt er per constructie buiten, en precies dáárop steunt de spaarquote-regel in WF-CASH-60 (op de budgetgrondslag mag de spaarbudget-/aflossingscorrectie NIET worden toegepast, want het geld is er nooit afgehaald). Gearchiveerde/weggemergde rijen tellen nergens mee. KIND-OPROL met het EIGEN interval van het kind (terugval: dat van de ouder) — dus 1.000×12 + 400×12 + 1.200×1 = €18.000/jaar, €1.500/mnd; dit spiegelt `computeYearlyMustExpenses` en NADRUKKELIJK niet `deriveBudgetTotals`, dat kinderlimieten rauw optelt en met het parent-interval normaliseert (een maandkind onder een jaarouder telt daar 12× te laag). Sluit de gebruiker "Markt" uit, dan zakt het totaal met exact €1.200 naar €16.800 terwijl de post ZICHTBAAR blijft staan (`excluded: true`) — uitsluiten is geen verbergen. Inkomstenkant: €36.000/jaar. REALISATIE VÓÓR PLAN (correctie 11 aug 2026): heeft een post boekingen in het rollende 12-maandsvenster, dan telt de GEMETEN som en draagt de post `source: \'realized\'`; heeft hij er geen, dan valt hij terug op zijn geplande limiet (`source: \'planned\'`), zodat een net aangemaakt budget bruikbaar blijft. Plan en meting worden nooit gemengd binnen één post, en welke van de twee geldt is zichtbaar. ÉÉN DELER PER GEBRUIKER (ADR 0138, eigenaarsbesluit 11 sep 2026): het venster telt uitsluitend AFGESLOTEN maanden (een boeking in de lopende maand telt pas mee zodra die maand voorbij is), en élke gerealiseerde post deelt door dezelfde `historyMonths` — het aantal afgesloten maanden met transactiehistorie van de gebruiker, geklemd op 1..12. De leeftijd van het budget (`created_at`) speelt geen rol meer: met zes maanden historie schaalt het budget uit 2020 (€2.400 → €4.800/jaar) precies zo ×2 als het budget uit maart 2026 (€600 → €1.200/jaar), en beide posten dragen `realizedMonths = 6`. Vóór dit besluit was de deler per post de budgetleeftijd, waardoor één boeking van €1.200 op een budget van drie maanden oud als "€300 per maand, berekend over 4 maanden" doortelde naast €100 per maand op een ouder budget (B-045), en de lopende maand de budgetsom — en daarmee de spaarquote — gedurende de maand liet verschuiven (B-041).',
    assertion: {
      kind: 'exact',
      expected:
      ...
      source:
        'lib/budget-basis.ts#computeBudgetBasis (+ de gedeelde `annualAmount`/`buildBudgetTypeMap` uit lib/budget-utils.ts, `BASIS_BUDGET_TYPE`, en de historiebasis `annualizeHistorySum`/`clampHistoryMonths` uit lib/history-basis.ts) — echte productiefuncties, geen mirror; de huishoud-deelfractie komt via `opts.shareFractionById` binnen en woont bewust buiten deze pure module (lib/household/budget-share.ts) — zie budget-checks.ts',
    },
  },
  {
  ...
  },
  {
    ...
    titel: 'Het eenmalige aanbod om je transacties te categoriseren (ADR 0158)',
    kriticiteit: 'BELANGRIJK',
    persona: 'lisa',
    given:
      'Een account dat /overzicht/budget voor het eerst opent. Drie uitgangssituaties: (a) er staan transacties ZONDER categorie (`allTimeUncatCount > 0`, de HEAD-count in `BudgetsClient` over alle tijden, transfers uitgezonderd); (b) alles is al gecategoriseerd (`allTimeUncatCount = 0`); (c) er staan helemaal geen transacties — dat is het domein van de buurman `BudgetKoppelNudge`, niet van dit aanbod.',
    when:
      'De gebruiker opent de budgetpagina, wacht tot het stil is (geen rondleiding, geen andere overlay), en kiest "Nu categoriseren" of "Later"; daarna herlaadt hij de pagina. Apart geval: de automatische eerste ophaal na de onboarding (WF-START-40) levert net transacties op en doet een `router.refresh()` zonder het clientcomponent te remounten.',
    then:
      '(a) Eén sheet (`ShellOverlay kind="sheet"`, titel "Je transacties categoriseren") met het BEVROREN aantal transacties zonder categorie, correct enkelvoud/meervoud, en twee knoppen in de sticky footer: "Nu categoriseren" (sluit en opent dezelfde bestaande `AICategorizeSheet` als de knop "Transacties categoriseren" — géén tweede categoriseer-UI) en "Later". Beide schrijven de keuze weg met `PUT /api/coachmark` ({id: \'budget-transacties-koppelen\', outcome: \'voltooid\'|\'overgeslagen\'}); ná een herlaad verschijnt het aanbod niet meer, ook op een ander apparaat niet — de staat staat op de eigen profielrij, niet in localStorage. Dat id staat op de DICHTE allowlist `COACHMARK_IDS` in `app/api/coachmark/route.ts`, naast `euro-view` en `overzicht-rondleiding`; een id daarbuiten wordt geweigerd. (b) en (c) tonen NIETS — er wordt zelfs geen coachmark-staat opgehaald zolang er niets te categoriseren valt (geen netwerkverkeer zonder zichtbaarheid), en het aanbod en `BudgetKoppelNudge` sluiten elkaar per constructie uit (die eist nul transacties, dit er minstens één zonder categorie). Valt het aantal alsnog naar nul terwijl de sheet openstaat, dan sluit hij zonder een keuze weg te schrijven. Bij een falende GET geldt "al gezien", zodat een hapering het aanbod niet bij elk bezoek laat opduiken. HERTELLING: `allTimeUncatCount` wordt niet meer alleen bij mount geteld maar bij élke verse serverronde (de effect hangt aan `initialData`) — anders bleef de teller ná de automatische ophaal op de oude waarde staan, verscheen dit aanbod nooit voor precies de gebruiker voor wie het bedoeld is, en meldde de actiestrip "Alles gecategoriseerd" boven een budget vol transacties zonder categorie. De KOPIJ is 19-09-2026 genormaliseerd op één werkwoord (B-059): categoriseren/gecategoriseerd/zonder categorie; het coachmark-id `budget-transacties-koppelen` blijft bewust ongewijzigd — dat is de opgeslagen sleutel.',
    assertion: {
      kind: 'ui-only',
      source:
        'components/app/budget-transacties-aanbod.tsx (poort transacties zonder categorie > 0 + useAttentionQuiet + coachmark-leesronde, bevroren aantal, ShellOverlay/ModalFooter) + app/api/coachmark/route.ts (COACHMARK_IDS-allowlist met budget-transacties-koppelen, eigen-rij read-modify-write) + components/app/budgets-client.tsx (refreshAllTimeUncatCount op initialData, onKoppelen → AICategorizeSheet) — gedrag/zichtbaarheid, geen cijfermatige uitkomst; route-toets in app/api/coachmark/route.test.ts',
    },
  },
]
