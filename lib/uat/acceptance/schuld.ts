...
const criteria: AcceptanceCriterion[] = [
  {
    ...
    kriticiteit: 'KERN',
    persona: 'lisa',
    given: 'Persona Lisa de Groot geladen (11 schulden, elk debt-type precies 1×), solo-perspectief. WEERGAVEMODUS (BEZ-3/APP-7): de vier onderstaande cijfers zijn zelf mode-onafhankelijk (dezelfde `totalBalance`/`weightedAvgInterest`-sommen). Wat ZICHTBAAR is op de figures-strip verschilt: in **Volledig** staan alle vier de cellen; in **Eenvoudig** kapt `FiguresStrip` af tot de eerste 2 — Totale schuld + Maandlasten — en blijven "Rente (gewogen)" en "Categorieën" (beheer-diepte) buiten beeld. De LTV-KPI op de Hypotheek-kaart staat los van de strip en is in beide modi ongewijzigd zichtbaar. TITEL (kop-herziening 19-09-2026; sinds 22-09-2026 een OORDEELZIN, ADR 0174 D6; spiegelt WF-BEZIT-01): `app/(app)/overzicht/schulden/page.tsx` roept `loadHefboomPageVerdict(supabase, perspective, \'schulden\')` aan — DEZELFDE `loadLeverScores`-bron als het statuspunt naast de pagina-`i` — en de kop is één zin, op mobiel én desktop gelijk (de TopBar draagt de paginanaam): "Je schulden *wegen licht*." (good) / "Je schulden *vragen aandacht*." (warn) / "Je schulden *wegen zwaar*." (bad), of "Je schulden zijn *nog niet in beeld*." bij een neutrale/onbekende hefboomscore (`lib/hefboom-oordeelzin.ts`). Het cursieve deel is een `<em>` in de stoplichtkleur; de punt staat erbuiten. Bewust "wegen licht" en niet "worden op schema afgelost": de score is de verhouding schuld/bezit, geen aflosschema, en wie schuldenvrij is mét vermogen staat op groen. Op de legacy-route /core/debts blijft de kale paginanaam staan. De hefboomtegel op /overzicht houdt de korte vorm uit `lib/hefboom-status-copy.ts`: "Lage schuldenlast" (good, sinds 22-09-2026; was "Aflossing op schema", om dezelfde reden als "wegen licht") / "Schuldenlast vraagt aandacht" (warn) / "Hoge schuldenlast" (bad). DECK (eenvoud-check B-071): "Al je schulden, met hun maandlast en rente. Elke aflossing levert tijd op, en hoe lager je schuldgraad, hoe meer ruimte je hebt."',
