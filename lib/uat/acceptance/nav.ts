...
const criteria: AcceptanceCriterion[] = [
  ...
  },
  {
    ...
    then: 'Perspectief-sectie bevat 3 items; het item voor "personal" heeft " · actief" in de sublabel, de andere twee niet. "Bedragen verbergen" (niet "tonen") staat in de lijst zolang privacyMasked=false. De sync-actie (`action:sync-prices`) is aanwezig ongeacht welke modules actief zijn — sinds 7 sep 2026 met label "Alles synchroniseren" (was "Synchroniseer prijzen") én zonder module-gate (was `vermogensregistratie`), omdat de actie dezelfde volledige ronde draait als de sync-knop in de header: koersen, bankgegevens, exchanges en wallets. Die knop kent zelf geen gate, en de ronde haalt inmiddels ook banktransacties op — die horen bij `budgetteren`, dus een gate op vermogensregistratie sloot een budgetteren-only gebruiker mét bankkoppeling ten onrechte uit (melding B-029). Het id en de veldnamen in `expected` blijven ongewijzigd; die spiegelen `nav-checks.ts`.',
    assertion: {
      kind: 'exact',
