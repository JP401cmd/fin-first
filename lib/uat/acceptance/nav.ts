...
const criteria: AcceptanceCriterion[] = [
  ...
  },
  {
    ...
    kriticiteit: 'BELANGRIJK',
    given: 'Budget-kind-dots (Transacties/Vaste lasten/Vooruitblik) en belasting-kind-dots (Box 1/2/3) in de sidebar. HERZIEN 06-09-2026 (ADR 0135): "Budget" is de OUDER geworden en staat niet meer als kind onder zichzelf; zijn eigen dot komt uit `leverKey: cashflow` (de hefboomscore uit `loadLeverScores`), de drie kinderen uit `buildCashflowCards`.',
    when: 'De dot-kleur op een budget-/belasting-subroute wordt vergeleken met de status op de bijbehorende kaart (voor de Budget-ouder: met de hefboomtegel op /overzicht).',
  ...
  },
  {
    ...
    kriticiteit: 'BELANGRIJK',
    given: 'Mobiele TopBar-utility-cluster op een tab-root; kompas-deeplinks in `lever-compass.tsx`. Sinds fase 4 van de eenvoudige weergave hangt de TRIGGER aan de weergavemodus (NAV-6): in Volledig de gekleurde stippen naast elkaar, in Eenvoudig één samengevat statuspunt met de zwaarste status (rood > oranje > groen > geen data). Sinds 22 sep 2026 zijn het er vijf: de vier hefbomen plus "Toekomst", gevoed door het plan-stoplicht (`usePlanStatus`, dezelfde bron als het punt naast "De toekomst" in het menu). Zolang die status nog nastreamt (neutral) blijven het er vier.',
    when: 'De gebruiker tikt de kompas-trigger aan en gaat door het paneel naar "Bezittingen"/"Schulden"/"Budget"/"Toekomst".',
    then: 'De kompas-deeplinks wijzen naar de canonieke /overzicht/*-routes: Bezittingen → /overzicht/bezittingen, Schulden → /overzicht/schulden, en de derde hefboom heet sinds ADR 0135 "Budget" (label in lever-compass.tsx én HEFBOOM_CONFIG) en wijst naar /overzicht/budget — de interne sleutel blijft bewust `cashflow` (`lib/hefboom-config.ts` + `lib/lever-mapping.ts`; die sleutel zit in scores, briefing-tags, data-tour-ids en deels in opgeslagen rijen, dus hernoemen zou data raken in plaats van tekst) (geen legacy /core/* of /will#cashflow-ankerverlies meer). Toekomst → /toekomst. Het PANEEL is in beide modi identiek: alle punten (de vier hefbomen + Toekomst) mét naam, status en detail, en het samengevatte punt telt het plan mee — de reductie zit uitsluitend in de trigger, dus er gaat geen informatie verloren. In Eenvoudig draagt het ene punt de status ook in zijn aria-label ("Kompas: aandacht — open het kompas"); "geen data" overstemt nooit een echte waarschuwing. OPGELOST (deze release): het uitgeklapte paneel hangt nu gecentreerd onder de trigger (`left-1/2 -translate-x-1/2`, met `max-w-[calc(100vw-2rem)]` als viewport-clamp) i.p.v. rechts uitgelijnd (`right-0`) — de trigger staat als tweede icoon in de TopBar-rij, niet aan de rechterrand, dus een `right-0`-geankerd 256px-paneel liep op smalle schermen (390px) voorbij de linkerrand af. Geborgd in lever-compass.deeplinks.test.tsx + lever-compass.simple-view.test.tsx.',
    assertion: {
      kind: 'ui-only',
      source: 'components/app/shell/lever-compass.tsx (LEVERS-config + useMobileCompassPoints + worstStatus + gecentreerde paneel-positionering) — canonieke routes; regressietests lever-compass.deeplinks.test.tsx en lever-compass.simple-view.test.tsx',
    },
  },
  {
    workflow: 'WF-NAV-07',
  ...
  },
  {
    ...
    then: 'Perspectief-sectie bevat 3 items; het item voor "personal" heeft " · actief" in de sublabel, de andere twee niet. "Switch naar verborgen bedragen" (niet "zichtbare") staat in de lijst zolang privacyMasked=false — sinds W-006 (11 sep 2026) noemen de schakelaars hun doelstand ("Switch naar …"); was "Bedragen verbergen". De sync-actie (`action:sync-prices`) is aanwezig ongeacht welke modules actief zijn — sinds 7 sep 2026 met label "Alles synchroniseren" (was "Synchroniseer prijzen") én zonder module-gate (was `vermogensregistratie`), omdat de actie dezelfde volledige ronde draait als de sync-knop in de header: koersen, bankgegevens, exchanges en wallets. Die knop kent zelf geen gate, en de ronde haalt inmiddels ook banktransacties op — die horen bij `budgetteren`, dus een gate op vermogensregistratie sloot een budgetteren-only gebruiker mét bankkoppeling ten onrechte uit (melding B-029). Het id en de veldnamen in `expected` blijven ongewijzigd; die spiegelen `nav-checks.ts`.',
    assertion: {
      kind: 'exact',
  ...
  },
  {
    ...
    assertion: {
      ...
      // 24 → 29 (6 sep 2026, ADR 0135): de vijf redirects die de opgeheven
      // cashflow-laag levend houden — /overzicht/cashflow zelf plus zijn vier
      // subpaden — inclusief de deeplinks met ?budget=/?maand=/?limit=/?rekening=
      // die in meldingen, briefing-mails en gedeelde links rondgaan. Bewust NIET
      // meeverhuisd: de redirect-BRONNEN zelf en /api/overzicht/cashflow-settings
      // (die draagt '/overzicht/cashflow' als deelstring).
      // 29 -> 31 (7 sep 2026, UR3-26): twee regels erbij die elk een tweede
      // live URL voor dezelfde inhoud opheffen —
      // /toekomst/samengestelde-interest -> /toekomst/rekenhulp (de losse
      // calculator had nul ingangen) en /core/checkin/historie ->
      // /mijn/checkins (twee URL's voor identieke inhoud). De bestaande
      ...
      // oorspronkelijke query zelf weer toe zolang de DESTINATION er zelf geen
  ...
  },
  {
    ...
    when: 'De gebruiker start een sync met/zonder koppelingen, of terwijl een sync al loopt; opent het sync-rapport (account-dropdown "Sync-rapport", of een klik op de globale knop bij fouten). TWEE INGANGEN, ÉÉN RONDE (B-029, 7 sep 2026): behalve de `GlobalSyncButton` in sidebar/top-bar start ook de ⌘K-actie "Alles synchroniseren" (`action:sync-prices`, WF-NAV-09) exact dezelfde ronde — beide via de gedeelde `useGlobalSyncRunner` (`components/sync/use-global-sync-runner.ts`). Die actie draaide tot dan `pricesOnly: true` en sloeg de bankstap, exchanges en wallets stil over; toets dus beide ingangen en verwacht identiek gedrag, inclusief dezelfde uur-rem per koppeling en dezelfde 10/dag-rem.',
  ...
  },
  {
    ...
    then: 'Elke sheet (en het ChatPanel) gedraagt zich identiek qua sleepgebaar (slide-omhoog/omlaag, dezelfde velocity/percentage-drempels) — BottomSheet-varianten klappen bovendien uit naar ~92vh (`initialMobileHeight`, ChatPanel kent dit niet); alle sluiten via drie routes en dekken de nav-pill af — behalve de NavMenuSheet, die als enige de pill zichtbaar laat (bevestigd als de enige uitzondering op de pill-regel). Eén bewuste uitzondering op de sluitroutes: een sheet met `lockedOpen` (vandaag alleen de eenmalige AI-keuze-overlay, ADR 0155 — zie WF-START-30) heeft GEEN X, reageert niet op Escape, backdrop-klik, swipe-down of de terug-knop en gaat uitsluitend dicht doordat de ouder `open={false}` zet na een gemaakte keuze; er bestaat geen halve vergrendeling (een zichtbare X die niets doet, of een poort die Escape weigert maar een veeg doorlaat). Het ChatPanel-gebaar is uitgeschakeld zolang de chat gepind is (desktop-zijbalk) of een melding wordt verstuurd (`meldingBezig`) — zie WF-WILL-24.',
    assertion: {
      kind: 'ui-only',
      source: 'components/app/bottom-sheet.tsx (incl. `lockedOpen`, bewaakt in bottom-sheet.test.tsx) + components/app/ai-consent-interstitial.tsx + lib/hooks/use-swipe-to-dismiss.ts + components/app/chat/chat-panel.tsx (useSwipeToDismiss-gebruik) — gedeelde modal-conventie, geen cijfermatige uitkomst',
    },
  },
  {
    workflow: 'WF-NAV-22',
  ...
  },
  {
    ...
    then: 'Het actie-label flipt: bij nominaal toont de palette "Switch naar huidige euro\'s" (icoon Wallet), bij reëel "Switch naar toekomstige euro\'s" (icoon CalendarClock; was "Toon …" tot W-006) — nooit de huidige stand als label (dat zou een no-op suggereren). Het command-palette-label en de sidebar-status-indicator (`EuroViewBadge`, ADR 0094) gebruiken hetzelfde `euroViewLabel()` (géén los label-format). Cross-device: de keuze staat op `profiles.euro_view` (own-row, `PUT /api/euro-view`) en wordt server-side in de layout ingelezen — dat schrijf-/leespad is een LIVE-vereiste, niet in een pure module na te rekenen (server-only createClient-import), en wordt daarom als toelichting genoemd, niet in de geautomatiseerde assertie.',
    assertion: {
      kind: 'exact',
