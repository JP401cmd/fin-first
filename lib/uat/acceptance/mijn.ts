...
const criteria: AcceptanceCriterion[] = [
  {
    ...
    then: 'Elke kaart landt op de juiste /mijn/*-subroute; het grid dekt de in de nav geregistreerde onderwerpen (feedback is bewust verweesd — alleen via ⌘K/URL bereikbaar). Rapportages staat sinds bevinding M14 bewust NIET in het grid: die heeft een vaste ingang in de desktop-zijbalk en zou anders twee keer verschijnen. Account stond om dezelfde reden ook niet in het grid zolang de zijbalk-footer een losse Account-link had; sinds die link op 15 sep 2026 verviel ("dat gaat via Mijn") is de Account-kaart terug — anders is Account op desktop onvindbaar (de mobiele nav-pill is `lg:hidden`). WEERGAVEMODUS (S8, optie B): in **Volledig** staan alle tien kaarten in één grid (sinds 15 sep 2026, ná Jaaroverzicht/Mijlpalen ook Account). In **Eenvoudig** staan er vier vooraan — profiel, privacy, koppelingen, uiterlijk — en zitten account, notificaties, check-ins, jaaroverzicht, mijlpalen en geavanceerd achter één ingeklapte DepthSection "Alle instellingen" (`data-collapsed="true"`) met een samenvatting die zegt wát erin zit (incl. "account"). Weggevouwen, niet verwijderd: de kinderen blijven gemount en `inert`, dus met een dichte sectie springt Tab van de laatste primaire kaart naar de disclosure-knop en niet in de verborgen links. Uiterlijk staat bewust vooraan — daar woont de weergavekeuze zelf, dus dat is de vluchtroute terug naar Volledig. Op /mijn zelf staat boven het grid geen tabbalk (in BEIDE weergavemodi — dit is een dubbeling wegnemen, geen diepte verbergen); zodra je op een subpagina landt verschijnt de tabbalk als zijwaartse navigatie, mét het Account-tabblad. Onderaan /mijn staat in beide weergavemodi één regel "TriFinity versie 0.92.001" (de patch in drie cijfers, `APP_VERSION_DISPLAY`) — alleen tekst, geen link; het getal is gelijk aan de bovenste notitie op /beheer/releases.',
    assertion: {
      kind: 'ui-only',
  ...
  },
  {
    ...
    then: 'Alle 7 stappen doorlopen en een maand-snapshot wordt vastgelegd; bedragen respecteren de privacy-maskering (WF-MIJN-17/NAV-11); de banner-ingang op /overzicht is een steekproef (WF-OVZ-13). Sinds plan-voorstel 7 (doelen-vieren, 1 sep 2026) eindigt het afronden niet meer met direct wegsturen maar met een kort krant-afsluitmoment ("Je maandbeeld staat vast"); staat de gebruiker daarmee op precies 3, 6 of 12 aaneengesloten maanden (bron: completedMonths van /api/monthly-checkin, gecapt op 13 — 13, zodat >12 op rij meetbaar blijft en \"Twaalf op rij\" maar één keer kan vallen; de reeks wordt afgeleid via de pure berekenReeks op de SERVER-maand, nooit een tweede telling), dan draagt het moment de reeks-erkenning ("Drie maanden op rij"). Erkennen, niet straffen: een gebroken reeks geeft nérgens een melding; een herhaalde afronding binnen dezelfde maand of een reeks boven 12 krijgt het standaard-vignet, nooit nogmaals de mijlpaal. Het afsluitmoment navigeert ná ±4,5 s zelf naar de vertrekpagina (pauzeerbaar op hover/focus; sluitknop = direct door) — dat is bedoeld gedrag, geen bug. De historie toont de lopende reeks vanaf 2 maanden. BEGROTING VAN NUL IN DE CHECK-IN (melding B-032, 08-09-2026) — twee verschillende antwoorden op dezelfde pagina, en dat is bedoeld. (1) De TERUGBLIK op een opgeslagen snapshot is zuivere WEERGAVE: die rekent niet meer zelf (`b.limit > 0 ? spent/limit : 0`) maar consumeert `budgetSpentPct` + `isOverBudget`, dus een historische rij met een begroting van nul waar wél op geboekt was toont nu een VOLLE balk naast het rode bedrag, in plaats van de eerdere lege balk. (2) De BUDGET-stap en het reflectie-oordeel houden bewust hun `limit > 0`-eis: die stap is een EDITOR waarvan `changedCount` (en daarmee de opslaanknop) `limit <= 0` overslaat, dus een nul-limiet-rij zou een invoerveld tonen dat niet weg te schrijven is; het reflectie-aantal zou anders budgetten noemen waarvan de gebruiker in de vorige stap geen enkele rij zag. Ziet de tester in de terugblik nog een lege balk naast een besteed bedrag, of duikt er in de budget-stap een niet-op-te-slaan nul-limiet-rij op, dan is dat de regressie. GESPREKSSTARTERS "vermogen-groei"/"vermogen-daling" (`app/api/checkin/gespreksstarters/route.ts`, `lib/checkin/gespreksstarters.ts`): tot 19-09-2026 vuurden deze twee nooit, omdat de route een eigen `net_worth_snapshots`-query op een kolom (`value`) las die de tabel nooit heeft gehad — stilzwijgend `null → []`. De route consumeert nu dezelfde 12-maands reeks als de dashboardbundel (`getNetWorthSnapshots12m`) voor `netWorthTrend`; bij een trend van meer dan €50 t.o.v. de vorige snapshot verschijnt de starter alsnog. Dit scenario is LEIDEND voor de check-in-flow.',
    assertion: {
      kind: 'ui-only',
