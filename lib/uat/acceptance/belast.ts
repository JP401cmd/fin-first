 ...
 *   LET OP (ADR 0177, 22-09-2026): die heuristiek voedt sinds dat besluit
 *   ALLEEN nog de Box 3-KAART en de titel van /overzicht/belasting/box3 — een
 *   feitelijke constatering over de grondslag ("Ruim boven de vrijstelling").
 *   De HEFBOOM Belasting (tegel op /overzicht, hub-titel, statuspunt, melding)
 *   oordeelt niet meer op de hoogte van de Box 3-heffing, en de euro-banden
 *   €100.000/€500.000 boven de heffingsvrije voet — waarmee een alleenstaande
 *   vanaf ~€159.000 box 3-vermogen rood stond, zonder plafond en zonder
 *   handeling die hem groen maakte — zijn vervallen. De nieuwe grondslag is
 *   ONBENUTTE FISCALE RUIMTE als aandeel van de eigen heffing over Box 1 + Box
 *   3: `onbenutRatio = som(netto besparing openstaande posten) / (box1Tax +
 *   box3Tax)`, met < 5% groen, 5–15% oranje, >= 15% rood; geen heffing bekend →
 *   neutral (nooit stil groen). Posten: partnerverdeling
 *   (`optimizePartnerAllocation.savingsVsEqual`), onbenutte jaarruimte
 *   (`computeJaarruimte` + `jaarruimteBesparing`) en samenstelling-shift met
 *   `netEffect > 0`. Teller en noemer schalen allebei met vermogen mee, dus de
 *   uitkomst niet: wie niets laat liggen staat groen bij élk vermogensniveau, en
 *   een bescheiden vermogen kan oranje staan als er echt geld ligt. Bron:
 *   `lib/fiscale-ruimte.ts#computeFiscaleRuimte` + `docs/adr/0177-*.md`.
...
const criteria: AcceptanceCriterion[] = [
  {
    ...
    kriticiteit: 'KERN',
    persona: 'compleet',
    given: 'Persona Tessa Compleet geladen; /overzicht/belasting toont drie box-kaarten + de "De druk"-sectie (Box 1 + Box 3, excl. Box 2). Sinds 26-08-2026 (bevinding H22) telt de opening wat er werkelijk staat: bij aanmerkelijk belang "Drie boxen, twee rekeningen" (en de colophon idem), zonder aanmerkelijk belang "Twee boxen, één rekening" — de oude vaste kop "Drie boxen, één rekening"/"Drie boxen, één som" beloofde een som van drie boxen die het totaal by design niet maakt. HERZIEN 19-09-2026 (kop-herziening — `PageVerdictOpening`): die H22-belofte woont sinds deze wijziging niet meer in de titel maar in de EERSTE zin van de DECK ("Drie boxen, twee rekeningen: Box 2 telt apart…" resp. "Twee boxen, één rekening: Box 1 en Box 3…", `hub-opening-copy.ts#buildBelastingHubOpening`, `year`-param vervallen met de kicker) en in de colophon (ongewijzigd). De TITEL zelf is sinds 22-09-2026 één OORDEELZIN (ADR 0174 D6), op mobiel én desktop gelijk (de TopBar draagt de paginanaam): sinds 22-09-2026 (ADR 0177) over de onbenutte fiscale ruimte: "Je fiscale ruimte is *goed benut*." (good) / "Je fiscale ruimte is *deels onbenut*." (warn) / "Je fiscale ruimte is *grotendeels onbenut*." (bad) / "Je fiscale ruimte is *nog niet in beeld*." (neutral), uit `lib/hefboom-oordeelzin.ts`; het cursieve deel is een `<em>` in de stoplichtkleur. Het onderwerp was tot ADR 0177 "Je Box 3-belasting" omdat de hefboomstatus alleen Box 3 mat; die grondslag is vervallen. De hefboom meet nu onbenutte ruimte over Box 1 én Box 3, dus "Je Box 3-belasting" zou te smal zijn en "Je belasting" zou suggereren dat de zin over de hóógte van de heffing gaat. Belasting is daarmee de enige hefboom waarvan het onderwerp niet het paginawoord draagt — dat staat links ervan in de TopBar (gepind in lib/hefboom-oordeelzin.test.ts). De tegel op /overzicht draagt dezelfde grondslag in tegelwoorden: "Niets onbenut" / "Ruimte onbenut" / "Veel ruimte onbenut" (`HEFBOOM_VERDICT.belasting`, lib/hefboom-status-copy.ts; het groene woord was kort "Ruimte benut" maar dat is woordelijk het label van `box1JaarruimteVerdict` op de Box 1-kaart eronder, voor uitsluitend de jaarruimte — eigenaarsbesluit 22-09-2026); de hedge "Mogelijk betaal je meer dan nodig" is vervallen omdat de posten nu geteld worden in plaats van vermoed. Het oordeel komt van de BELASTING-HEFBOOM uit `loadHefboomPageVerdict(..., "belasting")` — dezelfde bron als het statuspunt naast de `i` (`leverInfo("/overzicht/belasting", scores.tax)`), en dezelfde helper als de titels van /overzicht/bezittingen, /schulden en /budget. Bewust NÍET het Box 3-oordeel uit de bundel van deze pagina: dat leest een perspectief-gescoopte assetset terwijl `loadLeverScores` RLS-breed ophaalt, waardoor titel en stip in een huishouden uiteen konden lopen. Het Box 3-oordeel blijft op de Box 3-kaart hieronder en op de titel van /overzicht/belasting/box3; de deck sluit (eenvoud-check B-071, zonder het woord "oordeel") met de vaste tweede zin "De stand volgt de fiscale ruimte die onbenut blijft." — tot ADR 0177 was dat "Hoe meer Box 3-vermogen boven de vrijstelling, hoe meer je betaalt", een uitleg van de afgeschafte grondslag. De STATUS-DUIDING-MELDING op deze route noemt bij oranje en rood de grootste openstaande post mét bedrag ("Je laat fiscale ruimte liggen: onbenutte jaarruimte (ongeveer € 2.960 per jaar)." plus "Daarnaast ligt er nog een andere post open." zodra er meer dan één is), uit de byCause-variant van PAGE_STATUS_COPY voor /overzicht/belasting (lib/page-status/copy.ts) — constaterend, nooit imperatief (Wft), en met terugval op de generieke warn/bad-tekst wanneer er geen post is. Ook de Box 1-kaartstatus komt sinds deze wijziging uit `box1JaarruimteVerdict` (gedeeld met de titel van /overzicht/belasting/box1) i.p.v. een hier inline geschreven ternary, en de Box 3-kaarttekst "Optimaliseer Box 3" (een Wft-gevoelige gebiedende wijs) is vervangen door de beschrijvende `box3StatusVerdict`-zin.',
    ...
    assertion: {
      kind: 'consistency',
      source: 'lib/tax-overview.ts#buildTaxOverview — aggregatie-invariant total = box1+box2+box3, distribution som = 100, freedomDays = round(total/daily); box1Tax = computeBox1Tax over resolveBox1GrossIncome + resolveEigenWoningBox1Input via lib/tax-opportunities-loader.ts#loadFiscaleKansen (A=B met /overzicht/belasting/box1 — beide invoerhelften gedeeld sinds C8; de bron-grendel in lib/box1-income.eigen-woning.test.ts bewaakt dat); box3Tax = horizonData.box3Tax resp. de perspectief-heffing uit dezelfde loader (canonieke calculateBox3, A=B met /overzicht/belasting/box3). tax-overview.test.ts dekt de aggregator exact. TARIEVEN (C9/M4): effectiveRate/marginalRate zijn pass-through uit dezelfde computeBox1Tax-aanroep (loadFiscaleKansen.box1EffectiveRate/box1MarginalRate); `grossYearlyIncome` is uit TaxOverviewInput verwijderd zodat de teller/noemer-mismatch niet kan terugkeren. Bewaakt door lib/tax-overview.hub-tarieven.test.ts (motor-invariant effectief <= marginaal over €1.000-€400.000, absolute ruisband 1e-9 tariefpunten; exacte reconstructie van 36,6%/35,8%/56,01% bij bruto €93.369; bron-grendel op hub-pagina + kansen-loader) en components/overview/belasting/hub-totale-druk.test.tsx (Inkomen onbekend → geen enkel percentage). Titel/deck (kop-herziening 19-09-2026): app/(app)/overzicht/belasting/page.tsx (belastingVerdict via lib/hefboom-page-verdict.ts#loadHefboomPageVerdict → PageVerdictOpening) + app/(app)/overzicht/belasting/hub-opening-copy.ts#buildBelastingHubOpening (deck/colophon, gepind in hub-opening-copy.test.ts) + lib/jaarruimte.ts#box1JaarruimteVerdict + lib/box3-taxable-input.ts#box3StatusVerdict. HEFBOOM-OORDEEL (ADR 0177, 22-09-2026): lib/fiscale-ruimte.ts#computeFiscaleRuimte levert status + ratio + posten aan `loadLeverScores().fiscaleRuimte`; de kop-zin komt uit lib/hefboom-oordeelzin.ts (gepind in hefboom-oordeelzin.test.ts), het tegelwoord uit lib/hefboom-status-copy.ts (hefboom-status-copy.test.ts) en de oorzaak-melding uit lib/page-status/copy.ts#PAGE_STATUS_COPY.byCause via lib/page-status/resolve.ts (lib/page-status/__tests__/page-status.test.ts, blok "fiscale ruimte: de melding noemt de oorzaak"). De euro-banden €100.000/€500.000 boven de heffingsvrije voet bestaan niet meer.',
    },
  },
  {
  ...
  },
  {
    ...
    assertion: {
      ...
      source: 'lib/box1-tax.ts#computeBox1Tax (year 2026, grossYearlyIncome=grossFromNet(7600×12,2026)=160658, wozValue=540000, hypotheekRente=9300); heffingskortingen volledig afgebouwd boven de afbouwgrenzen 2026; tariefsaanpassing = min(aftrekpost, gross − topschijfgrens) × (topschijftarief − hypotheekAftrekMaxTarief), beide afgeleid uit BOX1_PARAMS[2026]. TITEL (kop-herziening 19-09-2026): app/(app)/overzicht/belasting/box1/page.tsx leest `loadLeverScores(...).box1Status` — dezelfde bron als het statuspunt naast de `i` — en vertaalt die met `box1JaarruimteVerdictFromStatus` naar de zin voor `BelastingBoxPageHeader` (`PageVerdictOpening`). Sinds 22-09-2026 (ADR 0177 + eigenaarsbesluit) loopt de stip WÉL op de canonieke bruto: `loadLeverScores` leidt hem af met `resolveLeverGrossYearly` (`grossFromNet`, mét voorrang voor de handmatige override `profiles.box1_gross_income`) en voedt daarmee `computeJaarruimte` → `box1JaarruimteVerdict`. Daarvóór stond achter de stip een lineaire vuistregel (netto / (1 − marginaal tarief)) die bruto met ruim 50% kon OVERschatten en de override niet kende, waardoor titel en stip elkaar konden tegenspreken. Gemeten gevolg van de omzetting: de stip verschuift alleen onder ± € 1.800 netto/mnd (oranje → groen); daarboven is de verdict-tak grof genoeg om zelfs € 14.000 heffingsverschil op te vangen. De drie uitkomsten zelf zijn ongewijzigd — "Onbenutte jaarruimte" (warn) bij jaarruimte>0, "Ruimte benut" (good) bij 0, "Inkomen onbekend" (neutral) zonder data; dezelfde afleiding als de Box 1-kaart op de hub (zie lib/jaarruimte.ts#box1JaarruimteVerdict, WF-BELAST-01).',
    },
  },
  {
  ...
  },
  {
    ...
    then: 'De relevantie-detectie (`relevant` op `loadBox2Materiality`, dezelfde breedte als het vervallen `hasBox2Relevance`: deelneming/DGA-vordering/DGA-schuld) geeft false → de pagina toont de rustige empty-state ("Geen aanmerkelijk belang gevonden") i.p.v. een lege rekenkaart. Server-side DB-detectie zonder cijfermatige uitkomst; `calculateBox2` met lege deelnemingen zou €0 geven maar wordt hier niet getoond. TITEL (kop-herziening 19-09-2026, `PageVerdictOpening` via `BelastingBoxPageHeader`): bij `!relevant` (dit criterium) luidt het oordeel "Niet van toepassing" (`neutral`); bij wél relevantie is het "Heffing over je belang" (`warn`, zodra `material`) of "Geen heffing dit jaar" (`neutral`) — bewust GEEN bedrag in de titel, want `loadBox2Materiality` is user-scoped terwijl `Box2Detail` in huishoudweergave het gecombineerde bedrag toont (een euro in de kop zou dan de grondslag eronder tegenspreken). Dezelfde loader voedt banner én titel (bevinding L8), dus ze kunnen niet uiteenlopen.',
    assertion: {
      kind: 'ui-only',
      source: 'lib/box2-relevance.ts#loadBox2Materiality (relevant/material, DB-query + calculateBox2) → false voor Willem → empty-state + titel "Niet van toepassing"; geen persona-cijfer uit een rekenmotor. app/(app)/overzicht/belasting/box2/page.tsx + components/overview/belasting-box-page-header.tsx (PageVerdictOpening-wrapper).',
    },
  },
  {
  ...
  },
  {
    ...
    assertion: {
      ...
      source: 'lib/box3-data.ts#calculateBox3 (year 2026, hasPartner false) op Willem: bankrekeningen als cash-assets (seed-persona zet bank_accounts → asset_type cash) + persona.assets; forfaits 2026 spaargeld 0,0128 / beleggingen 0,0600, heffingsvrij single €59.357, tarief 36%. TITEL (kop-herziening 19-09-2026): app/(app)/overzicht/belasting/box3/page.tsx leest `loadLeverScores(...).box3Status` (dezelfde bron als de sidebar-dot en de Box 3-kaart op de hub, geen tweede `computeBox3TaxableInput`-aanroep) en geeft de zin via `box3StatusVerdict` (lib/box3-taxable-input.ts) door aan `BelastingBoxPageHeader`: "Binnen de vrijstellingsruimte" (good) / "Boven de vrijstelling" (warn) / "Ruim boven de vrijstelling" (bad) / "Geen Box 3-vermogen" (neutral) — bij dit vermogen ruim boven de vrijstelling (grondslag €625.343 > 0) dus `warn`/`bad`, afhankelijk van de exacte statusdrempel in box3TaxStatus.',
    },
  },
  {
  ...
  },
  {
    ...
    assertion: {
      ...
      source:
        'lib/tax-optimizer/box3-strategies.ts#generateBox3Strategies (samenstelling-shift, returnCostEur/netEffect) + rank.ts#pickTopChoice (netEffect≤0 valt af, netFreedomDays wint) + box3-strategies.ts#buildCurrentStanding, op het echte Box3Result van Willem (calculateBox3, year 2026); jaarruimte-kans representatief via computeJaarruimte/jaarruimteBesparing op het bruto-inkomen van persona compleet (zelfde representatieve-cijfer-precedent als WF-BELAST-13). TITEL (kop-herziening 19-09-2026): deze pagina heeft geen stoplicht (een fiscale kans is geen "op koers/aandacht/actie"); app/(app)/overzicht/belasting/optimizer/page.tsx toont daarom `kansen.topChoice.netEffect` als KPI ("€X per jaar", `tone="neutral"`, géén kleuroordeel) — bij Willem dus het netto effect van de jaarruimte-kans, want de Box 3-shift met netEffect≤0 wordt nooit topChoice. Zonder positieve leidende kans (`topNetEffect<=0`) is de titel de kale paginanaam "Fiscale kansen". De deck vertaalt hetzelfde bedrag naar vrijheidstijd via `calculateFreedomTime`/`kansen.dailyExpenses` (canoniek dagtarief uit dezelfde loader-bundel, geen eigen som).',
    },
  },
  {
