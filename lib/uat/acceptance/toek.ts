...
const criteria: AcceptanceCriterion[] = [
  ...
  },
  {
    ...
    then: 'De uitleg-sheet toont "Gehanteerde rente: 5,0%" — de default EXCEL_TEKORT_LENING_RENTE (0,05), omdat Willem geen eigen deficit_loan_rate heeft. Dit is een exacte parameter-echo. Het ontstaans-/piekjaar van het tekort zijn kernel-uitkomsten (oracle). DISCREPANTIE/onbevestigd (uat2-toek.md): of het tekort daadwerkelijk optreedt hangt van de portefeuillegrootte in het venster af — als de rij niet verschijnt, documenteren als bevinding. TITEL (kop-herziening 19-09-2026, `ToekomstSubpageShell`/`PageVerdictOpening`): de vervallen kicker/H1 ("Welke gebeurtenissen verschuiven je vrijheid?") is vervangen door een kerncijfer, GEEN oordeel — "N gebeurtenissen" (enkelvoud bij 1), geteld op `horizonData.events.length`, dezelfde rijen als de view rendert en dezelfde telling als de Gebeurtenissen-navkaart op /toekomst; zonder gebeurtenissen is de titel de kale paginanaam "Gebeurtenissen" (`verdict={null}`). Neutrale inkt (`tone` niet gezet) — een gebeurtenis is geen stoplicht-oordeel.',
    assertion: {
      kind: 'exact',
      expected: 'tekortLeningRentePct=5.0',
      source: 'EXCEL_TEKORT_LENING_RENTE (lib/horizon-kernel/adapter/defaults.ts = 0.05) — default omdat PERSONAS.willem geen deficit_loan_rate-override heeft. Titel: app/(app)/toekomst/gebeurtenissen/page.tsx (horizonData.events.length → ToekomstSubpageShell.verdict).',
    },
  },
  {
    workflow: 'WF-TOEK-18',
