...
const criteria: AcceptanceCriterion[] = [
  ...
  },
  {
    ...
    then: 'De uitleg-sheet toont "Gehanteerde rente: 5,0%" — de default EXCEL_TEKORT_LENING_RENTE (0,05), omdat Willem geen eigen deficit_loan_rate heeft. Dit is een exacte parameter-echo. Het ontstaans-/piekjaar van het tekort zijn kernel-uitkomsten (oracle). DISCREPANTIE/onbevestigd (uat2-toek.md): of het tekort daadwerkelijk optreedt hangt van de portefeuillegrootte in het venster af — als de rij niet verschijnt, documenteren als bevinding. TITEL (kop-herziening 19-09-2026, gecorrigeerd 20-09-2026; `ToekomstSubpageShell`/`PageVerdictOpening`): de vervallen kicker/H1 ("Welke gebeurtenissen verschuiven je vrijheid?") is vervangen door de kale paginanaam "Gebeurtenissen" (`verdict={null}`) — géén kerncijfer in de titel. Het aantal staat precies één keer, in de sectiekop van de view ("N gebeurtenissen", enkelvoud bij 1), geteld op de rijen die de tijdlijn rendert: de server-events plus het door de kernel afgeleide verkoopmoment ("Berekend door je plan"), dat alleen client-side bestaat. Een server-geteld getal in de titel (`horizonData.events.length`) telde dat verkoopmoment niet mee: "2" in de kop, drie kaarten eronder. De sectiekop heeft geen kicker meer.',
    assertion: {
      kind: 'exact',
  ...
  },
  {
    ...
    then: 'Doelen toont een status-dot o.b.v. het slechtst presterende actieve doel; Gebeurtenissen toont het EERSTVOLGENDE moment: KPI = alleen het tijdstip ("2028", of "leeftijd 67" bij een event zonder datum), substext = "Volgende: <naam>"; zonder geplande events "Geen" + "Nog niets gepland", net als de Doelen- en Rekenhulp-kaart in dezelfde rij. De naam staat bewust NIET in de KPI-plek: dat is een cijfer-slot (`font-serif tabular-nums`) zonder afkapping, en een eventnaam mag 100 tekens zijn. De drilldown heet "Volgende", toont hetzelfde tijdstip en draagt de volzin "Volgende: <naam> · <jaar/leeftijd>" als tip. Deze kaart draagt bewust GÉÉN aantal (eigenaarsbesluit 20-09-2026): de tijdas rendert naast de server-events ook momenten die de kernel zelf uit het plan afleidt ("Berekend door je plan", alleen client-side), dus een server-geteld aantal stond structureel lager dan de telling in de sectiekop van de view — dezelfde tegenspraak die WF-TOEK-17 uit de paginatitel haalde. Voorkeuren toont strategienaam + SWR%. Drilldown = accordeon (één tegelijk); kaart-klik navigeert naar de subroute. In "Eenvoudig"-modus is de Rekenhulp-kaart verborgen en zijn de chevrons uitgeschakeld. Pure navigatie/weergave.',
    assertion: {
      kind: 'ui-only',
    },
  },
  ...
  },
  {
    ...
    then:
      'GRONDSLAG-CONSISTENTIE op het scherm: de getekende lijn, de FIRE-drempellijn waar hij naartoe loopt, de Monte-Carlo-band eromheen en het vrijheids-% eronder dragen alle vier dezelfde grootheid (D1/D7). Concreet: (a) de FIRE-stip LANDT op de FIRE-doellijn i.p.v. de volle overwaarde erboven — op de FIRE-maand geldt J == `requiredFirePortfolio` (ADR 0034-endpoint-invariant, vergrendeld in lib/horizon-kernel/fire-basis-invariant.test.ts) en de stip interpoleert nu over de J-punten; (b) de marktcheck-band komt uit de J-variant `bandLiquide` — een I-band om een J-lijn wordt NIET getekend (D7: liever geen band dan een gemengde); (c) elke FIRE-drempel wordt alleen getoond zolang de lijn met háár grondslag op het scherm staat; ook fase-splits, AOW-stip en gebeurtenis-markers verhuizen mee naar de J-lijn (D2). (d) De PILL naast de Doel-pill schakelt altijd de TWEEDE lijn en benoemt die: "Met je huis" bij Uitsluiten, "Zonder je huis" bij Verkopen/Opeethypotheek; bij Meerekenen bestaat hij niet. Hij staat in ÁLLE strategieën standaard UIT (D5) en bewaart zijn stand onder een eigen voorkeur-sleutel, zodat een oude "besteedbaar-lijn uit"-keuze niet stil de totaallijn uitzet. (e) De jaar-kassabon blijft BEWUST de volledige jaarbalans op de I-grondslag (D3: een bon is een balans, geen lens — het huis staat erop omdat je het bezit) met bij Uitsluiten één neutrale "waarvan besteedbaar"-regel op twee plekken: onder het hoofdcijfer én onder "Eind netto". Die regel doet NIET mee in de waterval en de bon blijft sluiten op `row.netWorth`. AANVULLING 19-09-2026 (bevinding L1 — de bon herhaalde de uitspraak van de gedempte staaf niet): zodra die besteedbaar-regel staat, MARKEERT de bon ook de regels die het betreft — de bezitting "Eigen woning" én elke hypotheek die aan dat huis gekoppeld is (`housingContext.eigenHuisMortgages`, dezelfde Set als de Opbouw-grafiek gebruikt) krijgen naam en bedrag in `--ink-meta` plus de sub-regel "telt niet mee voor je doel — je woont er". Dat is woordelijk DEZELFDE zin als de duidingsregel onder de grafiek (gedeelde constante `BUITEN_DOEL_ZIN` in lib/wealth-composition.ts); de DRAGER verschilt bewust — een bon is tekst, een staaf een vlak. Een hypotheek op een ánder pand blijft ongemarkeerd, net als in de grafiek. Daarboven staat nu ook de grondslagzin die D3 al voorschreef maar die nooit gerenderd werd: "Je huis staat op deze bon omdat je het bezit — het telt alleen niet mee in je vrijheidsdoel." De markering verandert GEEN bedrag en GEEN optelling: er komt geen extra regel bij, het huis blijft in "Eind netto", en de bon sluit onveranderd op I. Bewust GEEN reconciliatieblok "I − huis + hypotheek = besteedbaar" (L/M staan niet op `UnifiedProjectionRow`, en het zou niet sluiten bij twéé hypotheken op één huis). Conditie is de WEERGAVE-vlag `primaryBasis === \'liquid\'`, niet de kernelvlag — die staat óók bij Verkopen/Opeethypotheek aan. Gevolg van die keuze, bewust vastgelegd: onder de D6-terugval (partner-/huishoud-/AOW-stop-lijn → `\'total\'`) vervalt met de besteedbaar-regel óók de markering, terwijl de Opbouw-grafiek daar wél dempt; bon-intern consistent gaat hier vóór grafiek-consistent, omdat een markering zonder de J-regel ernaast een grondslag zou beloven die nergens op de bon staat. (f) De crosshair-tooltip zet "Zonder je huis" als primair bedrag met "Met je huis" gedimd eronder; de zes drijvers/drukkers blijven staan onder de grondslagkop "Wat er dit jaar gebeurde (mét je huis)" (D4 — `SimRow.growth` bevat daar de woningwaardestijging; gelabeld verschil, geen fout). Toetsvorm consistentie: alle cijfers zijn kernel-uitkomsten (oracle via /beheer/horizon-kernel); wat hier getoetst wordt is dat ze op één grootheid staan.',
    assertion: {
      kind: 'consistency',
      source:
        'consistentie-eis: lijn ∧ FIRE-drempel ∧ band ∧ vrijheids-% dragen dezelfde grootheid. Bron van de keuze: lib/horizon/liquid-wealth-line.ts#primaryChartBasis → `SimChartGeometryInput.primaryBasis` (lib/horizon/sim-chart-geometry.ts, default \'total\') → components/app/horizon/horizon-client.tsx#effectiveChartPrimaryBasis (D6-terugval bij partner-/huishoud-/AOW-stop-lijn) + bandLiquide; pill/tooltip in components/app/horizon/sim-chart.tsx; "waarvan besteedbaar"-regel + buiten-doel-markering (`buitenDoel` op `AssetTypeRow`/`DebtRow`, prop `eigenHuisMortgageIds`, grondslagzin) in components/app/horizon/horizon-year-details-sheet.tsx (consume-only `row.nettoLiquide` en de Set uit horizon-client.tsx#eigenHuisMortgageIds, geen parallelle som); gedeelde zin/inkt met de Opbouw-grafiek via lib/wealth-composition.ts#BUITEN_DOEL_ZIN/BUITEN_DOEL_INKT; gepind in components/app/horizon/horizon-year-details-sheet.test.tsx. FIRE-stip-op-drempel = ADR 0034-invariant, vergrendeld in lib/horizon-kernel/fire-basis-invariant.test.ts.',
    },
  },
  {
    workflow: 'WF-TOEK-38',
  ...
  },
  {
    ...
    then:
      'De doelsectie BLIJFT zichtbaar na het loslaten. Sinds 20 sep 2026 is dat structureel in plaats van geplakt: `verkenSectieZichtbaar` hangt alleen nog aan het perspectief (solo, geen partner-/huishoudlijn) en niet meer aan `displayMode`/`doelActief`, dus ook in Eenvoudig staat het blok er. De sessie-vlag `doelLosgelatenDezeSessie` die dit eerder opving is daarmee vervallen; de bron-grendel `horizon-client.doel-loslaten-terugweg.test.ts` bewaakt nu dat de afleiding niet opnieuw aan het doel of de modus gaat hangen. De knop "Maak dit mijn doel" verschijnt óók wanneer er alleen een stopkeuze is (`hasScenario || hasStopKeuze`), niet meer uitsluitend bij sliders — de doelvastleg-sheet kent die vorm wél (de fire-preview hangt aan `stand.stopAge`/`stopKoppel`). Legt de gebruiker opnieuw vast, dan verandert er aan de zichtbaarheid niets: die hangt niet aan het doel. De perspectief-gate blijft: in partner-/huishoudweergave bestaat het blok niet. Katern III ("Wat het betekent") hangt niet meer aan het in-/uitklappen van het doelscenario (dat klapt niet meer in, ADR 0170) en is dus niet meer "doel dicht = alles dicht". Verdwijnt de sectie na loslaten alsnog, of blijft "Maak dit mijn doel" weg bij een kale stopkeuze, dan is dat een regressie.',
    assertion: {
      kind: 'ui-only',
      source:
        'components/app/horizon/horizon-client.tsx (de perspectief-only afleiding `verkenSectieZichtbaar`, zonder sessie-vlag en zonder displayMode-gate, en de `hasScenario || hasStopKeuze`-gate op "Maak dit mijn doel") — sessiegebonden zichtbaarheidsgedrag zonder cijfermatige uitkomst; bewaakt in `components/app/horizon/horizon-client.doel-loslaten-terugweg.test.ts`.',
    },
  },
  {
    workflow: 'WF-TOEK-44',
  ...
  },
  {
    ...
    titel: 'Verkenning met de stop-knop tot plan maken met "Maak dit mijn stopmoment" (TPR-09)',
    kriticiteit: 'KERN',
    persona: 'willem',
    given:
      'Persona Willem op /toekomst met een plan dat stopt op zijn AOW-leeftijd (anker `aow`, eindleeftijd 90, eind-vorm "Niets, het mag op zijn"). Bij het doelscenario staan twee stopleeftijden: de stand die hij met de stop-knop verkent (scenario-voorkeur, niet het plan) en het anker in het plan (`profiles.fire_stop_anchor`/`fire_stop_age`). VOORHEEN stond er geen brug tussen die twee, en heette de marker niet wat hij is.',
    when:
  ...
  },
  {
    ...
    then:
      '(a) Onder bedrag en type staat "Tot wanneer" met "Blijft doorlopen" (standaard voor een nieuwe eigen gebeurtenis) en "Tot ik stop met werken"; de helptekst noemt per keuze wat er gebeurt en voor welk soort geld hij past. (b) De keuze staat na heropenen nog op "Tot ik stop met werken" (`life_events.metadata.tot_stopmoment = true`). (c) De Duur-figuur leest "tot stopmoment (58,5)" — het stopmoment van de run mét dit event, op halve jaren (vastStopLeeftijd ?? fireAgeFractional, nooit de afgeronde fireAge); de tooltip op de tijdstrip onder de grafiek en de chart-tooltip lezen "+€1.000/mnd · tot stopmoment (58,5)" (nooit meer "· 0 mnd"), de +N-cluster-sheet en de lijst op /toekomst/gebeurtenissen noemen dezelfde looptijd; wisselt hij naar een vast anker (bv. AOW 67) dan volgt de tekst het anker ("tot stopmoment (67)"); is het plan onbereikbaar (geen stopmoment binnen de horizon) dan staat er géén leeftijd maar "tot stopmoment — nog geen stopmoment binnen je plan"; ná het stopmoment staat het bedrag niet meer in de tooltip van Inkomen & Uitgaven. (d) Bij "Blijft doorlopen" staat het bedrag er wél, onder "Inkomsten uit gebeurtenissen" (nooit meer "AOW & pensioen" vóór de AOW-leeftijd); de sheet "Maak een levensgebeurtenis" toont "Tot wanneer" met "Tot ik stop met werken" voorgeselecteerd en de uitleg eronder. Bestaande gebeurtenissen zonder keuze rekenen ongewijzigd door.',
    assertion: {
      kind: 'ui-only',
      source:
        'components/app/horizon/event-pane-edit.tsx (`UntilToggle`) + lib/horizon/event-pane-edit-form.ts (`contUntilStop`, `buildDraftEvent`) + components/app/horizon/event-pane-view.tsx + components/future/calculator-to-life-event-sheet.tsx (`defaultUntilStop`) + lib/income-expense-breakdown.ts (`FIXED_LABELS`) + lib/horizon/event-duration-copy.ts (`describeEventDuration`, `eventStopAgeFromSim`; getoetst in event-duration-copy.test.ts) geconsumeerd door events-timeline.tsx, event-cluster-sheet.tsx, components/future/gebeurtenissen-view.tsx en het `detail`-veld van `chartEventOverlay` in horizon-client.tsx; het rekengedrag (afkap op fireMonth − 1, inert zonder vlag) is exact getoetst in lib/horizon-kernel/geb-eind-bij-stopmoment.test.ts.',
    },
  },
  {
    workflow: 'WF-TOEK-49',
  ...
  },
  {
    ...
    titel: 'Opbouw-grafiek kleurt schulden per soort en markeert het uitgesloten eigen huis mét zijn hypotheek (ADR 0148 + bugfix 18-09-2026)',
    kriticiteit: 'OVERIG',
    persona: 'willem',
    given: 'Persona Willem met een hypotheek, een tekort-lening-episode in de projectie en (in het opeethypotheek-scenario) een opeethypotheek-saldo. De opbouw-weergave van de tijdas-grafiek (`components/app/horizon/wealth-composition-chart.tsx`) toont schulden als onderdeel van de gestapelde balk. Voor subscenario (b): bij woonstrategie UITSLUITEN (`exclude_from_fire`; prop `homeExcludedFromFire` = `hasEigenHuis && isHomeExcludedFromFire`, gezet in horizon-client.tsx) draagt elke rij bovendien `vastgoedEigenHuis` en `schuldEigenHuisHypotheek` (lib/wealth-composition.ts#unifiedRowsToStackedRows, gevuld uit `housingContext.eigenHuisMortgages`). Willem heeft dáár naast de eigen woning óók beleggingsvastgoed en/of een hypotheek op een ánder pand — zonder die twee toetst (b) alleen de markering en niet de GRENS, en juist die grens is het punt.',
    when: 'De gebruiker schakelt de grafiek naar "Opbouw". (a) Hij leest de schuld-segmenten en hun legenda per jaar. (b) Hij zet de woonstrategie op Uitsluiten, leest de staaf, de legenda en de regel eronder (op desktop ook de hoverstrip), en zet de strategie daarna weer terug op Meetellen — de persona mag niet in Uitsluiten achterblijven (raakt WF-TOEK-06/07).',
    then: '(a) De schuld-segmenten zijn per soort gekleurd i.p.v. één ongedifferentieerd schuld-blok — hypotheek, overige schulden, opeethypotheek en tekort-lening dragen elk hun eigen, consistente kleur en naam in de legenda/tooltip. Puur weergavegedrag: de onderliggende bedragen komen ongewijzigd uit dezelfde kernel-rijen als de "Pad"-weergave. (b) HET GEDEMPTE EIGEN-HUIS-PAAR. Bij Uitsluiten BLIJFT het huis in de staaf staan — het is echt bezit en het netto vermogen hoort eerlijk te blijven — maar als eigen, gemarkeerd segment "Eigen woning", afgesplitst van de vastgoedband; de hypotheek op datzelfde huis wordt op dezelfde manier afgesplitst als "Hypotheek eigen woning". Beleggingsvastgoed ("Vastgoed") en een hypotheek op een ánder pand ("Hypotheek") blijven op volle sterkte — exact de grens van `filterAssetsForFire`. De legenda krijgt die twee regels erbij en daaronder verschijnt PRECIES DAN de duidingsregel; valt de schuldopsplitsing niet sluitend uit, dan valt de schuld terug op één ongedeeld "Schulden"-blok en wordt er géén eigen-huis-hypotheek gemarkeerd — en dan hoort de duidingsregel er ook niet te staan (afgeleid uit de GEPRODUCEERDE segmenten, niet uit de rij-velden). Bij Meetellen, of zonder eigen woning: één ongedeelde vastgoedband, geen "Eigen woning"-regel, geen duidingsregel — staaf identiek aan voorheen. De markering is WEERGAVE: vrijheids-%, doelbedrag en de "Pad"-lijn veranderen er niet door. MARKERING = DRIE DRAGERS (besluit 19-09-2026, WCAG 1.4.1): het huis-segment én zijn hypotheek-segment dragen elk DRIE dragers, in deze volgorde van betrouwbaarheid — (1) TEKST: een eigen legenda-regel "Eigen woning"/"Hypotheek eigen woning" plus de duidingsregel "Gearceerd en gedempt = telt niet mee voor je doel — je woont er." (één gedeelde zin-constante `BUITEN_DOEL_ZIN` in lib/wealth-composition.ts, woordelijk gelijk aan de sub-regel in de jaar-kassabon; één inkttoken `BUITEN_DOEL_INKT` = --ink-meta op beide oppervlakken; één naam, uit ASSET_TYPE_LABELS); (2) ARCERING: een SVG-`<pattern>`-arcering (45°, tegel 4px) over exact die twee segmenten en niet over de meetellende buren, met dezelfde arcering op de legenda- én tooltip-swatch zodat de legenda de sleutel blijft; (3) DEKKING (0,45) als derde drager. Dekking mag NOOIT de enige drager zijn: nagerekend haalt ze nergens de 3:1 van WCAG 1.4.11 (huis vs papier 1,58–1,82:1; vs de stapelbuur "Overig" 1,09–1,18:1 over 19 accenten). MOBIEL-VOORBEHOUD: op ~360px is een jaarstaaf ≈3px breed en is de arcering marginaal — daar is de TEKST (legenda, duidingsregel, kassabon) de primaire drager, en dat is geen tekortkoming maar de bedoelde rangorde. De arcering blijft chart-only; de kassabon markeert hetzelfde met tekst.',
    assertion: {
      kind: 'ui-only',
      source: 'components/app/horizon/wealth-composition-chart.tsx#positiveSegments + debtSegments (segment-kleuren per schuldsoort ADR 0148; gedempt+gearceerd eigen-huis-paar EIGEN_HUIS_SEGMENT/EIGEN_HUIS_HYPOTHEEK_SEGMENT, GEDEMPTE_DEKKING, GEDEMPTE_ARCERING_TEGEL, gedempteSwatchStijl, duidingsregel) + lib/wealth-composition.ts#unifiedRowsToStackedRows (vastgoedEigenHuis/schuldEigenHuisHypotheek) + lib/wealth-composition.ts (BUITEN_DOEL_ZIN / BUITEN_DOEL_INKT / EIGEN_WONING_LABEL, gedeeld met de jaar-kassabon — zie WF-TOEK-37 (e)) — consumeert bestaande kernel-uitkomsten zonder herberekening. Grendels: components/app/horizon/wealth-composition-chart.test.tsx en lib/derive-wealth-composition.test.ts.',
    },
  },
  {
    workflow: 'WF-TOEK-54',
  ...
  },
  {
    ...
    titel: 'Haalbare uitgave na pensioen: duidingsregel in de "Na pensioen"-tegel, één grondslag met de knop (ADR 0160, bijgesteld door ADR 0170)',
    kriticiteit: 'KERN',
    persona: 'willem',
    given:
    ...
    assertion: {
      ...
      source:
        'lib/horizon/haalbare-uitgave.ts#HAALBARE_UITGAVE_DREMPEL (250; de bisectie-precisie € 50/jaar en de bovengrens-factor 3× zijn module-privé constanten, gepind in lib/horizon/haalbare-uitgave.test.ts) + lib/scenario-events.ts#UITGAVE_NA_PENSIOEN_STAP (600) + lib/horizon/anker-copy.ts#haalbaarBijUitgaveRegel op synthetische HaalbareUitgave-fixtures (géén kernel-run — de tekst-/kleur-/drempellaag is puur; de bisectie zelf is kernel-bewezen in lib/horizon/haalbare-uitgave.test.ts en lib/horizon/scenario-presets.haalbare-uitgave.test.ts) — zie toek-checks.ts. Kleurtokens (text-negative/text-positive) zijn de bestaande semantische tokens (CLAUDE.md kleurconventie); de bron-test op de tegels bewaakt dat er geen Tailwind-standaardkleur of losse hex in de nieuwe markup staat.',
    },
  },
  {
    workflow: 'WF-TOEK-58',
    scenarioId: 'UAT-TOEK-58',
    titel: 'Het doelscenario is vijf knoppen met een driekleurige schaal: de grens staat op de knop en beweegt mee (ADR 0170)',
    kriticiteit: 'KERN',
    persona: 'willem',
    given:
      'Persona Willem op /toekomst. Het doelscenario staat IN de grafiekkaart, direct onder de fasering: één blok met de vraag van het plan ("Kun je op 62 stoppen?" resp. "Wanneer kun je stoppen?"), een uitkomstregel, vijf knoppen — Meer verdienen · Minder uitgeven · Uitgave na pensioen · Nalatenschap · Stopleeftijd — in vijf vormen te zetten met de schakelaar in de kop (B7/B11: Wijzers = vijf halfronde meters, Balken = vijf balken, Rad = één rij van ~90 px met links een verticaal draairad dat het onderwerp kiest — vegen, met per onderwerp een stoplichtpunt — en rechts één balk voor dat onderwerp, Harp = vijf stroken van 44 px met een doorgetrokken lijn door de duimen en een gestreepte lijn door de gedekt-merken (B12, mobiel; de gestreepte lijn breekt waar een grens buiten bereik ligt), Vijfhoek = één pentagram met sleepbare hoekpunten, een gestreepte gedekte vijfhoek en een legenda waarvan de regels de echte range-inputs dragen (B12, laptop); de keuze staat server-side in `knopWeergave`, één lijst `KNOP_WEERGAVEN`) — een ingeklapt blok "Rendement per categorie" en onderaan één opslaan-balk. Elke knop draagt dezelfde driekleurige schaal: rood = het plan reikt niet, oranje = gedekt met minder dan 10 % marge, groen = ruim gedekt. De twee grenzen per knop komen uit `computeLabGrenzen` (lib/horizon/lab-grenzen.ts), die in de kernel-worker op lane `grenzen` draait en per knop bisecteert met ÉÉN predicaat: `solveFire` met het stop-anker op de stand van de stop-knop, gedekt zodra de status geen shortfall is. Twee subscenario\'s: (a) een vast stopmoment (`aow`/`age`), waar de 10 %-marge een langere REIKWIJDTE eist — het plan moet ook reiken tot `eind + 10 % × (eind − planStop)`; (b) "zo vroeg als het kan" (`solved`), waar de marge 10 % van de jaren tot de vrijheidsleeftijd is.',
    when:
      'De gebruiker verschuift één knop (bv. Meer verdienen omhoog) en kijkt eerst naar de grafiek erboven, en leest daarna (1) de kleuren en de grensregel ("gedekt vanaf … · ruim vanaf …") onder ÉLKE knop, (2) het zone-woord naast de vraag, (3) de uitkomstregel, en (4) de opslaan-balk. Daarna legt hij de stand vast ("Maak dit mijn doel"), verschuift opnieuw, herstelt zijn doel, en herlaadt de pagina.',
    then:
      'Zodra de eerste knop beweegt verschijnt de gestippelde wat-als-lijn vanzelf in de grafiek (B9), met daartussen een gearceerd verschilvlak: groen waar de wat-als hóger ligt dan de basislijn, rood waar hij láger ligt, en op een kruising wisselt de kleur op het sníjpunt zelf — nooit een rood vlak bóven de basislijn. Zet de gebruiker die lijn daarna bewust uit, dan blijft hij uit tot hij terug naar basis gaat. De grenzen van álle knoppen verschuiven mee — één knop bewegen verandert wat de andere vier nodig hebben, want elke bisectie houdt de overige knoppen op hun huidige stand. Zolang de nieuwe batch loopt blijven de vorige grenzen staan op halve dekking (`pending`, `aria-busy`), zodat er geen lege as knippert en niets verspringt. Een grens buiten het bereik van de knop wordt niet als getal genoemd maar als reden ("de grens ligt boven het bereik van deze knop"); een niet-monotone dekking (woningverkoop, potregel) levert `null` en een grijze as — liever geen grens dan een grens die niet klopt. MERKTEKEN (20 sep 2026): op de grens rood→oranje ("gedekt") staat op de schaal van elke knop een merkteken in volle inkt, hoger dan de kleursegmenten, dat met dezelfde duur meeschuift (merkteken en kleurgrens lezen als één beweging); ligt de grens buiten het bereik van de knop dan staat er géén merkteken (net als de grensregel die dan een reden noemt). GREEP: op de meter zit een bolletje op de naaldstand (module-accentkleur, geen stoplicht-semantiek) waarmee je de knop op hoek sleept; het schrijft via dezelfde onChange als het invoerveld (geen tweede schrijfpad) en snapt op de stap van de knop. De knop Nalatenschap verschijnt alleen bij eind-vorm `legacy`; bij "vermogen in stand houden" staat er één regel die dát zegt in plaats van een stille lege plek, en de knop Stopleeftijd verdwijnt onder het `now`-anker. De opslaan-balk doorloopt vier standen: niets verschoven ("Verschuif een knop om een doel te maken") → nog niet opgeslagen (Maak dit mijn doel · Terug naar basis) → opgeslagen als je doel op {datum} (Doel loslaten) → gewijzigd t.o.v. je doel (Doel bijwerken · Herstel mijn doel · Doel loslaten); onder het `now`-anker zegt de balk dat het lab daar geen doel vastlegt (ADR 0145 D6). Na een herlaad staan alle vijf knoppen terug: `uitgaveNaPensioen` en `nalatenschap` reizen sinds ADR 0170 mee in de scenario-pref én in `doel.stand`, dus "Herstel mijn doel" zet ook die twee terug. De UITKOMSTmaat is ONGEWIJZIGD t.o.v. WF-TOEK-49: `plan_coverage` bij een tekort, `end_balance` bij een gedekt plan, `fire_age` onder `solved`. VIJF KNOPPEN = VIJF DOELEN (20 sep 2026, defect): de sheet "Maak dit mijn doel"/"Werk je doel bij" toonde tot dan maar twee aanvinkbare rijen (Spaarquote en Vrijheidsleeftijd) terwijl de gebruiker alle vijf knoppen verschoof — het open punt uit ADR 0170 ("uitgave na pensioen en nalatenschap worden géén doeltype") is teruggedraaid. Elke verschoven knop levert nu een rij, in de schermvolgorde van de knoppen: Extra inleg (`extra_deposit`, €/mnd) · Spaarquote (`savings_rate`) · Uitgave na pensioen (`retirement_expense`, €/jaar) · Nalatenschap (`legacy_amount`, €) · het anker-uitkomstdoel. De drie nieuwe zijn plan-PARAMETERS en hangen dus NIET aan het stopmoment (ze verschijnen ook onder `solved`, waar `dekking` juist geweigerd wordt); een knop die op nul of negatief staat levert géén rij, en "Doel loslaten" ruimt ook deze drie op. VOORTGANG: twee van de drie worden GEMETEN aan de PLAN-INSTELLING waar ze over gaan, geconsumeerd uit de kernel-run die de doelsync tóch al doet (`VrijheidsgetalSnapshot.uitgaveNaPensioenPerJaar` = `KernelInput.inkomenUitgaven.uitgaveNaPensioenPerJaar` van díe run; `planLegacyAmount` = `FirePlan.legacyAmount` bij eind-vorm `legacy`, anders 0). Het doel sluit dus zodra het plan de verandering draagt die je vastlegde — verandert de gebruiker zijn uitgave na pensioen op /toekomst, dan beweegt de doelkaart mee zonder dat er iets herberekend wordt. `extra_deposit` heeft bewust GEEN meting (een storting is in de transactiehistorie niet van sparen te onderscheiden) en toont daarom zijn streefbedrag mét de notitie `extraInlegGoalNoMetricNote`; die kaart geeft geen 0%-balk en geen rode status, en telt NIET mee in de melding "doelen buiten je plan" (de notitie zegt "niet te meten", niet "past niet bij je plan"). Ziet de tester een extra-inleg-kaart op "€0 van €500 · 0%" staan, of een uitgave-na-pensioen-kaart die niet meebeweegt met het plan, dan is dat een regressie. Verschijnt een verschoven knop níét in de sheet, dan is dat de regressie die dit criterium bewaakt.',
    assertion: {
      kind: 'direction',
      source:
        'lib/horizon/lab-grenzen.ts#computeLabGrenzen (de bisectie + het ene predicaat; richting/monotonie bewezen met de ECHTE motor in lib/horizon/lab-grenzen.kernel.test.ts: op de gevonden grens geen shortfall, één rasterstap aan de verkeerde kant wél, en `ruim` voorbij `gedekt` in de richting van de knop) + lib/horizon/lab-grenzen-types.ts#zoneVanWaarde/zoneVanHuidig (de zone-afleiding, puur) + lib/horizon-kernel/worker/run-in-worker.ts#runLabGrenzenAsync op lane `grenzen` (één batch, verdrongen ⇒ null ⇒ vorige grenzen blijven staan) + components/app/horizon/lab-slider.tsx#labSegmenten (de drie breedtes; richting `dalend` spiegelt) + components/app/horizon/lab-wijzer.tsx (het gedekt-merkteken via posOf/hoekVanFractie, en de sleepgreep `fractieUitPunt`/`snapNaarStap`, bewaakt in lab-wijzer.test.ts en lab-slider.test.tsx) + lib/horizon/scenario-diff-vlakken.ts#buildDiffVlakken (het verschilvlak, incl. de geïnterpoleerde kruising — puur getest in scenario-diff-vlakken.test.ts) + de bron-grendel components/app/horizon/horizon-client.diff-vlak.test.ts (één bron voor het vlak, één grondslag, en de lijn verschijnt alleen op de overgang) + components/app/horizon/lab-opslaan-balk.tsx (de vier standen) + de bron-grendel components/app/horizon/horizon-client.lab-uitkomst.test.ts (geen tweede grens-afleiding in de host: geen solveFire, geen bisectie, geen Math.pow). Kleur: de drie zones gebruiken de semantische stoplicht-tokens (bg-negative/bg-warning/bg-positive), nooit een module-accent — de gebruiker kan ze dus niet instellen (CLAUDE.md kleurconventie).',
    },
  },
]
 ...
 * de knop "Uitgave na pensioen" in het doelscenario (sinds ADR 0170 de derde van vijf); beide
