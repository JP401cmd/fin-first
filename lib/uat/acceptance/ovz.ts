...
const criteria: AcceptanceCriterion[] = [
  {
    ...
    then: 'BOVENAAN STAAT HET ANTWOORD, NIET DE VRAAG (H20/ADR 0130): het eerste blok toont dat antwoord nu LETTERLIJK in de titel. HERZIEN 19-09-2026, 20-09-2026 en 22-09-2026 (kop-herziening; B-069, F4 van "Topbar & oordeelzin"): de kop van `OverzichtHeroPrimary` is de BEGROETING ("Goedemorgen, {naam}", een `EditorialHeadline` als h2) — de hub is bewust de uitzondering op de oordeel-aanhef (eigenaar, 20-09-2026; de kop-herziening van 19-09 maakte er kortstondig "Overzicht | {bandwoord}" van). Het gezondheidsoordeel staat in de DECK eronder, als één lopende zin: "Je financiële gezondheid is {bandwoord}, gezien je {pijlers}." Het bandwoord staat in kleine letters en in de stoplichtkleur uit `healthScoreTone` (health.total ≥60 → good/"groen", ≥40 → warn/"oranje", anders bad/"rood"), de rest van de zin in inkt. {pijlers} noemt ALLEEN de pijlergroepen die in déze score meetellen (`healthScoreBasisPhrase` in `lib/financial-health.ts`, die `health.pillars` leest — de actieve set na module-gating én data-beschikbaarheid), in de vaste volgorde bezittingen, buffer, schulden, uitgaven: met alle modules en gegevens "gezien je bezittingen, buffer, schulden en uitgaven"; met alleen budgetteren "gezien je buffer en uitgaven"; zonder modules "gezien je buffer". De zin noemt nooit "vrijheid" of "oordeel" en geen getal (het getal staat in de gezondheidskaart). Onbekende grondslag of geen score ⇒ de deck is de neutrale zin "Hoe je ervoor staat, in één blik: je vier hefbomen, je gezondheid en je plan." De vroegere tweede zin "De score weegt rondkomen, buffer, schuld en vrijheid." is weg (melding B-069: onbegrijpelijk); de weging staat in de receipt achter de gezondheidskaart en als term "Gezondheidsscore" onder de pagina-`i`. De kicker met de datum blijft erboven staan (ongewijzigd). De maand-check-in staat er ONDER (het `banners`-slot van `OverzichtHeroPrimary`, tussen de aanhef en hefbomen-kompas), in béíde weergaven — de positie is geen modus-keuze. De welkomstgids staat HIER NIET MEER: geen gids-banner en geen geminimaliseerd gids-punt naast de pagina-informatieknop (ADR 0130 — hij woont in Fin, zie WF-OVZ-14). De aanhef, de vier hefboomtegels (incl. netto vermogen en de Box 3-hefboom) ÉN de gezondheidskaart verschijnen DIRECT bij het openen (`OverzichtHeroPrimary`, blok 1 — sinds de kaart "gezondheid & netto vermogen los laden van widgets" hangt de gezondheidskaart niet meer op de zware widget-bundel). Twee losse Suspense-stromen komen daarna in: de mini-vermogen-grafiek (`heroChart`, two-staps: eerst het Vandaag-anker met het al-bekende netto vermogen, dan de volle projectie/historie) en de widget-rail/briefing/snelkoppelingen (`secondary`, editorial skeleton op vaste hoogtes tegen CLS) — zelfde inhoud, gefaseerde verschijning, dezelfde cijfers. Netto vermogen = totalAssets (€1.619.700) − totalDebts (€0) = €1.619.700. Box 3 (synthetisch): grondslagSparen = 100.000 − 59.357 = €40.643; effectiefRendement = forfaitairSpaargeld/rendementsgrondslag = 1.280/100.000 = 1,28% (enige categorie, dus geen blend); box3Income = 40.643 × 0,0128 = €520,23; tax = 520,23 × 0,36 ≈ €187,28. In de Eenvoudige weergave is de hub verder ontdaan van bijzaak (fase 1): de drie "alles bekijken"-links onder de briefing vervallen (de navigatie draagt ze al, OVZ-3) en de legenda van de vermogensgrafiek valt terug op "Verloop" + "Bandbreedte" zonder percentiel-notatie of kaal leeftijdsgetal (OVZ-4) — de uitleg daarvan staat in de pagina-\'i\', samen met de betekenis van de stoplicht-stippen (OVZ-1). De CIJFERS zijn in beide modi identiek; het is uitsluitend presentatie-reductie.',
    assertion: {
      kind: 'exact',
  ...
  },
  {
    ...
    then: 'Schuldratio = 13.900/9.700 = 1,433 (143%) ≥ 1 → pijlerscore 0 → statuskleur rood (via `pillarStatus`, <50 → bad). In de Eenvoudige weergave draagt elke tegel het OORDEEL in gewone taal als primaire regel ("Hoge schuldenlast"; voor Daan op de Belasting-tegel "Niets onbenut" — hij laat geen noemenswaardige fiscale ruimte liggen; zonder gegevens "Nog geen gegevens") met het bedrag gedempt eronder — S1, richtingsbesluit R5 "duiding boven reductie". Dat vervangt de eerdere OVZ-2-reductie (alleen hoofdcijfer + statuspunt), die de status voor screenreader- en touch-gebruikers onbereikbaar maakte. Wél nog steeds weg in Eenvoudig: de chevron/drill-down en de "excl. eigen woning · €X"-regel. In Volledig zijn de tegels ongewijzigd, met één uitzondering: het schulden-oordeel bij warn is gewone taal ("Schuldenlast vraagt aandacht") i.p.v. "Schuldratio {rawValue}" — het ratiogetal staat in de drill-down. Sinds UR2-04 volgt óók het Belasting-oordeel zijn eigen status i.p.v. één vaste waarschuwingszin voor alle drie de statussen; sinds ADR 0177 (22-09-2026) gaan die woorden over ONBENUTTE FISCALE RUIMTE en niet meer over de hoogte van de heffing: "Niets onbenut" / "Ruimte onbenut" / "Veel ruimte onbenut" (was "Belastingdruk beperkt" / "Mogelijk betaal je meer dan nodig" / "Hoge belastingdruk" — die grondslag stond bij hoog vermogen altijd rood en was nooit groen te krijgen), en leest het kompas in de shell hetzelfde generieke statuswoord als de kaart-dot (LEVERAGE_STATUS_LABEL). Sinds UR2-10 geldt bovendien: op een account waar NIETS geregistreerd staat (geen vermogen én geen schuld) is de Schulden-tegel neutraal — dus "Nog geen gegevens" in Eenvoudig en géén oordeelregel in Volledig, in plaats van de eerdere waarschuwing "Schuldenlast vraagt aandacht". Schuldenvrij MÉT vermogen blijft groen.',
    assertion: {
      kind: 'exact',
  ...
  },
  {
    ...
    titel: 'Vrijheidstijd: runway (totaal) + week-delta-plausibiliteitsgrens',
    kriticiteit: 'KERN',
    given: 'Persona Willem (stopt op zijn 45e, eigen eindleeftijd 90, uitgave na stoppen €17.940/jr) en persona Daan (vandaag al zonder liquide vermogen). Plus een week-over-week-scenario op één account: week 1 wordt bevroren op een runway van 124 maanden terwijl de transactiehistorie nog half geïmporteerd is, week 2 draait op de realistische 24 maanden.',
    when: '`summarizeRunway` duidt de kernel-runway voor beide persona\'s; `computeRunwayWeekDelta` bepaalt de week-over-week delta inclusief plausibiliteitsgrens.',
    then: 'ÉÉN GROOTHEID OP /OVERZICHT: DE RUNWAY (ADR 0126 D1, PR C afgerond 2 sep 2026). TOTAAL = de RUNWAY uit de horizon-kernel: "hoe lang kom ik mee als ik nu stop", met rendement, inflatie, AOW, belasting en de eigen woon-/eindstrategie erin. Willem: kind=reaches-end-age, duur "minstens 45 jaar" (ondergrens tot zijn eigen eindleeftijd 90, nooit als exacte duur gepresenteerd), zin "Als je nu zou stoppen, reikt je vermogen tot voorbij je 90e." Daan: kind=deficit levert GEEN meetpunt en GEEN kop — geen claim is beter dan een verkeerde. Dezelfde poort geldt bij unavailable (geen geboortedatum / geen geloofwaardige uitgavenbasis / kernfout) en bij een D7-inconsistentie (eindstrategie \'Vermogen opeten\' die tot de eindleeftijd reikt zonder solverstatus reached_now). WEEK-OVER-WEEK (guard, bug "−3788 dagen minder"): een bevroren basis van 124 maanden tegen een huidige runway van 24 maanden is geen weekbeweging maar een datacorrectie; hij valt buiten de plausibele bandbreedte — |delta| ≥ 365 dagen (FREEDOM_DELTA_MIN_DAYS, in maanden ingevoerd via 365/12) ÉN > 25% van het huidige totaal (FREEDOM_DELTA_MAX_SHARE) — en wordt onderdrukt: deltaMonths=null + isImplausibleDelta=true, waarna het vrijheidsblok van de briefing-mail het totaal zonder weekverschil toont. Een normale week (twee maanden erbij) blijft gewoon zichtbaar. DE MARGINALE DAGDELTA BESTAAT NIET MEER (B-028, eigenaar-besluit sep 2026): de regel "Tegen je huidige uitgaven kwam er sinds gisteren X dagen vrijheid bij" onder de begroeting is VERWIJDERD, met de hele keten eronder (`SindsVorigBezoek(Loader)`, `lib/overview/sinds-vorig-bezoek.ts`, `touchLastSeen` + de bezoekmarker in `profiles.briefing_snapshot`). Een DAG-delta heeft in dit domein te weinig waarde — de waarde zit in maandelijks gebruik — en er komt bewust GEEN vervangende regel. Onder de begroeting hoort dus geen tweede vrijheidsgetal meer te staan; ziet de tester daar toch een dagdelta, dan is dat een regressie. IN-APP SURFACE VERVALLEN (UR2-09, eigenaar-besluit 31 aug 2026): het blok "Jouw vrijheid deze week" op /overzicht is verwijderd. De week-delta leeft nog uitsluitend in de briefing-e-mail (lib/briefing/email-template.ts). Op /overzicht blijft alleen de kop-zin naast de masthead, en die rekent LIVE uit de runway van het request (buildBriefingHeadline op computeHorizonRunway, zonder weekverschil).',
    assertion: {
      kind: 'exact',
      expected:
      ...
      source:
        'lib/briefing/overview-briefing.ts#summarizeRunway/#runwayDurationLabel/#runwaySentence/#computeRunwayWeekDelta — échte productiefuncties, geen mirror',
    },
  },
  {
    workflow: 'WF-OVZ-10',
