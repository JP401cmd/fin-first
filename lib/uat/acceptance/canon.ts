...
const criteria: AcceptanceCriterion[] = [
  ...
  },
  {
    ...
    then: 'Per domein exact dezelfde categorische status uit één scoringsbron (`loadLeverScores`/`computeLeverScores`/`pillarStatus`, `lib/leverage-status.ts`). Stoplichtkleuren volgen NIET het module-accent (semantiek blijft semantisch). Register-getal 12. TWEE GRONDSLAG-GATEN GESLOTEN (11 aug 2026), allebei in de loader die op ÉLKE route in het shell-pad hangt: (1) het Box 1-inkomen achter de belasting-dot leest sinds ADR 0103 dezelfde budgetgrondslag als het grondslagblok op /overzicht/budget/transacties (`loadBudgetBasis` → `resolveEffectiveIncomeExpenses` mét budget-argument) in plaats van stil op de transactiegrondslag te blijven; (2) de losse-rekening-optelling voor de sidebar-netWorth-metric gebruikt de canonieke, huishoud-gewogen `unlinkedCashTotal`/`resolveUnlinkedCashShare` in plaats van een eigen `reduce` over de saldi. Beide waren drift-bronnen tussen de dot en de pagina, niet tussen de vier oppervlakken onderling — de gelijkheidseis hierboven verandert er niet door, ze wordt er waar door. DERDE GRONDSLAG-GAT GESLOTEN (B-030, 7 sep 2026), nu op de BUDGET-hefboom: `loadLeverScores` voerde `computeLeverScores` de RAUWE 6-maands transactiemeting, terwijl de hefboomkaart op /overzicht al de EFFECTIEVE spaarquote toonde (`healthScoreInput.savingsRate6m` — een veldnaam-misnomer, de horizon-loader vult \'m met `effectiveSavingsRate`). Eén hefboom, één scherm, twee percentages (kaart 25 %, kompas 12 %) — en het stoplicht hing aan het verkeerde van de twee. De loader levert nu `resolveSavingsSource(...).effectiveSavingsRatePct` voor zowel de detailregel als de score. Op een transactie/transactie-grondslag is dat per definitie hetzelfde getal als voorheen; op een handmatige of budget-grondslag KAN de Budget-status hierdoor verschuiven (kompas-popover, desktop-sidebar én de status-duiding-melding op /overzicht/budget bewegen dan samen mee — dát is precies de eis van dit criterium). Blijft de grondslag helemaal onbekend (`resolveAmountWithBasis` → `\'unknown\'`, ADR 0131), dan blijft de quote `null` en toont de hefboom "onvoldoende data" in plaats van een verzonnen 0 %.',
    assertion: {
      kind: 'consistency',
      source: 'consistentie-eis: register-getal 12 (hefboom-status) uit lib/leverage-status.ts + lib/lever-scores-loader.ts (dat sindsdien `loadBudgetBasis`, `unlinkedCashTotal` én `resolveSavingsSource`/`extrapolateAnnualIncome` consumeert i.p.v. een eigen inkomens-/saldo-som of de rauwe meting); categorisch identiek op vier oppervlakken.',
    },
  },
  {
