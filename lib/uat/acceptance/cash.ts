 ...
 * transactiepagina. HERZIEN 19-09-2026 (kop-herziening — `PageVerdictOpening`):
 * de pagina opent sindsdien weer op "maand" (kalendermaand), niet op het
 * rollende 30-dagenvenster waarop hij tussen ADR 0135 (6 sep) en deze wijziging
 * stond — het oordeel in de titel en het statuspunt rekenen op de gerealiseerde
 * kalendermaand, en de oude default liet cijfers en oordeel een ander venster
 * beschrijven. "30 dagen" blijft kiesbaar in de periode-selector, alleen niet
 * meer als opening. De daggrafiek rendert uitsluitend bij periode "maand".
 *
...
const criteria: AcceptanceCriterion[] = [
  {
    ...
    kriticiteit: 'KERN',
    given: 'HERNOEMD 07-09-2026 (UR3-13 F2, optie C): de derde onderdeel-kaart heet op het scherm "Vooruitblik", niet "Forecast" (`lib/cashflow-cards.ts` label, breadcrumb en sidebar volgen). De MAP — en dus de URL /overzicht/budget/forecast — blijft bewust `forecast`, net als de code-identifiers `buildForecast`/`forecastCardStatus`/`hasForecast`; alleen de gebruikerszichtbare tekst verhuist mee. Synthetische kaartinvoer: Transacties-kaart met inkomen/uitgaven-paren op de statusgrenzen; Vaste-lasten-kaart met aandeel-ratio\'s op de statusgrenzen; Vooruitblik-kaart met netto/mnd op de statusgrenzen. De statusfuncties zelf zijn mode-onafhankelijk — `buildCashflowCards` berekent voor alle vier de kaarten (incl. Budget en Vooruitblik) altijd, ongeacht weergavemodus (de weergavemodus raakt alleen de PRESENTATIE, nooit de berekening). VIER GEBOUWD, DRIE GETOOND — HERZIEN 06-09-2026 (ADR 0135): de landing van deze onderdelen is niet langer de cashflow-hub maar `/overzicht/budget` zelf, en die pagina rendert via `budgetSubCards(cards)` uitsluitend Transacties, Vaste lasten en Vooruitblik. De Budget-kaart wordt nog wél gebouwd — hij zou naar zichzelf linken — maar niet getoond; zijn STATUS voedt de hefboomtegel op /overzicht en de sidebar-dot (die dot komt voor `/overzicht/budget` zelf uit de lichte `loadLeverScores`-familie, niet uit deze kaartenset). WEERGAVEMODUS (CF-1/CF-2): in **Volledig** staan de drie kaarten met KPI/venster/substext/chevron zichtbaar. In **Eenvoudig** rendert `CashflowLandingCards` de `verdict`-variant (oordeel primair + kerngetal mét venster + status-dot, géén chevron). HERZIEN 28-08-2026 (S4 + S5): CF-1 (compacte one-liner zonder cijfer) en CF-2 (Vooruitblik-kaart weg) zijn allebei teruggedraaid onder het R5-richtingsbesluit "duiding boven reductie"; CF-2 verborg op mobiel de enige contextuele ingang naar /overzicht/budget/forecast. De landing toont dus in béide modi dezelfde drie kaarten, de routes waren en blijven bereikbaar, en de sidebar leest nog steeds alle vier de statussen. HALVE-MAAND-UITZONDERING (bevinding C6, 26-08-2026): dit grensgeval-toetsblok roept `transactiesCardStatus` aan ZONDER `expectedMonthlyIncome`/`forecastNetPerMonth` (beide optioneel, default 0/null) — daarmee blijft `isCurrentMonthIncomeIncomplete` per constructie `false` en de grens <0%→bad ongewijzigd. De uitzondering zelf (zie WF-CASH-51) is dus geen sluipende wijziging van DIT grensgeval, maar een derde, optionele voorwaarde die alleen intreedt als de caller ze meegeeft. TITEL DRAAGT SINDS 19-09-2026 HET CASHFLOW-HEFBOOMOORDEEL, SINDS 22-09-2026 ALS ÉÉN ZIN (kop-herziening; ADR 0174 D6): boven de drie kaarten toont `BudgetEditorialHeader` een oordeelzin, op mobiel én desktop gelijk (de TopBar draagt de paginanaam): "Je budget is *op koers*." (good) / "Je budget *vraagt aandacht*." (warn) / "Je budget staat *onder druk*." (bad) / "Je budget is *nog niet te beoordelen*." (neutral — niet "nog niet in beeld", want de pagina toont altijd een cijferblok), uit `lib/hefboom-oordeelzin.ts`. Bewust zonder oorzaak in de zin (sinds 22-09-2026; eerder "op koers met sparen · onder je spaardoel · laat een tekort zien"): de cashflow-status mengt spaarquote en budgetoverschrijding 50/50, dus alleen budgetten met drie overschrijdingen is rood zonder gemeten tekort, en 0% sparen met alle budgetten binnen de limiet is groen. De hefboomtegel op /overzicht zegt "Budget op koers" / "Budget vraagt aandacht" / "Budget onder druk"; het cursieve deel is een `<em>` in de stoplichtkleur. De kop STROOMT: het onderwerp "Je budget" staat in de eerste byte, de rest van de zin komt via een eigen `<Suspense>`-server-child (`budget-verdict.tsx#BudgetVerdict`) uit DEZELFDE `loadLeverScores(...).scores.cashflow` als het statuspunt naast de pagina-`i` op deze route — niet uit de (nog niet geladen) kaartenstatus, om te voorkomen dat titel en stip iets anders zeggen. Tijdens het laden staat er dus "Je budget", zonder verspringen. Op de legacy-route /core/budgets blijft de kale paginanaam staan. De vervallen kicker ("Budgetteren · augustus") verhuisde naar de deck, die sinds de eenvoud-check B-071 luidt: "Je inkomen en uitgaven in {maand}. We kijken hoeveel je spaart en of je binnen je budgetten blijft." (maand in kleine letters, bv. "in september 2026"; zonder maandlabel "deze maand") — zonder het woord "oordeel".',
  ...
  },
  {
    ...
    given: 'Persona Daan Bakker op /overzicht/budget/transacties; de Inkomen/Uitgaven-cellen van de geldstroom-kaart leunen op gejitterde seed-transacties. VERHUISD 06-09-2026 (ADR 0135): de kassabonnen stonden op de cashflow-hub (`components/app/cash-overview.tsx`, verwijderd) en wonen nu in `components/overview/transacties/geldstroom-kassabonnen.tsx`. GEDRAGSWIJZIGING, expliciet besloten: de bonnen beschrijven de periode van de `PeriodeSelector` en niet meer stilzwijgend de kalendermaand-tot-nu-toe; het venster staat daarom in de titel van elke sheet. HERZIEN 19-09-2026: de default van die `PeriodeSelector` ging van het rollende 30-dagenvenster terug naar "maand" (kalendermaand) — de kop-herziening rekent het paginaoordeel op de gerealiseerde kalendermaand, en de oude default liet de bonnen (en de rest van de analyse) een ánder venster beschrijven dan de titel. "30 dagen" blijft een kiesbare stand; wie die kiest ziet dat venster ook hier terug in de titel van de sheet. De doorklik zelf is nieuw op deze pagina — de cijfers stonden er al, de ingang ontbrak (`KpiCell` werd een button, in BEIDE weergavemodi: `GeldstroomGauge` in Volledig en `GeldstroomZin` in Eenvoudig dragen dezelfde `onOpenIncome`/`onOpenExpense`).',
    when: 'De gebruiker opent de inkomsten- en uitgaven-kassabon vanaf de geldstroom-kaart.',
    then: 'De som van de per-rekening-regels (inkomsten) resp. per-budget-regels (uitgaven, incl. "Ongecategoriseerd" voor een ontbrekend of onbekend budget) is exact gelijk aan het periodetotaal onderaan de bon — een A=B-consistentietoets, geen vast cijfer (de onderliggende bedragen zijn jitter-afhankelijk). Het totaal komt bewust NIET uit de opsomming maar uit dezelfde `summarizeFlow`-samenvatting die de kaart zelf toont: elke regel is een `summarizeFlow` over een deelverzameling van diezelfde rijen, dus met identieke classificatie (teken van `amount`) en hetzelfde overboekingsfilter. Bekende, aanvaarde afwijking: een legacy-rij met zowel een split als een `budget_id` zou onder "Ongecategoriseerd" landen in plaats van op zijn eigen budget — het TOTAAL klopt in beide gevallen. VOLGORDE, vastgelegd 07-09-2026 (was nergens getoetst): de twee bonnen sorteren BEWUST verschillend en dat is met het oog te controleren. De INKOMSTEN-bon loopt de rekeningenlijst af in `sort_order` — dezelfde reeks als de rekeningkaarten en als de opgeheven hub, zodat de lezer een rekening elke maand op dezelfde plek vindt — met de restpost "Overige rekeningen" (inkomen zonder rekening, of op een rekening die deze pagina niet kent) ALTIJD als laatste regel, nooit meesorterend. De UITGAVEN-bon sorteert wél, aflopend op bedrag, inclusief de regel "Ongecategoriseerd"; dezelfde aflopende sortering geldt voor de kindbudgetten in het uitklap-detail. Een bon die zichzelf op bedrag hersorteert terwijl zijn kaarten dat niet doen is een bevinding, en een restpost die niet onderaan staat leest als een gewone rekening.',
    assertion: {
      kind: 'consistency',
      source: 'components/overview/transacties/geldstroom-kassabonnen.tsx (summarizeByKey per rekening/budget; `incomeLines` loopt `accountMap` af met de restpost als laatste push, `expenseLines`/`drillChildren` doen `.sort((a, b) => b.amount - a.amount)`) vs. lib/transaction-insights.ts#summarizeFlow over dezelfde periode-rijen — som-regels moet gelijk zijn aan het bon-totaal; de volgorde is een oogtoets op dezelfde regels, geen tweede berekening',
    },
  },
  {
    workflow: 'WF-CASH-04',
  ...
  },
  {
    ...
    titel: 'Grondslag aanpassen op de transactiepagina (inkomen, spaarquote, uitgaven)',
    kriticiteit: 'KERN',
    given: 'HERNOEMD 07-09-2026 (ADR 0135): de titel volgt nu de catalogus — het grondslag-instellingenblok verhuisde met de opgeheven cashflow-hub mee naar /overzicht/budget/transacties. De REKENREGEL is ongewijzigd. Triple {inkomen €3.600, uitgaven €2.808, spaarquote wordt niet gebruikt}, laatst-bewerkt="expenses".',
  ...
  },
  {
    ...
    kriticiteit: 'KERN',
    given: 'Een auto-gedetecteerd item "Basic-Fit" €44,90/mnd, aangeleverd door `GET /api/detect-recurring` (`detectRecurringTransactions`). GEWIJZIGD 12-09-2026 (V-001): het analysevenster ging van 12 naar 24 maanden (`RECURRING_ANALYSIS_MONTHS`, één gedeelde constante voor deze route, `/api/subscriptions/detect-ai` en `/api/subscriptions/analyse-ai`), en een jaar-/halfjaarpatroon (frequency `yearly`) telt al bij 2 waarnemingen als \'medium\' mee (was 3 — met 3 verplichte betalingen viel elk jaarabonnement structureel buiten de lijst, want dat vraagt 3 jaar historie). Maandelijks/wekelijks/kwartaal houden hun eigen, ongewijzigde drempels (3/4/2). De merknamenlijst is met ~38 patronen uitgebreid. TWEE BEGRENZINGEN horen bij die verruiming: (a) STAARTTERMIJN — een patroon waarvan de LAATSTE betaling langer geleden is dan ruwweg 1,5× het interval (`STALE_AFTER_DAYS`: wekelijks 21 d, maandelijks 50 d, kwartaal 140 d, jaar 430 d) zakt naar \'low\' en valt dus uit de vaste-lasten-cijfers; zonder die regel zou een in feb. 2025 opgezegde Ziggo van €55/mnd in sep. 2026 weer meetellen, want `detectFrequency` kijkt alleen naar de intervallen en nooit naar hoe lang geleden de laatste betaling was. (b) BEDRAGSEIS BIJ n=2 — bij precies twee waarnemingen geldt amountCV < 0,15 in plaats van < 0,50, want met twee bedragen is die coëfficiënt een zwak signaal (€100 + €280 geeft 0,47) en zouden twee losse aankopen een abonnement worden.',
  ...
  },
  {
    ...
    kriticiteit: 'BELANGRIJK',
    given: 'AI-add-on actief resp. uit/kill-switch aan. GEWIJZIGD 12-09-2026 (V-001): `app/api/subscriptions/analyse-ai/route.ts` stuurt nu ook de \'low\'-confidence-kandidaten van `detectRecurringTransactions` naar de AI ter beoordeling (voorheen alleen \'medium\'/\'high\'); die kandidaten komen — mét AI-oordeel — in de voorstellenlijst terecht, maar tellen NIET mee in het bestaande vaste-lasten-totaal totdat de gebruiker ze bevestigt. BEGRENSD (eigenaarsbesluit 12-09-2026): op het CLOUD-pad gaat een \'low\'-kandidaat alleen mee als zijn tegenpartij een treffer heeft in de merknamenlijst (`suggestedCategory` is niet `other_expense`/`other_income`) — de low-staart bestaat grotendeels uit onherkende, particuliere tegenpartijen, en `sanitizeForAI` laat de tegenpartijnaam bewust staan. Het LOKALE pad (`{ candidatesOnly: true }`, on-device) houdt de volledige lijst: daar is geen leverancier. Zelfde 24-maands-venster (`RECURRING_ANALYSIS_MONTHS`) als WF-CASH-17.',
    when: 'De gebruiker klikt "Laat Fin analyseren".',
    then: 'Mét AI: voorstellen per rij met classificatie + reden — nu inclusief de eerder weggefilterde \'low\'-kandidaten (bv. een net-op-2-betalingen jaarabonnement). De rij toont de betrouwbaarheid uit de DETECTIE (`confidence`, meegegeven in de response); de AI-REDEN gaat NIET over zekerheid — de promptregel per kandidaat draagt alleen naam | frequentie | €/keer | Nx gezien | auto-categorie, dus het model kent de confidence niet en kan er niets over zeggen. Opslaan toont een toast met het aantal en ververst de vaste-lasten-cijfers. Zonder AI: melding "AI is niet geconfigureerd…" (422), handmatige weg blijft beschikbaar.',
    assertion: {
      kind: 'ui-only',
      source: 'components/app/ai-vaste-kosten-sheet.tsx (POST /api/subscriptions/analyse-ai) → app/api/subscriptions/analyse-ai/route.ts, op lib/recurring-detection.ts#detectRecurringTransactions + RECURRING_ANALYSIS_MONTHS en lib/ai/vaste-kosten-kandidaten.ts#selectVasteKostenAiKandidaten (de cloud/lokaal-begrenzing, apart getest) (zie ook app/api/subscriptions/detect-ai/route.ts, dezelfde detectiekern voor de bredere \'herkent de gebruiker dit als abonnement\'-vraag) — AI-output, niet deterministisch toetsbaar',
    },
  },
  {
    workflow: 'WF-CASH-22',
  ...
  },
  {
    ...
    then: 'Zolang er iets te kiezen valt staat er GÉÉN voorselectie en blijft de knop uitgeschakeld met de hint "Kies eerst waar de data terechtkomt" tot een keuze is gemaakt. Kiest de gebruiker een bestaande rekening ZONDER budget-tracking, dan verschijnt één VOORGEVINKTE, uitzetbare optie "Neem deze rekening mee in mijn budgetten" (B2); een bestaande rekening MET tracking krijgt geen vraag; een nieuwe rekening krijgt budgetteren altijd aan. Een rekening met een reeds actieve koppeling is (sinds fase 6) niet kiesbaar — `isSelectableTargetOption` — en de route weigert haar met een 409. NIEUW (ADR 0156): naast de rekeningen (`accounts`) levert dezelfde GET een tweede lijst `assets` — eigen, actieve cash-BEZITTINGEN zonder `bank_accounts`-rij (bv. de betaalrekening uit de onboarding), óók als er nul rekeningen zijn; nooit die van de partner. Die staan als gewone kiesbare optie in de lijst (naam, instelling of "Rekening uit je overzicht", evt. `···· <laatste cijfers>`, "nog geen transacties") met de uitleg dat deze rekening wordt bijgewerkt in plaats van dat er een tweede bijkomt; ook daar verschijnt de budget-vraag als het bezit nog geen budget-tracking heeft. De uitleg "Kies je een rekening die je al hebt…" staat er alleen als er iets te kiezen valt. `POST /api/bank-connect/auth-link` accepteert optioneel `target_bank_account_id` óf `target_asset_id` (niet beide, en een bezit-id niet naast `relink_connection_account_id` → 400 "Kies één doelrekening") + `enable_budget_tracking`, valideert eigenaarschap en schrijft de keuze op de pending `bank_connections`-rij (`target_bank_account_id`, `link_intent = \'nieuw\'`); bij een `target_asset_id` maakt de server eerst zelf de companion-rij aan (`ensureCompanionForCashAsset`) en loopt daarna exact dezelfde toets. Een andermans/onbekende/ongeschikte `target_bank_account_id` of `target_asset_id` (geen cash, gedeactiveerd) levert dezelfde vaste 400 zonder pending-rij. Een body zónder keuze blijft geldig. Vanaf hier BLOCKED voor de daadwerkelijke doorverwijzing/callback/sync-resultaat (X nieuw/Y dup) — vereist een echte of sandbox TrueLayer-koppeling, expliciet buiten bereik van deze UAT-ronde. VERVALLEN bevinding: de vroegere copy-inconsistentie ("GoCardless" in de koppel-banner vs. "TrueLayer" hier) bestaat niet meer — `koppel-rekening-banner.tsx` is sinds fase 2 (TXN-1) herschreven en noemt alleen nog "TrueLayer". WEERGAVEMODUS (TXN-1, herzien 28 aug 2026 via M40): "Bank koppelen" staat in BEIDE modi als eigen knop in de actie-rij van /overzicht/budget/transacties — de vul-routes zijn niet langer mode-afhankelijk. De wizard zelf (deze pagina) is in beide modi ongewijzigd.',
    assertion: {
      kind: 'ui-only',
      source: 'app/(app)/core/cash/connect/page.tsx (stap 2 "Rekening & bevestigen") + components/app/bank-connect/target-account-choice.tsx + app/api/bank-connect/accounts/route.ts (GET, kandidatenlijst + `assets` via lib/truelayer/target-asset.ts#loadCompanionlessCashAssets) + app/api/bank-connect/auth-link/route.ts (POST, eigenaarschapscheck + `target_asset_id` → ensureCompanionForCashAsset + pending-rij) + lib/truelayer/start-bank-connect.ts (gedeelde body-opbouw met de onboarding) — vereist sandbox-koppeling voor het vervolg, niet pure-testbaar; zie ook de GoCardless/TrueLayer-copy-bevinding',
    },
  },
  {
    workflow: 'WF-CASH-31',
  ...
  },
  {
    ...
    kriticiteit: 'BELANGRIJK',
    given: 'HERZIEN 17 sep 2026 (ADR 0156): de bankkoppeling in de onboarding draait nu als afrondingsstap NÁ de opslag (`onboarding_completed = true`, afrondingsmarkering open op `bank`), en de onboarding heeft dan al een companion-loze betaalrekening (cash-bezit) aangemaakt. Die komt sinds deze release terug in de `assets`-lijst van `GET /api/bank-connect/accounts`, dus in het standaard onboardingpad zijn er GEEN nul kandidaten meer — dat pad (voorselectie van dat ene bezit) is WF-START-34 (leidend). Dit scenario toetst de resterende nul-toestand: een gebruiker zonder `bank_accounts`-rijen ÉN zonder actief eigen cash-bezit (bv. de onboarding zonder betaalrekening, of de wizard op /core/cash/connect bij een leeg account), plus het geval dat de kandidatenlijst niet geladen kon worden (netwerkfout).',
    when: 'De gebruiker doorloopt wizardstap 2 ("Rekening & bevestigen") resp. stap A van de onboarding-bankstap.',
    then: 'Pas bij nul rekeningen ÉN nul cash-bezittingen (`accounts.length + assets.length === 0`) wordt de lijst WEGGELATEN — niet leeg getoond — met de ene regel dat er nog geen rekening is; de uitleg "Kies je een rekening die je al hebt…" verschijnt dan niet (die zou de regel tegenspreken). "Nieuwe rekening aanmaken" staat voorgeselecteerd, dus "Verbind met <bank>" is direct bruikbaar. Kon de lijst niet geladen worden, dan geldt hetzelfde voorgeselecteerde gedrag, mét een zichtbare waarschuwing (beide lijsten leeg). De callback stuurt terug naar `/onboarding?bank_connected=1` resp. `?bank_error=1` zolang de onboarding nog niet is opgeslagen ÓF de afrondingsmarkering nog open staat (`hoortBijOnboarding`, zie WF-START-35) — niet naar de reguliere success-pagina. De wizard loopt op onboarding niet dood.',
    assertion: {
      kind: 'ui-only',
      source: 'components/app/bank-connect/target-account-choice.tsx (nul rekeningen én nul bezittingen → lijst en uitleg weggelaten, "nieuw" voorgeselecteerd) + lib/truelayer/start-bank-connect.ts#fetchTargetOptions + app/api/bank-connect/accounts/route.ts (`accounts` + `assets`) + app/api/bank-connect/callback/route.ts#hoortBijOnboarding — vereist een verse/onboarding-testaccount, niet pure-testbaar',
    },
  },
  {
    workflow: 'WF-CASH-45',
  ...
  },
  {
    ...
    kriticiteit: 'KERN',
    given:
      'Het grondslag-instellingenblok (`components/overview/cashflow-instellingen-blok.tsx`, lazy ingeladen via `CashflowInstellingenBlokLazy` — óók in Eenvoudig, daar achter een disclosure). VERPLAATST 06-09-2026 (ADR 0135) EN NOGMAALS VERPLAATST (W-002, vóór de kop-herziening): het blok stond eerst onderaan de opgeheven cashflow-hub, verhuisde met ADR 0135 naar onderaan /overzicht/budget/transacties, en woont sinds W-002 op de vierde tegel /overzicht/budget/instellingen (samen met de rekeningkeuze `BudgetCashSources`) — VERHUISD, niet gekopieerd: op de transactiepagina staat hij niet meer. Gedrag en opslag zijn bij beide verhuizingen ongewijzigd meeverhuisd. HERZIEN 19-09-2026 (kop-herziening — `PageVerdictOpening`): /overzicht/budget/instellingen draagt bewust GEEN oordeel in de titel (`verdict={null}`, dus géén statuspunt naast de pagina-`i`) — een instelling is een keuze, geen score; de titel is de kale paginanaam "Instellingen" en de deck luidt "Welke rekeningen meetellen en waarop je cijfers rusten. Wat je hier wijzigt, rekent je budget meteen mee." Eén synthetische situatie waarin de drie bronnen bewust UITEENLOPEN: profielschatting €2.000/mnd, gemeten transacties €3.000/mnd, budgetsom €3.600/mnd. De keuze staat per kant op `profiles.income_source` / `profiles.expenses_source`; de uitsluitlijst op de eigen, exclusieve kolom `profiles.cashflow_basis_prefs` (NADRUKKELIJK niet `feature_preferences`, die kolom wordt als volledige overwrite geschreven).',
    when:
...
const cardR7Criteria: AcceptanceCriterion[] = [
  ...
  },
  {
    ...
    titel: 'Grenzenpot precies op de grens: "Grens bereikt" als vierde WEERGAVE-stand (ADR 0136)',
    kriticiteit: 'BELANGRIJK',
    persona: 'daan',
    given:
      'Een grenzenpot met een limiet van €500 waarvan de lopende periode precies €500 heeft opgesoupeerd. De MOTOR (`lib/spend-limits/engine.ts#computePeriodOutcome`) zegt daarover: `status: \'within\'` (de regel is `matched > limit`, dus exact op de grens telt als binnen), `periodHeadroom: 0` én `isNearLimit: true` (500/500 = 100 % ≥ SPEND_LIMIT_NEAR_LIMIT_PCT). VOORHEEN las de weergave alleen `status` + `isNearLimit` en toonde daardoor "Dicht bij je grens — Er is nog ruimte, maar niet veel" pál naast "€ 0 ruimte": een belofte en haar tegenspraak op één regel (melding B-027).',
    when:
      'De gebruiker leest die pot op de vijf weergave-oppervlakken — de grenzenpot-sectie op /overzicht/budget/transacties (kaart én de samenvatting van de ingeklapte sectie), de prestatiepane, het periode-rooster/de periodegrafiek en de widget op /overzicht — en krijgt de bijbehorende melding in /berichten.',
    then:
      'De WEERGAVE kent sinds ADR 0136 een vierde stand tussen `near` en `exceeded`: `reached`, met het label "Grens bereikt" (inline: "grens bereikt"), de WARNING-kleurfamilie (`text-warning` / `var(--warning)`, niet negative — er is niets overschreden) en waar er een vlak getekend wordt een sterkere rand dan `near`. Hij WINT van `near`: op de grens staan is geen "bijna". De drempel is `periodHeadroom < 0,005` (SPEND_LIMIT_HEADROOM_EPSILON — een halve cent, precies de grens waaronder het bedrag ernaast naar "€ 0,00" afrondt), met dezelfde `limitAmount > 0`-guard als `isNearLimit`, zodat een lege periode op een nulgrens niet permanent "grens bereikt" meldt. De kaart vervangt de headroom-regel door "geen ruimte meer" en toont daar bewust GEEN bedrag; de ingeklapte sectiesamenvatting telt ze apart ("n op je grens", naast "n boven je grens" en "n dicht bij je grens"); het periode-rooster krijgt een vierde celtoestand (`geen uitgaven` / `binnen` / `grens bereikt` / `boven`) en de melding is een eigen event `reached` ("<pot>: grens bereikt", priority 3 — gelijk aan `near`, dus buiten de dringend-bak) ZONDER `headroom`-bedrag in de metadata. DE MOTOR IS ONGEMOEID — dat is de kern van dit criterium: exact op de grens blijft rekenkundig BINNEN. `status` blijft `within`, de periode telt als TREFFER, de reeks loopt door, `exceededPeriodCount` blijft 0 en de reeksscore blijft 100. Zes afgesloten periodes die alle zes precies op de grens eindigen leveren dus nog steeds een volle score — zakt dat cijfer, dan is de weergave-stand de motor in gelekt. Om diezelfde reden spiegelen de fixtures van WF-CASH-61 en WF-CASH-64 bewust de MOTOR-regel en niet deze stand.',
    assertion: {
      kind: 'exact',
      expected:
        'motorStatusOpDeGrens=within; motorNearOpDeGrens=true; motorHeadroomOpDeGrens=0; weergaveOpDeGrens=reached; weergaveNetEronder=near; weergaveRuimBinnen=within; weergaveEroverheen=exceeded; weergaveNulGrens=within; restOnderEpsilon=reached; restOpEpsilon=within; label=Grens bereikt; kleur=text-warning; scoreOpDeGrens=100; hitRateOpDeGrens=100; exceededTellingOpDeGrens=0; reeksOpDeGrens=6',
      source:
        'lib/spend-limits/engine.ts#computePeriodOutcome/computeSpendLimitScore/computeStreaks + lib/spend-limits/status-display.ts#resolveSpendLimitOutcomeState/resolveSpendLimitDisplayStatus/SPEND_LIMIT_HEADROOM_EPSILON/SPEND_LIMIT_STATUS_LABEL/SPEND_LIMIT_STATUS_TEXT_CLASS (échte productiefuncties op de échte motoruitkomst, geen mirror) + lib/notifications/spend-limit.ts#decideSpendLimitEvents (`reached`-tak) + docs/adr/0136-grens-bereikt-is-een-weergave-stand.md — zie cash-checks.ts',
    },
  },
  {
    workflow: 'WF-CASH-68',
    scenarioId: 'UAT-CASH-68',
    titel: 'Vul-ingangen staan één keer op de transactiepagina, niet twee keer',
    kriticiteit: 'BELANGRIJK',
    persona: 'daan',
    given:
      'NIEUW 07-09-2026 — dit gedrag landde in `dc569998a` en had nul criteria. De vul-routes "Importeer transacties" en "Bank koppelen" staan sinds M40 in BEIDE weergavemodi in de actie-rij van `TransactiesAnalyse` (een vul-route is geen diepte die je in Eenvoudig mag wegnemen; ze zaten voorheen in het `HideInSimple`-blok). Bovenaan diezelfde pagina staat de `KoppelRekeningBanner`, die exact één toestand kent: hij rendert uitsluitend bij NUL zichtbare rekeningen en biedt dan zélf al koppelen én importeren. Zonder verdere maatregel zou een leeg account beide ingangen twee keer op één scherm zien. Twee takken: account A zonder enige zichtbare rekening (`accountCount === 0`), account B met minstens één rekening.',
    when: 'De gebruiker opent /overzicht/budget/transacties op account A en daarna op account B, in Eenvoudig én in Volledig.',
    then: 'Tak A (nul rekeningen): de banner staat bovenaan MET zijn twee knoppen, en de actie-rij van de analyse laat "Importeer transacties" en "Bank koppelen" juist WEG — de pagina biedt elke vul-route precies één keer aan. Tak B (≥1 rekening): de banner rendert niets en de twee ingangen staan wél in de actie-rij. De schakelaar is één prop met dezelfde conditie als de banner zelf (`vulIngangenInBanner={accountCount === 0}`), dus de twee kunnen per constructie niet uit elkaar lopen; hij raakt uitsluitend deze twee links en laat "Zoeken en bulkbewerken" (Volledig-only) en het "…"-menu in Eenvoudig ongemoeid. In beide takken is elke vul-route bereikbaar — dit dedupliceert, het verbergt niets.',
    assertion: {
      kind: 'ui-only',
      source:
