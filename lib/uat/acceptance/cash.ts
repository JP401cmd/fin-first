...
const criteria: AcceptanceCriterion[] = [
  {
    ...
    kriticiteit: 'KERN',
    given: 'Synthetische kaartinvoer: Transacties-kaart met inkomen/uitgaven-paren op de statusgrenzen; Vaste-lasten-kaart met aandeel-ratio\'s op de statusgrenzen; Forecast-kaart met netto/mnd op de statusgrenzen. De statusfuncties zelf zijn mode-onafhankelijk — `buildCashflowCards` berekent voor alle vier de kaarten (incl. Forecast) altijd, ongeacht weergavemodus (CF-2, de reductie zit puur in de landingsweergave). WEERGAVEMODUS op de LANDING (/overzicht/cashflow, CF-1/CF-2): in **Volledig** staan alle vier de kaarten met KPI/venster/substext/chevron zichtbaar. In **Eenvoudig** rendert `CashflowLandingCards` compact (icoon + label, géén KPI/substext/status-dot/chevron) en toont de Forecast-kaart niet (4→3) — de route /overzicht/cashflow/forecast blijft bereikbaar en de sidebar-statusdot leest nog steeds alle vier de statussen.',
  ...
  },
  {
    ...
    kriticiteit: 'KERN',
    given: 'Genoteerd maandtotaal Uitgaven vóór toevoeging (jitter-afhankelijke basis); nieuwe transactie Uitgave €42,50. WEERGAVEMODUS (TXN-1) op de actie-rij van /overzicht/cashflow/transacties: "Nieuwe transactie" is in BEIDE modi de primaire, direct zichtbare knop — dit criterium is dus mode-onafhankelijk. Alleen "Importeer transacties" en "Bank koppelen" verhuizen in **Eenvoudig** naar het "…"-menu (ShellOverlay-sheet); in **Volledig** staan ze ongewijzigd als eigen knoppen naast "Nieuwe transactie".',
  ...
  },
  {
    ...
    kriticiteit: 'KERN',
    given: 'De pagina /overzicht/cashflow rendert de vier hefboom-kaarten via `buildCashflowCards` (`lib/dashboard-data-loader.ts` + `lib/cashflow-data-loader.ts` + `lib/vaste-lasten-summary.ts` als invoer). ADR 0073 ("grondslag in de veldnaam") legt vast dat elk inkomsten-/uitgavenveld op de bundel zijn venster in de naam draagt. WEERGAVEMODUS (CF-1/CF-3): de hieronder getoetste `budgetKpi`/`transKpi`-WAARDEN zijn mode-onafhankelijk (`buildCashflowCards` rekent ze altijd uit, server-side). Of ze ook ZICHTBAAR zijn verschilt: in **Volledig** toont `CashflowLandingCards` de KPI-tekst op elke kaart; in **Eenvoudig** rendert de kaart compact (icoon + label) en is de KPI-tekst zelf NIET op de landing te zien — pas na doorklikken naar de sub-pagina. De Transacties-kaart draagt bovendien een venster-label (`kpiWindow`, CF-3) — "in {maand} tot nu toe" — onder de KPI in Volledig; de Budget-kaart heeft bewust GEEN venster-label (het cijfer is een restant, geen som-over-venster) en de "Budgetdekking"-tip in het uitklap-detail noemt sindsdien hetzelfde vensterlabel in plaats van het vaste "deze maand".',
  ...
  },
  {
    ...
    titel: 'Grenzenpot: berekenen en reeks — kwartaal/jaar-periodes, isNearLimit, streaks, trend (fase 2–5, ADR 0089/0092)',
    kriticiteit: 'KERN',
    given: 'De rekenmotor `lib/spend-limits/engine.ts` op synthetische, on-the-fly invoer (géén opgeslagen periode-uitkomsten — elke weergave rekent opnieuw uit de transacties). Subscenario a: een kwartaalpot (limiet €900) met aggregaat-rijen in de drie maanden van Q3 2026 én in de aangrenzende maand (juni) erbuiten, plus een jaarpot (limiet €5.000) met de 12 kalendermaanden van 2026 én een rij in december 2025 erbuiten. Subscenario b: `isNearLimit`-grensgevallen — een grens van €0 en een reeds overschreden periode op 200% van de grens. Subscenario c: zes afgesloten periodes met precies één overschrijding ertussenin (streaks), en vijf trend-randgevallen (2, 4 en 6 afgesloten periodes, met prior=0 en met een verschil onder de 5%-drempel). Subscenario d: dezelfde periode vóór en ná een refund (retroactief herstel) en de tegenpartij-sleutel-parityhelpers.',
