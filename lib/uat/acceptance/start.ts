...
const criteria: AcceptanceCriterion[] = [
  ...
  },
  {
    ...
    titel: 'Stap "Fin en je gegevens": AI in de beta zelf aanzetten, weigeren en terugdraaien (ADR 0155 + 0157)',
    kriticiteit: 'KERN',
    given: 'Een vers account (profiel aangemaakt door de trigger, `ai_enabled` standaard false, `ai_consent_at` NULL, `active_subscriptions` leeg). De onboarding opent ná de welkomstpopup met de stap "Fin en je gegevens" (stap-id `ai_keuze`, vóór "naam"): de titel uit lib/beta-addons.ts#BETA_ADDON_COPY.ai.titel, de beta-uitleg "Straks wordt AI een abonnement van €9 per maand …" (betaAddonNotice, prijs uit de catalogus) en één schakelaar "AI aanzetten" die UIT staat (role switch, aria-checked false). De privacyverklaring (AiConsentFacts, lib/ai/privacy-facts.ts) staat er nog niet.',
    when: 'De gebruiker (a) klikt meteen "Verder zonder AI"; (b) zet de schakelaar aan, leest de verklaring en klikt "Verder met AI"; (c) koos "nee", rondt de onboarding af en opent Fin, de briefing en de pensioenstap, en klikt daar op "AI aanzetten"; (d) zet AI later op /mijn/account uit — óf logt in met een bestaand account dat de `ai`-add-on al heeft maar waarvan `ai_consent_at` NULL is.',
    then: '(a) `POST /api/consent/ai` legt decision withdrawn vast (source onboarding); de flow gaat pas na een 200 naar "naam". (b) De verklaring klapt open zodra de schakelaar aan gaat, met de regel "Met “Verder met AI” geef je Fin toestemming …"; `POST /api/beta/addon` {tier ai, active true, source onboarding} schrijft éérst `consent_events` (granted) + de profielrij (`ai_enabled=true`) en pas dán `ai` in `active_subscriptions` (+ regel in tier_assignments_log met "(beta-keuze)"); bij een fout blijft de stap staan met `role="alert"`. (c) De AI-ingangen tonen de gedeelde upsell "… werkt als je AI aanzet" met de beta-uitleg; de knop opent de popup (BetaAddonDialog) met dezelfde schakelaar uit, "AI aanzetten" disabled tot de schakelaar aan staat, de verklaring pas ná de schakelaar; na bevestigen ververst de shell en werkt de functie. Overzicht, budget, toekomst, belasting en doelen werken zonder AI volledig. (d) "AI uitzetten" haalt de add-on weg en legt withdrawn vast. Het bestaande account MÉT add-on en zonder keuze krijgt éénmalig de vergrendelde keuze-overlay (ShellOverlay confirm, `lockedOpen`); een account zónder add-on krijgt die overlay níet — de vraag komt bij het aanzetten.',
    assertion: {
      kind: 'ui-only',
      source: 'app/api/beta/addon/route.ts (toestemming → add-on, eigen rij, zod-strict, vlag BETA_SELF_SERVE_ADDONS) + lib/ai/consent-record.ts (volgorde event → profiel) + components/app/beta-addon/beta-addon-choice.tsx (één body voor popup en onboarding) + app/(app)/layout.tsx (overlay alleen met ai-add-on) — gedrag/tekst-toets; bewaakt door app/api/beta/addon/route.test.ts, app/api/consent/ai/route.test.ts en de componenttests',
    },
  },
  {
