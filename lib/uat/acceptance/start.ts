...
const criteria: AcceptanceCriterion[] = [
  ...
  },
  {
    ...
    kriticiteit: 'KERN',
    given: 'Sanne\'s ingevulde check (zie WF-START-06): Noodfonds €6.600 (cash), Spaargeld €15.000 (savings), Beleggingen €8.000 (investment), geen schulden, geen eigen woning, maanduitgaven €2.200.',
    when: 'De bezoeker opent het Vrijheidsrapport (/check/rapport?token=...) en leest "Foto van nu" en het gezondheidsgetal.',
    then: 'Netto vermogen (FIRE-eligible, geen huis om te wegen) = €29.600 (exacte som). Canonieke buffer-dekking (`computeEmergencyFundMonths`, ALLEEN liquide types savings/checking/cash — Beleggingen telt niet mee) = 21.600/2.200 ≈ 9,8182 maanden — bewust anders dan de 3,0-maanden-preview uit WF-START-06 (die deelde alleen het buffer-veld, vóór de overige bezittingen bekend waren). Vrijheids-% en gezondheidsgetal zijn NIET hand-narekenbaar (horizon-kernel resp. 7-indicator-samenstelling) — alleen richting/orde-grootte toetsbaar, geen exact cijfer.',
    assertion: {
      kind: 'exact',
  ...
  },
  {
    ...
    then: 'Spaarquote-preview = 30%. Pensioen-uitgaven-prefill = €20.160/jaar (80% × €2.100 × 12). Noodfonds-prefill = €6.300 (3× €2.100, al een veelvoud van €100). Recap netto vermogen = €32.500 − €9.000 = €23.500. Ná activatie moet /overzicht/cashflow exact dezelfde 30% spaarquote tonen (SSoT-eis, zelfde motor als WF-START-10).',
    assertion: {
      kind: 'exact',
  ...
  },
  {
    ...
    then: 'Prefill-bedrag = 3× €1.900 = €5.700 (al een veelvoud van €100, geen afronding zichtbaar); bij een leeg/0-bedrag wordt bij opslaan géén doel weggeschreven (stil overgeslagen).',
    assertion: {
      kind: 'exact',
  ...
  },
  {
    ...
    then: 'Harde navigatie naar /toekomst zonder terug-flits naar /onboarding; een reeds-geonboarde gebruiker die /onboarding opent → redirect naar /overzicht; een niet-geonboarde gebruiker die een app-route opent → redirect naar /onboarding. De cijfers zelf zijn al gedekt door WF-START-18/19; hier telt alleen de landing/poort-navigatie.',
    assertion: {
      kind: 'ui-only',
      source: 'window.location.assign + onboarding-poort in app/(app)/layout.tsx, geen eigen berekening',
    },
  },
]
