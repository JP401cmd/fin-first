...
const criteria: AcceptanceCriterion[] = [
  ...
  },
  {
    ...
    when: 'De gebruiker kiest eerst in de stap "Fin en je gegevens" (WF-START-30, groep 1 samen met Profiel) en doorloopt daarna de stappen "inkomen"/"uitgaven" (spaarquote-preview), "uitgaven na pensioen" (prefill), "spaardoel" (Noodfonds-preset), "Jouw plan" (standaardpad — twee vragen, zie WF-START-28) en "klaar" (recap). Na "Begin met TriFinity" volgt sinds ADR 0156 NIET direct het successcherm: ná de opslag komen eerst de afrondingsstappen budget (WF-START-31..33) en bank (WF-START-34..36) in groep 9/9, pas daarna het successcherm en de landing in de app.',
  ...
  },
  {
    ...
    kriticiteit: 'KERN',
    given: 'De pensioen-stap (groep 5/9): schatting-pad met ingangsleeftijd-invoer, geklemd tussen 50 en 75, met de AOW-leeftijd van de gebruiker als fallback (placeholder + hint tonen die leeftijd; een inschat-hulp kan het bedrag vullen uit bruto jaarsalaris × jaren opbouw).',
  ...
  },
  {
    ...
    kriticiteit: 'BELANGRIJK',
    given: 'Netto maandinkomen €2.800, maanduitgaven €1.900, spaardoel-stap (groep 6/9).',
  ...
  },
  {
    ...
    titel: 'Stap "Fin en je gegevens": AI-toestemming kiezen, weigeren en terugdraaien (ADR 0155)',
    kriticiteit: 'KERN',
    given: 'Een vers account (profiel aangemaakt door de trigger, `ai_enabled` standaard false, `ai_consent_at` NULL — migratie 20260917130000). De onboarding opent ná de welkomstpopup met de stap "Fin en je gegevens" (stap-id `ai_keuze`, vóór "naam"): de vraag uit lib/ai/privacy-facts.ts#AI_CONSENT_QUESTION, twee gelijkwaardige tegels zonder voorselectie ("Ja, Fin mag mijn gegevens gebruiken" / "Nee, geen AI", elk met keuze · effect · waarom) en de feiten (wat gaat mee, wat wordt gemaskeerd, lokale variant, omkeerbaar, wat vervalt/blijft) — dezelfde teksten als op /mijn/privacy.',
    when: 'De gebruiker (a) klikt "Verder" zonder keuze; (b) kiest "Ja" en klikt "Verder"; (c) kiest "Nee", rondt de onboarding af en opent Fin, de briefing en de pensioenstap; (d) zet later op /mijn/privacy de AI-schakelaar om — óf logt in met een bestaand account waarvan `ai_consent_at` NULL is (of herstelt een concept dat al voorbij deze stap stond).',
    then: '(a) "Verder" staat uit tot er een keuze is — geen stille default. (b) `POST /api/consent/ai` schrijft éérst één rij in `consent_events` (kind ai_cloud, decision granted, version AI_CONSENT_VERSION, source onboarding) en dán de eigen profielrij (`ai_enabled=true`, `ai_consent_at`, `ai_consent_version`); pas na een 200 gaat de flow naar "naam"; bij een fout blijft de stap staan met een `role="alert"`. (c) Het event draagt decision withdrawn, `ai_enabled=false`; de onboarding rondt gewoon af (save-own-data slaat zonder AI-extractie op), /overzicht laadt, de chat en de briefing melden AI_DISABLED_GATE_MESSAGE ("AI staat uit in je instellingen …", 403 `ai_disabled` uit lib/ai/privacy-gate.ts vóór de tier-gate), de pensioenstap biedt alleen de XML-/JSON-route; overzicht, budget, toekomst, belasting en doelen werken volledig. (d) De schakelaar gaat door dezelfde route (source mijn-privacy) — nooit meer client-direct — en de regel "Keuze vastgelegd op <datum> · versie <versie>" toont het tijdstip van de laatste keuze; elke omkering is een nieuw event (append-only: geen UPDATE/DELETE voor gebruikers). Voor het bestaande account zonder keuze toont de app-shell éénmalig een vergrendelde keuze-overlay (ShellOverlay kind confirm met `lockedOpen`: geen X, Escape/backdrop/swipe/terug-knop sluiten niet) met dezelfde twee knoppen en feiten; na de keuze verdwijnt hij en verschijnt hij nooit meer — de bron van "al gekozen" is `profiles.ai_consent_at`, nooit localStorage. Bestaande accounts houden AI aan tot ze kiezen (eigenaarsbesluit: pragmatisch, geen backfill).',
    assertion: {
      kind: 'ui-only',
      source: 'app/api/consent/ai/route.ts (volgorde event → profiel, zod-strict, envelope) + lib/ai/privacy-gate.ts#assertCloudAllowed (handhaving via ai_enabled) + lib/ai/privacy-facts.ts (één kopijbron voor stap, overlay en /mijn/privacy) — gedrag/tekst-toets, geen cijfermatige uitkomst; bewaakt door app/api/consent/ai/route.test.ts en de componenttests',
    },
  },
  {
  ...
  },
  {
    ...
    kriticiteit: 'BELANGRIJK',
    given: 'De budget-afrondingsstap met een gekozen template of "Leeg beginnen" geladen in de gedeelde `BudgetTreeEditor`.',
  ...
  },
  {
    ...
    when: '(a) Precies één cash-bezit met `account_type=\'checking\'` en geen `bank_accounts`-rijen (het standaardgeval ná onboarding) — `preselectTarget` bepaalt de voorselectie. (b) Twee of meer betaalrekening-kandidaten. (c) De gebruiker rondt de koppeling af met een gekozen cash-bezit als doel.',
