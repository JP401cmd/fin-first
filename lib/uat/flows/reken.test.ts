...
const NODE_IDS = new Set(REKEN_FLOW.nodes.map((n) => n.id))

describe('REKEN_FLOW — curatie-integriteit', () => {
  it('is de REKEN-zone', () => {
    expect(REKEN_FLOW.zone).toBe('REKEN')
  })

  it('heeft unieke knoop-ids', () => {
    expect(NODE_IDS.size).toBe(REKEN_FLOW.nodes.length)
  })

  it('elke scenarioId (indien aanwezig) bestaat en heeft zone REKEN', () => {
    for (const node of REKEN_FLOW.nodes) {
      ...
      expect(scenario!.zone, `scenario ${node.scenarioId} moet zone REKEN hebben`).toBe('REKEN')
    }
  })

  it('elke edge.from/edge.to verwijst naar een bestaande node-id', () => {
    for (const edge of REKEN_FLOW.edges) {
