...
describe('BUDGET_FLOW — curatie-integriteit', () => {
  ...
  it('dekt alle 29 WF-BUDGET-scenario\'s (01..29, aaneengesloten — geen verwijsregel-gaten)', () => {
    const covered = new Set(
      BUDGET_FLOW.nodes.map((n) => n.scenarioId).filter((id): id is string => Boolean(id)),
    )
    const expected = Array.from({ length: 29 }, (_, i) => `UAT-BUDGET-${String(i + 1).padStart(2, '0')}`)
