...
describe('BUDGET_FLOW — curatie-integriteit', () => {
  ...
  it('dekt alle 28 WF-BUDGET-scenario\'s (01..28, aaneengesloten — geen verwijsregel-gaten)', () => {
    const covered = new Set(
      BUDGET_FLOW.nodes.map((n) => n.scenarioId).filter((id): id is string => Boolean(id)),
    )
    const expected = Array.from({ length: 28 }, (_, i) => `UAT-BUDGET-${String(i + 1).padStart(2, '0')}`)
