...
describe('KRUIS_FLOW — curatie-integriteit', () => {
  ...
  it('dekt alle 26 statusdragende UAT-KRUIS-scenario\'s (01..26) als flow-knoop', () => {
    const covered = new Set(
      KRUIS_FLOW.nodes.map((n) => n.scenarioId).filter((id): id is string => Boolean(id)),
    )
    const expected = Array.from({ length: 26 }, (_, i) => `UAT-KRUIS-${String(i + 1).padStart(2, '0')}`)
    for (const id of expected) {
      expect(covered.has(id), `${id} moet als flow-knoop voorkomen`).toBe(true)
    }
    // Precies deze 26, geen dubbele scenario-knopen.
