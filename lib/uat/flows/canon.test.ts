...
const NODE_IDS = new Set(CANON_FLOW.nodes.map((n) => n.id))

describe('CANON_FLOW — curatie-integriteit', () => {
  it('is de CANON-zone', () => {
    expect(CANON_FLOW.zone).toBe('CANON')
  })

  it('heeft unieke knoop-ids', () => {
    expect(NODE_IDS.size).toBe(CANON_FLOW.nodes.length)
  })

  it('elke scenarioId (indien aanwezig) bestaat en heeft zone CANON', () => {
    for (const node of CANON_FLOW.nodes) {
      ...
      expect(scenario!.zone, `scenario ${node.scenarioId} moet zone CANON hebben`).toBe('CANON')
    }
  })

  it('elke edge.from/edge.to verwijst naar een bestaande node-id', () => {
    for (const edge of CANON_FLOW.edges) {
  ...
  it("dekt alle 15 statusdragende UAT-CANON-scenario's (01..15) als flow-knoop", () => {
    const covered = new Set(
      CANON_FLOW.nodes.map((n) => n.scenarioId).filter((id): id is string => Boolean(id)),
    )
    const expected = Array.from({ length: 15 }, (_, i) => `UAT-CANON-${String(i + 1).padStart(2, '0')}`)
    for (const id of expected) {
      expect(covered.has(id), `${id} moet als flow-knoop voorkomen`).toBe(true)
    }
    // Precies deze 15, geen dubbele scenario-knopen.
    expect(covered.size).toBe(15)
  })

  it('bevat cross-knopen naar de belangrijkste consumerende domeinen', () => {
    const crossZones = new Set(
      CANON_FLOW.nodes.filter((n) => n.kind === 'cross').map((n) => n.crossZone),
    )
    for (const zone of ['OVZ', 'BEZIT', 'BELAST', 'TOEK', 'CASH', 'RAPP'] as UatZone[]) {
      expect(crossZones.has(zone), `cross naar ${zone} verwacht`).toBe(true)
    }
  })

  it('heeft een entry-knoop (register) en een outcome-knoop', () => {
    expect(CANON_FLOW.nodes.some((n) => n.kind === 'entry')).toBe(true)
    expect(CANON_FLOW.nodes.some((n) => n.kind === 'outcome')).toBe(true)
  })
})
