...
export const ARCHI_CONCERNS: ArchiConcern[] = [
  {
    ...
    detail:
      'Sinds ADR 0171 duidt de ingest elk nieuwsartikel één keer in de achterkant naar parameters (lib/krant/duiding.ts) en gaat die duiding na de codecontroles automatisch vrij (B4). Waar een fout in de LLM-editie één lezer raakte, raakt een verkeerde parameter hier straks (1B) iedereen in de doelgroep: een systemische fout. Mitigatie: gesloten schema, grondingstoets op elk getal (lib/nummer-grond.ts), plausibiliteit per mechanisme, fail-closed (afgewezen bij twijfel; mechanisme vervalt bij een ongegrond getal), terugtrekken met reden in beheer, en de K1-meting van twee weken nul foute getallen bij rekenende mechanismen. Sinds fase 2 (22-09-2026) is /beheer/nieuws de enige lezer: een beheer-only oppervlak (superadmin-gate, geen gebruiker ziet iets) dat de duiding toont met per getal het grond-citaat, terugtrekken met wie/wanneer/waarom op de rij + admin_actions_log mogelijk maakt en de K1-meting per week afleidt — dat valt binnen de schaduw en is precies de mitigatie; een gebruikerslezer (1B/1C) blijft verboden tot de Wft-woordenlijst hieronder in code staat. Van de drie restpunten uit de security-run van 21-09-2026 staan (1) en (3) nog open vóór 1B/1C; (2) is dicht sinds fase 1: (1) de Wft-woordenlijst STAAT inmiddels in code (`vindWftOvertreding`, lib/krant/wft-woordenlijst.ts) en toetst gebiedende wijs, handelingskeuze, aanbieders en euro-only — maar hij draait bij het MATCHEN (lib/krant/matcher.ts, per lezer op de samenvatting), niet bij het duiden, en dekt alleen die vier patronen; een geïnjecteerde zin zonder getal, URL, meta-patroon of lexiconwoord passeert ook de tekstpoort van fase 2 en blijft dus staan; (2) DICHT sinds Krant 1F fase 1 (ADR 0176): het ophalen loopt via `haalOp` (lib/news-sources.ts) — redirects handmatig, hoogstens 3, elke hop hertoetst met `isVeiligeBronUrl` en binnen dezelfde host, body met een bytecap van 2 MB; sinds fase 2 doet de duidingsstap bovendien zélf geen HTTP meer (de regelbron-fetch is weg), dus de ingest is de enige aanroeper; restrisico DNS-rebinding staat open; (3) een versie-bump zet álle geduide rijen terug op wacht en draint op 60/dag — een bump vraagt een tijdelijk hogere cap. Verwijder dit punt zodra de K1-poort is gehaald (terugtrek-route en meting-paneel zijn gebouwd in fase 2, live met de release die ze meeneemt) én de drie restpunten dicht zijn; verzwaar het naar risk zodra 1B/1C de duiding aan lezers toont voordat die poort en de drie restpunten zijn gehaald.',
    severity: 'debt',
    elementIds: ['as-nieuws'],
    reviewedAt: '2026-09-22',
  },
  {
    id: 'nieuws-duiding-detailpagina-pas-in-fase-3',
    title: 'De duiding leest één fragment per rij — het échte artikel achter een lijstitem haalt ze niet op',
    detail:
      'Sinds Krant 1F fase 2 (22-09-2026) is de grondslag van de duiding uitsluitend `bron_kop` + `bron_fragment` van de eigen rij: geen paginatekst van de overzichtspagina meer, geen `raw_content`, geen `summary`, geen modelkop. Dat sluit de P1-klasse (vier regressiegevallen in lib/krant/__golden__/bron-*.json; de bijt-proef staat in lib/krant/duiding-poort.golden.test.ts) en het onderscheid tussen bronsoorten vervalt daarmee volledig — élke soort leest dezelfde twee kolommen, geborgd door de bron-scan in lib/krant/duiding.grondslag.test.ts. Wat ervoor is ingeruild: bij een `web_lijst`-item is het fragment de LINKTEKST, dus vaak één regel. Een artikel dat zijn cijfer pas op de detailpagina noemt, levert nu een duiding zonder mechanisme en (na de tekstpoort) vaak zonder samenvatting — de lezer krijgt dan bronkop + link. Dat is de fail-closed kant, maar het drukt de dekking. Het ophalen van de detailpagina per item (`bron.detailUrl` staat al in de fixtures) is bewust uitgesteld naar fase 3, achter een security-run: het is een fetch per rij naar een URL die van een externe pagina komt, en de fetch-hardening van fase 1 (`haalOp`, redirect-grens, `isVeiligeBronUrl`, bytecap) dekt die aanroepvorm nog niet. Sinds ADR 0178 (25-09-2026) wordt dit punt tijdelijk ZWAARDER bij een modelstoring: valt `kiesArtikelLinks` weg, dan neemt `terugvalLinks` de eerste vier links per lijstpagina zonder relevantie-oordeel, dus juist méér dunne-fragment-items — precies de soort die hier de dekking drukt. Dat is bewust (een bronklasse die leeft weegt zwaarder dan een bak zonder ruis), begrensd op vier per bron, en zichtbaar via de oorzaak-codes `terugval_geen_model`/`terugval_model_fout`. Verwijder dit punt zodra fase 3 de detailpagina per item ophaalt en de K1-meting laat zien dat de dekking (perGrondslag.fragment vs. de poort) daarmee terug is.',
    severity: 'debt',
    elementIds: ['as-nieuws'],
    reviewedAt: '2026-09-25',
  },
  {
    id: 'krant-twee-editiepaden-tot-1c',
    title: 'Twee editiepaden naast elkaar tot 1C: het LLM-archief en de schaduw van de Krant',
    detail:
      'Sinds ADR 0173 bouwt de weekcron (/api/krant/cron, maandag 06:00 UTC) per lezer een deterministische editie en schrijft die in krant_edities/krant_editie_items met bron "schaduw" — bewust in EIGEN tabellen (keuze 8 van de eigenaar, 21-09-2026), zodat news_editions (het LLM-archief met vijf lezers, o.a. het verversingsquotum in checkRefreshLimit) en het lokale pad ongemoeid blijven. Gevolg: twee paden die niets delen behalve news_articles. Het profiel dat de matcher leest (nieuwsprofiel) wordt wekelijks uit de eigen data afgeleid met een expliciete user_id-scope per tabel (de cron draait op de service-role en de SELECT-policy op assets is huishoud-gedeeld) — de bron-scan in lib/krant/profiel-afleiding.test.ts is daar de vangrail, niet RLS. Niets leest krant_edities: er is geen route, geen loader en geen UI; de leesstatus van het cloudpad draagt model-id\'s zonder koppeling naar het artikel, dus "gezien" telt in K1 alleen op het lokale pad. Verwijder dit punt zodra 1C de lezers op krant_edities zet, de oude AI-edities met met_ai = true heeft verhuisd (B9) en de leesstatus op het artikel-id staat.',
    severity: 'info',
  ...
  },
  {
    ...
    detail:
      'Een cijfer dat op maandenoude transacties rust ziet er precies zo uit als een cijfer van gisteren. `lib/transaction-staleness.ts` velt daarover één canoniek oordeel, maar het TONEN ervan was opt-in per scherm: van de dertien oppervlakken die op transactie-afgeleide cijfers rekenen droegen er twee de melding. UR3-22 fase 1 heeft de opt-in vervangen door een gedeelde ingang (`components/app/stale-data-guard.tsx` → provider → `StaleNoticeBanner` + `StaleNoticeDot`), /overzicht en /overzicht/budget/transacties daarop overgezet (inclusief het ontbrekende terughaalpunt op de tweede) en het grootste gat gedicht: de cloud-AI-context draagt nu een expliciet voorbehoud (`lib/ai/context/data-freshness.ts`), waar Fin eerder stellig “je hebt deze maand…” kon zeggen op vijf maanden oude cijfers. NOG NIET GEDEKT: de check-in (Budget-/Vooruitblik-stap), /toekomst (horizon build-input), de briefing, /mijn/jaaroverzicht, /api/benchmark-comparison, de cash-flow-widget bij een gevuld cijfer, en de lokale on-device Fin (`renderOverview` in local-chat-prompt.ts — die raakt de prompt-parity-poort en hoort in dezelfde ronde als een DNA-wijziging). Er is ook nog geen gate die afdwingt dat een NIEUW scherm het voorbehoud meebrengt. Verwijder dit punt zodra die oppervlakken door de guard (of de contextregel) gedekt zijn.',
    severity: 'risk',
    elementIds: ['as-transacties', 'as-coach', 'as-planning'],
  ...
  },
  {
    ...
    detail:
      'ADR 0139 legt de filter op rekeningen met "budgetteren uit" in `public.tx_month_aggregate` (migratie 20260912120000). Daarmee honoreren de zes cijfers die op dat aggregaat staan de keuze: budgetsom, transactie-jaarinkomen, spaarquote, dagtarief, FIRE-uitgaven en de snapshots. De oppervlakken die hun eigen rijen uit `transactions` halen, doen dat NIET — en dat zijn er meer dan één: `lib/core-data-loader.ts` haalt de huidige-maand- en vorige-maand-besteding rauw op (`BUDGET_SPENDING_TX_COLUMNS`, r1028/r1070) plus een 6-maands budget-sparklinevenster (r1061); `lib/budgets-data-loader.ts` doet hetzelfde voor /overzicht/budget (r96 huidige maand, r120 twaalf maanden); en de AI-contextbouwers (`lib/ai/context/budget-insights-context.ts`, `budget-summary.ts`, `spending-patterns-context.ts`, `wil-context.ts`, `lib/ai/tools/lookup.ts`, `lib/ai/local/local-chat-context.ts`) lezen rechtstreeks. OMGEZET (19 sep 2026): `app/api/checkin/gespreksstarters/route.ts` stond hier niet op de lijst maar was er wel één van — de 6-maands spaarquote-sommen kwamen uit rauwe rijen op de kolom `is_income`; de route leest nu `getTxAgg12m` + `deriveSavingsRate6mWindow` en honoreert de filter dus wél (kaart "restdivergentie na R2"). De zuster-route `app/api/checkin/overview/route.ts` leest zijn 6-maands rijen voor de FIRE-leeftijd nog rauw. Gevolg zodra iemand budgetteren uitzet: /overzicht/budget kan voor "deze maand" een hoger uitgavenbedrag tonen dan de 12-maands grondslag op datzelfde scherm, en Fin kan een bedrag citeren dat nergens anders in de app staat — één keuze, twee werkelijkheden. Vandaag INERT: op productie is er NUL uitgesloten rekening (gemeten 12-09-2026) — de enige kandidaat was de archief-bucket, en die is sinds het eigenaarsbesluit van 12 sep 2026 juist expliciet vrijgesteld (bewaarde historie na een rekeningverwijdering is opruimen, geen budgetkeuze). Dat venster sluit bij de eerste gebruiker die budgetteren uitzet op een rekening met historie. BEWUST NIET in dezelfde snede gerepareerd: de huidige-maand-vensters zijn een andere grootheid met een eigen regressie-oppervlak (ADR 0073 `currentMonth*`/`prevMonth*`), en ze meeveranderen zou zichtbare bedragen verschuiven om een reden die los staat van de aggregaat-definitie. De richting ligt wel vast en staat al als norm in CLAUDE.md ("consume, don\'t recompute" — nooit een eigen tel-lus over transacties). Verwijder dit punt zodra die vensters via het maandaggregaat lopen, of zodra de rauwe lezers dezelfde rekeningfilter toepassen.',
    severity: 'risk',
  ...
  },
  {
    ...
    detail:
      'De euro-weergave-toggle (ADR 0093) volgt de kernel: alleen oppervlakken met een kernelrij onder zich (UnifiedProjectionRow.inflationFactor) kunnen meebewegen tussen nominaal en huidige euro\'s. Eén oppervlak draait een EIGEN projectiemotor náást de kernel en heeft dus geen canonieke factor — `// euro-view: exempt` gemarkeerd, niet gedeflateerd: het geval dat niet in het oorspronkelijke ontwerp voorzien was, de aanvullend-pensioenprojectie achter `DashboardData.pensionMonthlyGross` in `components/widgets/pensioen-aow-widget.tsx`. HERIJKT 12-09-2026 (B-047): het waren er vier; de twee slider-kaarten (`compound-insight-card.tsx` + `fee-impact-card.tsx` op `lib/compound-projection.ts`) zijn met hun rekenmotor uit de app verwijderd, dus dat deel van de schuld is afgelost in plaats van gesaneerd. HERIJKT 19-09-2026 (B-053): `components/core/net-worth-projection-chart.tsx` (eigen compound-projectie mét een floor-at-zero) was sinds zijn introductie nooit aan een route gekoppeld — dode code met een bekende bug — en is verwijderd; de motor-schuld is daarmee tot dit ene oppervlak teruggebracht. Dit is een structurele klasse, geen losse voetnoten: elke motor náást de kernel is een toekomstige plek waar "€ vandaag" en "€ toekomstig" impliciet door elkaar kunnen lopen zodra iemand er zonder dit punt te lezen een tweede weergave aan toevoegt. Opvolging is een rekenmotor-sanering (aparte productkaart), geen presentatiewerk — vermeng de twee soorten besluiten niet. Verwijder dit punt zodra die sanering de resterende motor onder de kernel brengt of hem expliciet en blijvend als losstaand besluit.',
    severity: 'debt',
  ...
  },
  {
    ...
    title: 'Horizon-kernel: negen bekende, bewuste grenzen/afwijkingen t.o.v. het Excel-oracle',
    detail:
      'Tien structurele kanttekeningen bij de horizon-kernel (ADR 0032), geen bugs maar wel aandachtspunten. (1) B93 reached_now-quirk: bij een doelbedrag van €0 (legacy-nalatenschap €0, deplete zonder expliciet doel, én de pensioen-eindstrategie — B36 is daar per definitie 0) geeft het solver-statusblok altijd status reached_now zodra Prognose!J(0) ≥ 0 — letterlijke Excel-parity. De "nu al bereikt"-WEERGAVE is op beide consumer-paden afgedekt (bridge.ts#isKernelReachedNowDisplay sinds juli 2026; de scalar-router past sinds 2026-08-05 dezelfde regel toe nadat de benchmark-peer "nu al vrij" toonde op een mediaan-vermogen en pensioen-strategie-gebruikers via dit pad "Bereikt!" kregen): "nu al bereikt" alleen wanneer de bisectie ~ de startmaand vond, anders de echte gevonden FIRE-leeftijd. BIJGEWERKT 26-08-2026 (bevinding M6, gap-besluit V21 / ADR 0109): het restverschil — een reached_now-stand met gap < 0 (de verhulde horizon-parkeerstand) die alléén op het scalar-pad "Niet haalbaar" toonde terwijl de bridge fireReachable op true liet staan — is NIET langer alleen een weergaveverschil. Op het app-pad lost de KERN het nu op (zie punt 6). Wat open blijft: "fireAge ≥ eindleeftijd" (structureel ontsparen bij deplete, gap exact 0) toont op beide paden nog een concrete leeftijd — of dat "Niet haalbaar" hoort te zijn is een open weergave-besluit. Op het parity-/fixture-pad blijft de kern Excel-exact (docs/horizon-excel-oracle-plan.md §V12). (2) De scenario-band/Monte-Carlo-wrappers (lib/horizon-kernel/wrappers/band.ts, wrappers/mc.ts) leveren uitsluitend SCALAIRE uitkomsten per scenario (fireAge, netto-vermogen-bij-FIRE, netto-liquide-bij-eindleeftijd — de Sim!B/C/D-rij) — geen volledige per-jaar LedgerRow-achtige asset-/schuld-/onttrekkingsbreakdown per scenario. Een toekomstige UI die per-scenario samenstelling wil tonen (net als de hoofdgrafiek) vindt die breakdown niet; dat vereist een aparte volledige kernel-run per scenario. (3) Lege-surplus-doelpot-divergentie (gap-besluit V17, eigenaar 2026-07-03): staat de surplus-doelpot op €0, dan laat het Excel-oracle het maandspaar VERDAMPEN (Σgewicht=0 → inleg €0), terwijl de kernel via de bewuste degeneratie-fallback (toenameGewichten tak A) het spaargeld wél in de lege pot stort — bij het eigenaar-account het verschil tussen FIRE 59,58 (kernel) en 89,33 (Excel, zelfde invoer; end-to-end-verificatie scripts/horizon-oracle/*eigenaar-live*). De kernel-extensie is de gewenste semantiek; definitieve borging = Excel v6 fixen + fixtures herextraheren (eigenaar-actie); tot die tijd bewaken surplus-/withdrawal-evaporation-tests het kernel-gedrag en mag deze divergentie NIET "richting oracle" worden weggefixt zonder nieuw eigenaar-besluit. (4) Tekort-aflossing-uit-liquide (gap-besluit V19 / ADR 0033, eigenaar 2026-07-04, F6-bugfix): het Excel v5-oracle lost een tekort-lening alléén af uit de positieve maandkasstroom-surplus-tak — in de onttrekkingsfase 0 — waardoor een verkoop-transitie-lag-piek (eigenaar: €6.758 op leeftijd 75) 17 jaar met 5% rente compoundt terwijl er >€900k liquide náást staat. De kernel lost dit tekort voortaan (app-pad) maandelijks af uit de resterende liquide bezit-capaciteit (m−1-lag, onttrekking-waterval-volgorde, Σruw=0). Schakelbaar via KernelInput.tekortAflossingUitLiquide: app-pad AAN, parity-/fixture-pad UIT (input-from-fixture zet de vlag niet → 735 fixtures byte-groen). TRANSITIONEEL: borging = Excel v6 fixen + fixtures herextraheren; vangnet lib/horizon-kernel/tekort-aflossing-liquide.test.ts; niet "richting oracle" wegfixen zonder nieuw eigenaar-besluit. (5) AOW-basis-divergentie (gap-besluit V20 / ADR 0064, eigenaar 2026-07-29): het Excel v5-oracle rekent Auto-geb B21 op de 2025-basis €1.452 (alleenstaand) / €993 (samenwonend), terwijl de app de canonieke SVB-bedragen uit lib/constants.ts toont (€1.581,55 / €1.084,13 per 1-7-2026) — ~8% verschil. Opgelost als INJECTIE, niet als gelijktrekking: KernelInput.autoGebeurtenissen.aowBasisPerMaand is optioneel en inert-by-default (weggelaten → oracle-fallback in tables/auto-gebeurtenissen.ts; input-from-fixture zet het veld níet → fixtures byte-groen), en de app-adapter injecteert APP_AOW_BASIS_PER_MAAND (= de constants). Partner-AOW (PT!B9) volgt dezelfde grondslag. Gevolg: het app-pad rekent bewust met een hogere AOW dan het oracle; die divergentie blijft bestaan tot Excel v6 dezelfde basis draagt en de fixtures heréxtraheerd zijn. Vangnet lib/horizon-kernel/aow-basis-injectie.test.ts; oracle-fallback niet wijzigen zonder fixture-herijking + nieuw eigenaar-besluit. (6) Onmogelijke FIRE-uitkomsten uit B93 (gap-besluit V21 / ADR 0109, eigenaar 26-08-2026, bevinding M6): het Excel-statusblok toetst `Prognose!J(0) ≥ B36` VÓÓR `B38 < 0`. Bij B36 = 0 (deplete/pensioen/legacy-0) is die toets triviaal waar en maskeert hij de horizon-PARKEERSTAND (B16 = 100, gap < 0); bij perpetual wordt B36 zelfs negatief (J@FIRE < 0) en valt de gap op fireAge = 100 per constructie exact 0, zodat de horizon-check niet ingrijpt. bridge.ts leidde daaruit fireReachable = true + fireAgeFractional = 100 af — /toekomst toonde "VRIJHEIDSLEEFTIJD 100,0 jaar" en "DOELBEDRAG €−11.328.971 benodigd" als hard feit. Opgelost met KernelInput.reachedNowVereistBereikbaarDoel (optioneel, inert-by-default): reached_now mag niet vallen op B36 < 0 of B38 < 0 ⇒ unreachable_within_horizon. App-pad AAN (adapter), parity-/fixture-pad UIT (input-from-fixture zet de vlag niet → fixtures byte-groen, géén herijking). De bridge markeert bovendien de eind-horizon-terugval expliciet (requiredFireIsEndOfHorizonFallback) i.p.v. de geprojecteerde eindstand stil als "benodigd vermogen" door te geven. Tweede verdedigingslinie in de weergave: lib/horizon/outcome-guard.ts (leeftijd ≥ horizonplafond — alias van kernel MAX_AGE, geen tweede getal — of doelbedrag ≤ 0/terugval ⇒ "We missen gegevens"), geconsumeerd op /toekomst (KPI-tegels, kassabonnen, sim-widget, grafiek-uitleg) én /overzicht (Vrijheid-strip, mini-vermogensgrafiek via fireAgeForDisplay). Vangnet lib/horizon-kernel/onmogelijke-uitkomst.test.ts + lib/horizon/outcome-guard.test.ts; de kern-scoping niet "richting oracle" wegfixen zonder nieuw eigenaar-besluit. (7) Guardrails-anker bij FIRE-maand 0 (ADR 0126/0127, gedicht 2 sep 2026): het guardrails-onttrekkingsanker was self-captured op maand fireMaand−1 — bestond niet bij fireMonth = 0, dus het anker bleef 0 voor alle 1200 maanden en gaf een PERMANENTE cut op de uitgave-term. De engine initialiseert het anker sinds 2 sep 2026 zelf op de T0-liquide-stand, zodat elke run die op maand 0 kortsluit (de "nu stoppen"-runway/eindstrategie) een correcte guardrails-factor krijgt. Bewuste rest-afwijking die NIET is opgelost: Ont!G(0) is de oracle-cel ""→0 (m−1-lag), dus maand 0 zelf houdt niettemin één maand de floor-factor — één van 1200, geaccepteerd om de oracle-parity niet te breken. Vangnet lib/horizon-kernel/guardrails-anker-fire-maand-0.test.ts; niet "richting oracle" wegfixen zonder nieuw eigenaar-besluit. (8) Schuld→bezit-overloop verdampte bij surplus "Schulden aflossen" (gap-besluit V24, UAT WF-TOEK-25-bug1, gedicht 3 sep 2026): bij surplus_group = schuld_aflossen zet de adapter (prio-overgang.ts#toenamePrio) ÁLLE bezit-categorieën op reserve-prio 5, dus wBezitToename (verdeling/weights.ts#halveningWeights) is all-zero; de schuld-categorieën zijn bovendien statisch op t=0 gevuld/gewogen. Zodra de aanwijsbare schuld was afgelost (caps 0) lekte het volledige aflos-budget als onbenut, en de HC:HH-overloop naar bezit (onbenut · wBezitToename = onbenut · 0) liet het maandoverschot voor de rest van de horizon uit het grootboek VERDAMPEN — op de persona "compleet" verschoof enkel deze keuze de vrijheidsleeftijd 42 → 53 en het afbouw-startbedrag €1,17M → €2,24M. Zelfde bugklasse als (3) en de afname-/onttrekking-degeneratie. Het Excel-oracle kent deze configuratie niet (schuld_aflossen is een app-specifieke prio-mapping; de schuld-prio-fixture heeft gevulde bezit-toename-prio\'s, dus daar is wBezitToename > 0). Opgelost als kernel-extensie, INERT-BY-CONSTRUCTION (poort: Σ wBezitToename = 0 én onbenut > ε — op de 19 fixtures nooit waar; parity-suites byte-groen, géén KernelInput-vlag, zelfde patroon als V17): de overloop valt terug op de gedeelde instroom-ladder bezitInstroomFallbackWeights (verdeling/weights.ts: prio 1..4 zonder gevuld-eis → prio-5-reserve gelijk gewogen → all-nul), die sinds deze fix óók de V17-toename-degeneratie (tables/toename-afname.ts#toenameGewichten) voedt — één formule, één plek — en POT-BEWUST is: alleen categorieën met ≥ 1 niet-woning-pot zijn instroom-doel, want Bez deelt per stuk door aantal (aantal 0 → het bedrag verdampte alsnog één tabel later; bv. "Naar beleggingen" gekozen zonder beleggingspot) en de eigen woning is bij "meerekenen" wel TS-liquide maar geen spaarpot voor kasoverschot. Bewuste semantiek ná aflossing: gelijk verdeeld over de liquide reserve-potten (de gebruiker gaf geen tweede bestemming op; expliciete categorie-prio\'s (V5) sturen de overloop via het reguliere pad). Vangnet lib/horizon-kernel/aflossing-overloop-verdamping.test.ts (tabel + ladder + engine/solver); niet "richting oracle" wegfixen zonder nieuw eigenaar-besluit. (9) App-pensioen verlaat het oracle-pad (ADR 0129 D9/B4, eigenaar 3 sep 2026, klasse ADR 0033): het Excel-oracle kent het stopmoment alleen als eindstrategie-selector "Pensioenleeftijd" (B16 kortsluit op ES!C15, B35 = het artefact 100, MC!B8 = ROUND(B16,2) ≥ ES!C15 → per constructie 100%). Sinds F2 stuurt de app-adapter de EIND-VORM als selector plus een los `KernelInput.stopAnker`-blok (`{soort:\'aow\'}`), zodat de eindleeftijd uit `fire_end_age` komt (M1 zette die kolom op 100 voor de bestaande pensioen-rijen — identieke uitkomst), Monte-Carlo voor pensioen een échte simulatie is i.p.v. triviaal 100%, en het statusblok `anchor_shortfall` zet (de bridge vertaalt dat tijdens F2/F3 nog naar `pension_shortfall` voor het bestaande UI-blok). Contract-ronde 5 sep 2026 scherpte de anker-tak aan: onder een vast anker is élke "nee" een tekort (`tekortLening > 0 ∨ gap < 0 ∨ doelbedrag < 0` → `anchor_shortfall`), nooit `unreachable_within_horizon` (bisectie-taal); het perpetual-doel wordt bij een stopmoment vóór de startleeftijd op max(0, FIRE-maand) gelezen; `ankerMaand` is het stopmoment van de RUN (`SolveFireResult.vastStopLeeftijd`), niet van het plan. Het bewijs dat de app-uitkomst voor pensioen nog klopt is voortaan de gouden rij `B-pensioen` (lib/regression-tests/horizon-strategie/matrix.ts, rijen identiek onder M1-waarden) plus anker.test.ts/bridge.status-compat.test.ts, niet meer de oracle zelf; de 736 fixtures blijven byte-identiek omdat `input-from-fixture` het blok nooit zet (patroon V17/V19/V21). Niet "richting oracle" wegfixen zonder nieuw eigenaar-besluit; F4 verwijdert de compat-vertaling en de selector \'Nu stoppen\'. (10) Niet-geïndexeerd pensioen is nominaal vast, niet gede-indexeerd-en-centraal-geïndexeerd (gap-besluit V25 / ADR 0167, eigenaar 19 sep 2026): de Excel-structuur voor Auto-gebeurtenissen M bij H="Nee" deelt het maandbedrag één keer naar de ingangsleeftijd en laat CF!H het daarna élke maand terug-indexeren — exact op de ingangsmaand, daarna ×(1+i)^(m/12−ingang): dat is groei, geen vast bedrag. Onbeproefd (geen van de 19 fixtures heeft H="Nee") en intern strijdig met het partnerpensioen PT!K, dat bij niet-geïndexeerd wél vlak nominaal blijft. Dezelfde fout zat in de adapter voor eigen gebeurtenissen (cashflowToGebPost deelde naar de startmaand) — daar is het géén oracle-afwijking (Excel-Geb kent geen indexatievlag per post) en is de fix GebPost.nominaalVast. Voor pensioen: KernelInput.pensioenNominaalVast — app-pad AAN (adapter/index.ts), parity-/fixture-pad UIT (input-from-fixture zet de vlag niet → fixtures byte-groen). Live geraakt bij de fix: 24/55 actieve niet-geïndexeerde periodieke events bij 12 gebruikers + 15/18 pensioen-events; niet-geïndexeerde inkomsten worden later in het leven LAGER (FIRE-leeftijd later / dekking lager), niet-geïndexeerde kosten lager. Vangnet lib/horizon-kernel/geb-nominaal-vast.test.ts; niet "richting oracle" wegfixen zonder nieuw eigenaar-besluit; definitieve borging = Excel v6 met een indexatievlag per post + fixtures herextraheren.',
    severity: 'debt',
    elementIds: ['as-planning', 'fn-toekomstplannen'],
    reviewedAt: '2026-09-05',
  },
  ...
  },
  {
    ...
    detail:
      'ADR 0107 maakte de canonieke server-FIRE-run perspectief-bewust: `computeHorizonFireSim(supabase, perspective)` keyt zijn cache op het perspectief en voedt de kernel met `fireAssets`/`fireDebts` — de perspectief-rijen met het huishoud-aandeel toegepast. De VERMOGENS-kant is daarmee correct in de huishoud-/partnerblik. Drie dingen blijven bewust persoonlijk en horen bij de vervolgkaarten (M22/H6/H4): (1) de UITGAVEN-kant — `effectiveInput.monthlyExpenses` en `yearlyMustExpenses` komen uit de eigen transacties/budgetten, dus een huishoud-run deelt huishoudvermogen door persoonlijke uitgaven. Dat was óók de grondslag van de closed-form benadering die ADR 0107 verving, dus de scheefheid is geërfd, niet geïntroduceerd; ze is nu alleen exact in plaats van benaderd. (2) `loadDashboardData` (de widget-rail, briefing, deelbare vrijheidskaart) is in zijn geheel persoonlijk — mengen met een perspectief-FIRE-doel zou een huishoud-noemer onder een persoonlijke teller zetten, erger dan het huidige verschil. UITZONDERING sinds H4 (26 aug 2026): op /overzicht overschrijft `withCanonicalOverviewFigures` DRIE bundelvelden met de canonieke horizon-waarden — `healthScore`, `freedomPct` en `emergencyFund`. Dat mengt niets: het vervangt teller ÉN noemer in één keer door een intern consistent cijfer, precies zoals de hero en de kassabon erboven het al toonden. De onderliggende loader blijft persoonlijk; de noodfonds-pot is dat óók op het canonieke pad (hij leest de eigen assets-rijen, terwijl `totalAssets`/`totalDebts` in diezelfde bundel wél perspectief-correct zijn — een aparte kaart waard). (3) De /toekomst-CLIENT-run draait op de eigen asset-/schuldrijen (horizon-client#loadData haalt ze zelf op), dus /toekomst is end-to-end persoonlijk; /toekomst/page.tsx geeft daarom bewust géén perspectief mee, anders zou de server-hero huishoud zijn en de curve eronder persoonlijk. In de huishoudblik toont /toekomst de gecombineerde lijn van de huishoud-sectie (`buildHouseholdProjectionInput` → `household-router`, mét PT-partnerlaag) als hoofdlijn. Zichtbaar gevolg: in de huishoudblik geeft /overzicht een huishoud-vrijheids-% en /toekomst een persoonlijk. PARTNERBLOK GEDICHT (TPR-07, 13 sep 2026, fase 1): de canonieke huishoud-run (`computeHorizonFireSim(supabase, "household")`) draaide tot dan een SOLO-kernel op de gecombineerde potten — Box 3 heffingvrij ×1, geen partner-inkomen/-AOW — terwijl de huishoud-sectie de PT-laag wél draaide: twee grondslagen voor hetzelfde huishouden. Nu bouwt ÉÉN helper (`lib/horizon-kernel/adapter/partner-blok.ts#buildKernelPartnerBlok`) het partnerblok voor beide routes; server-side vult `lib/horizon/partner-kernel-blok.ts` het uit de privacy-gated RPC `household_member_profiles` (toekomst verborgen / geen geboortedatum → solo, zoals voorheen). Gepind in lib/horizon-kernel/convergentie-router.partner.test.ts: zelfde KernelInput en zelfde FIRE-leeftijd als de sectie-combined-run op hetzelfde testhuishouden. TIJDAS GEDICHT (TPR-07 fase 2a, eigenaarsbesluit 19 sep 2026, optie A): de head van de huishoud-sectie (`lib/household-projection.ts`) is voortaan de KIJKER, niet meer de oudste partner — dezelfde as als de canonieke run op het eigen profiel, dus /toekomst en /overzicht spreken per kijker één as en de run draait op de eigen instellingen. "Oudste overal" (optie B) is afgewezen omdat de canonieke run dan op de instellingen van de PARTNER zou draaien (privacy-verbreding). De PT-laag is as-agnostisch (een oudere partner geeft een drempel ≤ 0), bewezen met de spiegelfixture (kijker jongste) in convergentie-router.partner.test.ts. Gevolg: partners zien voor hetzelfde huishouden een ander leeftijdsgetal (zelfde kalenderdatum) — de huishoud-FIRE-leeftijd wordt daarom altijd als kalenderjaar + beide leeftijden getoond (household-fire-section.tsx). De eigen overlay op /toekomst is niet langer head-only; partner-eventmarkers staan DOB-verschoven op de kijker-as; de partner-AOW-marker komt uit de bridge (`KernelUnifiedResult.partnerAowAge` uit PT!B11 — consume, don\'t recompute). De schrijfactie `set_household_combined_summary` is vervallen (kijker-afhankelijk, geen lezers); RPC + kolom opruimen = aparte schemawijziging. WAT NOG OPEN STAAT (TPR-07 fase 2b/2c): (b) de UITGAVENKANT hierboven (huishoud-methode auto_shared/sum_partners/custom vs. eigen budgetten — één resolver in lib/retirement-expense-basis.ts); (c) de partner-eigen levensgebeurtenissen (RPC `household_partner_life_events`) zitten wél in de sectie-combined-run — ongeschoven op de head-as, een latent fase-1-defect — en niet in de canonieke run; ontwerp: via het partnerblok (PT!B6-B8 + DOB-verschuiving), nooit via top-level lifeEvents. Verwijder dit punt zodra de huishoud-uitgavengrondslag bestaat en /toekomst end-to-end op hetzelfde perspectief draait.',
    severity: 'debt',
    elementIds: ['as-planning', 'fn-toekomstplannen'],
