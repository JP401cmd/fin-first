...
export const ARCHI_CONCERNS: ArchiConcern[] = [
  ...
  },
  {
    ...
    title: 'bank_accounts.linked_asset_id en valuations.entity_id missen de eigenaarschaps-guard van hun zusterkolommen',
    detail:
      'RLS scopet de RIJ, niet de WAARDE van een FK-kolom daarop (ADR 0075) — vierde en vijfde keer dat dit patroon opduikt, na profiles.role/commercial_tier (ADR 0049), bank_connections.target_bank_account_id (fase 4, gedicht) en bank_connection_accounts.bank_account_id (fase 6, gedicht). (a) bank_accounts.linked_asset_id heeft géén guard-trigger: de policy op bank_accounts is breed genoeg dat een browserclient deze kolom naar een asset van een huishoudpartner zou kunnen laten wijzen. lib/truelayer/balance-sync.ts#syncAccountBalance filtert sinds fase 8 elke schrijfronde op user_id als code-side control, maar dat dekt alleen dat ene aanroeppad, geen datalaag-invariant. (b) valuations.entity_id is de zwaarste variant: nul triggers, geen FK (het veld is polymorf — entity_type kiest assets of debts — dus een echte FK kán niet), en een INSERT-with_check die alléén user_id toetst, waardoor ook ownership en household_id vrij zetbaar zijn. Een huishoudpartner kan zo een verzonnen waardering op een GEDEELDE bezitting van de ander schrijven, en lib/assets-data-loader.ts leest valuations zonder user_id/ownership-filter en groepeert op entity_id — die rij landt dus in de waarderingshistorie van de eigenaar, buiten de perspectief-loader om. De oude globale sleutel UNIQUE (entity_id, valuation_date) remt dat vandaag tot dagen waarop de eigenaar zelf nog niets had; die sleutel verdwijnt in stap 2 van de expand/contract-migratie (stap 1 = 20260730072804_add_valuations_user_scoped_unique.sql). De guard-trigger + het scopen van die loaderquery zijn daarmee een VOORWAARDE voor stap 2, niet een vervolg erop. Verwijder dit punt zodra beide kolommen een guard-trigger hebben (spiegel van guard_bank_connection_target_account) en de valuations-lezing in assets-data-loader.ts eigenaar-gescoopt is.',
    severity: 'risk',
