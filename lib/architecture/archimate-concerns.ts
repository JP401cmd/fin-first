...
export const ARCHI_CONCERNS: ArchiConcern[] = [
  ...
  },
  {
    ...
    title: 'Crypto-kolommen van assets bereiken de browser nog via de gedeelde loaders',
    detail:
      'VERSMALD op 31 jul 2026 — de structurele oorzaak is weg, één plek en één restkolom staan nog open. Het punt: een `select(\'*\')` op een tabel met veld-encryptie levert de browser óók `*_encrypted` en `*_hash`. Die hash is een blind index (HMAC-SHA256 onder een server-only sleutel): dezelfde invoer geeft altijd dezelfde waarde, dus een stabiele correlatiesleutel die niets toevoegt aan wat het scherm toont. Op `assets` weegt dat zwaarder dan op de zustertabellen omdat de SELECT-policy daar HUISHOUD-GEDEELD is (`auth.uid()=user_id OR (ownership=\'shared\' AND household_id IS NOT NULL AND household_id=user_household_id())`): bij een gedeelde bezitting belandt het materiaal van de PARTNER in de bundel van de vragende gebruiker, buiten de perspectief-loaders om. GEDICHT: `bank_accounts` (984b54eba), `bank_connection_accounts` (1e2125e8f), `bank_connections` heeft geen client-reads, en op `assets` zijn zeven van de negen plekken omgezet naar de gedeelde kolomconstante `ASSET_CLIENT_COLUMNS` (lib/asset-data.ts) — asset-detail-flow, asset-pane (beide lijst + één-rij), whatif-page-client, revalue, verhuurrendement-tab en hypotheekplanner-tab. GEDEELTELIJK GEDICHT, NIET STRUCTUREEL: scripts/check-client-data-reads.mjs heeft nu een tweede regel op KOLOMNIVEAU die `select(\'*\')` op deze vier tabellen in \'use client\'-bestanden hard afkeurt; die regel staat bewust LOS van de grandfather-allowlist, dus een bestand daarop zetten onderdrukt hem niet. Maar die regel dekt alleen een LETTERLIJKE `.from(\'assets\').select(\'*\')` ín een bestand dat zélf een `use client`-directive draagt — en precies daarbuiten lopen de twee grootste paden nog. WAT RESTEERT, drie dingen, zwaarst eerst. (1) DE GEDEELDE PERSPECTIEF-LOADER. `lib/household/perspective-loader.ts#loadPerspectiveData` doet zelf `supabase.from(\'assets\').select(\'*\')` en wordt vanuit DRIE clientbestanden aangeroepen: components/core/assets-client.tsx:309 (de bezittingen-hoofdpagina), components/app/cash-overview.tsx:329 en components/core/debt-category-page.tsx:216. Dat bestand draagt geen `use client`-directive, dus de gate opent het niet eens — de grootste assets-lezer van de app is voor regel 2 onzichtbaar. Bovendien levert de RPC `household_partner_items(\'assets\')` bij privacyniveau `full` een `to_jsonb(a)` van de PERSOONLIJKE bezittingen van de partner, inclusief alle drie de account-nummer-kolommen. (2) DE SERVER-GELEVERDE BUNDELS. `lib/core-data-loader.ts#fullAssets`, `lib/server-data/base.ts#getActiveAssets` (via horizon-data-loader) en `lib/assets-data-loader.ts` (via dezelfde perspectief-loader) lezen alle drie `select(\'*\')` en gaan als prop naar een clientcomponent (`<CoreLanding>`, `<HorizonPage>`, `<AssetsPage>`); Next serialiseert die props volledig in de RSC-payload, dus de kolommen staan in de paginabron. (3) app/(app)/core/checkin/page.tsx:244 doet nog een letterlijke `select(\'*\')`; bewust overgeslagen omdat het bestand in een parallelle sessie onder handen was. Het staat als enige entry op COLUMN_RULE_RESIDUE in die gate — een lijst die alleen kan krimpen (een entry die géén overtreding meer is, maakt de gate hard rood). LOS DAARVAN staat het plaintext `account_number` dat de browser nog bereikt langs twee één-rij-reloads (asset-detail-flow + asset-pane, `${ASSET_CLIENT_COLUMNS}, account_number`) en langs de server-geleverde `asset`-prop. Dat is geen regressie maar bestaand en NOODZAKELIJK gedrag: `AssetForm` in components/core/assets-client.tsx vult er het IBAN-veld mee en schrijft het bij opslaan terug (`account_number: iban || null`), dus de kolom weglaten zou het rekeningnummer bij de volgende bewerking WISSEN. De echte uitweg loopt via die vorm (rekeningnummer als eigen, eigenaar-gescoopt schrijfpad) en valt samen met de Stage B-drop van `assets.account_number`. Verwijder dit punt pas als (1), (2) en (3) om zijn; de kolomregel is tot die tijd een gedeeltelijke vangrail, geen bewijs.',
    severity: 'risk',
