...
export const CALCULATIONS: Calculation[] = [
  ...
  },
  {
    ...
    outputs: ['LedgerRow[] (grootboek; per rij liquideVermogen = FIRE-eligible pot ÉN besteedbaarVermogen = RAUW besteedbare pot)', 'V_nodig per jaar', 'FIRE-snijpunt (integer fireAge + fractionele fireAgeFractional)', 'tabellen A–G'],
    formula: 'forward: vermogen × (1+reëel rendement) + sparen − Box 3 − onttrekking; surplus (opbouw) = annualSavings − flaggedAflossing + freedHousingCost + events; freedHousingCost = Σ lastActiveRente van afgeloste geflagde schulden (ALLÉÉN het RENTE-deel van het laatste lopende jaar valt vrij na payoff — de aflossing is dan al hersteld doordat flaggedAflossing → 0; 0 zolang ze lopen); afbouw-annuïteit teert op de RAUW besteedbare pot (withdrawableLiquidValue, ex de spendable+saleManaged downsize-woning) NIET op de eligibility-pot — ADR 0028-addendum; backward: V_nodig[i] = (V_nodig[i+1] + nettoBehoefte) / (1 + reële onttrekkingsvoet)',
    files: ['lib/horizon-engine/engine.ts', 'lib/horizon-engine/strategies.ts', 'lib/horizon-engine/adapter.ts', 'lib/horizon-engine/build-input.ts', 'lib/horizon-engine/scalar-bridge.ts', 'lib/withdrawal-strategy.ts', 'lib/pot-rules.ts', 'lib/sale-config.ts', 'lib/housing-strategy.ts', 'app/(app)/horizon/whatif/whatif-page-client.tsx'],
    functions: ['runHorizonLedger', 'crossingAge', 'blendedRealReturnStart', 'meetsStrategyTarget', 'ledgerToUnifiedResult', 'runSelectedProjection', 'runScalarProjectionV2', 'applyWithdrawalStrategy', 'computeAnnuityBase', 'buildHorizonInput', 'buildGenericAssetLiquidations', 'buildV2DownsizeHousing', 'buildDownsizeRentAndLiquidation', 'resolveDownsizeTriggerV2', 'applyDownsizeValuationBasis', 'buildV2ReverseMortgageHousing', 'reverseMortgageBorrowable', 'liquidValue', 'liquidSumStart', 'withdrawableLiquidValue', 'withdrawableRealReturn', 'countsAsEligibilityLiquid', 'mayBeRawWithdrawn', 'expandSingleGroupToAssetTypes', 'sellNextOnDemand', 'withdrawWithOnDemand', 'drawReverseMortgage'],
    constants: [
      { label: 'SALES_COSTS_BY_TYPE', value: 'verkoopkosten per asset-type bij liquidatie — roerend (voertuig/inboedel/overig/deelneming) 2%, vastgoed 3%; per-event override via metadata.verkoopkostenPct [0,0.20] (lib/constants.ts; eigen_huis-downsize draagt zijn eigen %)' },
      { label: 'REVERSE_MORTGAGE_DEFAULT_MAX_LOAN_PCT', value: '0,50 — max % van de overwaarde dat als opeethypotheek opgenomen kan worden (NL-markt 35–65%); tevens de FIRE-eligibility-fractie én de LTV-cap. lib/constants.ts' },
      { label: 'REVERSE_MORTGAGE_DEFAULT_RATE', value: '0,055 — opeethypotheek-rente (NL 2026), bewust LAGER dan ongedekt (woning = onderpand). NOMINAAL genoteerd; de engine accrued ’m — net als élke andere schuldrente in blok 3 — direct op het reëel-gegroeide saldo zónder reëel-conversie. lib/constants.ts' },
      { label: 'DOWNSIZE_DEFAULT_SALES_COSTS_PCT', value: '0,04 — default verkoopkosten eigen-woning-downsize (makelaar+notaris+verhuizen); gebruiker-overschrijfbaar. lib/constants.ts' },
      { label: 'HOUSING_COST_AFTER_SALE_PCT', value: '0,04/jr van WOZ — geschatte woonlast na verkoop (huur). lib/constants.ts' },
      { label: 'DownsizeConfig.saleValuationBasis', value: "'market' (default) | 'woz' — grondslag voor de engine-huiswaarde bij downsize (ADR 0031). 'market' = current_value (= dashboard-netto vermogen); 'woz' = woz_value (conservatiever). Eén bron: applyDownsizeValuationBasis substitueert current_value → woz_value vóór de engine, dus netto vermogen, FIRE-pot, verkoopopbrengst én preview delen dezelfde basis. Default 'market' = continuïteit (geen wijziging). lib/housing-strategy.ts" },
    ],
    elementIds: ['as-planning', 'fn-toekomstplannen'],
    note: 'Architectuur (invarianten + uitbreidingsregels): docs/architecture/horizon-engine-v2.md. Plan: docs/horizon-tabel-rekenmotor-plan.md. ADR 0013 + 0014 + 0015 + 0017 + 0019 + 0021 + 0028 (incl. addendum drawdown-grondslag). Beheer-inspectie (v2-only): /beheer/horizon-tabellen-mij (echte-gebruiker-tabellen A–G) en /beheer/grafiek-werking (functionele referentie); de oude v1↔v2-vergelijk-inspector /beheer/horizon-tabellen is verwijderd (geen v1 meer om tegen te vergelijken). Single source of truth = LedgerRow[] (per asset/schuld); intern reëel, de adapter is het ENIGE reëel→nominaal-punt; FIRE-detectie per eindstrategie: perpetual/legacy = forward doel-zoektocht (V_nodig is referentielijn, niet detector); deplete/pensioen = liquide ≥ V_nodig op de FIRE-leeftijd (crossing met de referentielijn, ADR 0027). Zuiver (Fase 1, jun 2026): de backward-annuïteit `backwardVnodig` disconteert nu op de WERKELIJKE waarde-gewogen blended reële voet van de FIRE-eligible startpot (`blendedRealReturnStart`) i.p.v. de oude verborgen 0,6×-buffer — doel-lijn en drawdown delen één grondslag (geen mismatch). Dit beslecht het openstaande productbesluit dat ADR 0027 §Gevolgen noemde. De hogere disconto-voet drukt V_nodig over de hele lijn omlaag (FIRE schuift ~1–3 jr, getoond doelbedrag −0,7..−4,35%); de getekende stip valt via `crossingAge` op de doel-lijn, maar de INTEGER `liquideAtFire` kan tot één discrete jaarstap boven `requiredFirePortfolioAtFire` liggen (de opbouwcurve passeert de dalende V_nodig-lijn discreet). Bodem-eerlijkheid (onhoudbare uitgaven → fireReachable=false) komt van de premature-collapse-guard in `meetsStrategyTarget`, niet langer van de buffer. Strategieën zijn pure plug-ins. VPW (variabel percentage) combineert ALLEEN met eindstrategie deplete ("Vermogen opeten"): runHorizonLedger retourneert vroeg een leeg/onbereikbaar resultaat (rows:[], fireReachable:false) bij vpw×legacy/perpetual/pensioen, omdat VPW de pot tegen de einddatum volledig leegtrekt (vpwRate=1.0 laatste jaar) — onverenigbaar met nalatenschap/behoud/vaste onttrekking (spiegelt de verwijderde v1-block; bewaakt door test/horizon-vpw-guard.test.ts). Woonlast-RENTE-vrijval bij payoff (opbouwfase): de spaarquote-baseline heeft de héle jaarlast (rente + aflossing) van een geflagde schuld (include_aflossing_in_savings) als uitgave verrekend en de aflossing daarna teruggeteld (annualSavings = inkomen − uitgaven + flagged_aflossing). NETTO blijft binnen annualSavings dus alleen de RENTE permanent afgetrokken; de aflossing valt weg (uitgave − terugtelling = 0). Bij payoff brengt flaggedAflossing → 0 de aflossing AL terug in het surplus — dus valt uitsluitend het RENTE-deel nog vrij, niet de volledige jaarlast. (Eerder liet de engine de volledige annualPayment vrijvallen → de aflossing werd dubbel hersteld, R te hoog. Gecorrigeerd jun 2026.) De engine onthoudt per geflagde schuld lastActiveRente = de rente (begin × rate) van haar laatste lopende jaar (zolang begin > 0) en telt Σ lastActiveRente van reeds afgeloste geflagde schulden (begin === 0 dit jaar) op als freedHousingCost. De rente is begin × rate met begin = balance AL gewogen via net_worth_inclusion_pct (buildDebts) → automatisch inclusion_pct-consistent met flaggedAflossing en de baseline. Zolang een geflagde schuld loopt is freedHousingCost 0 → byte-identiek aan de pre-feature baseline-formule (regressie). Aflossingsvrije schulden (begin daalt nooit naar 0) en niet-geflagde schulden geven geen vrijval; bij meerdere geflagde schulden valt elke onafhankelijk haar eigen rente vrij bij payoff. (test/horizon-engine.test.ts dekt de rente-vrijval-, loader-economie-pin-, looptijd-, aflossingsvrij-, niet-geflagd- en meerdere-schulden-cases.) Eindstrategie-onttrekking: deplete = spend-down-annuïteit (→ ~€0); legacy = need-only (residu groeit naar de nalatenschap, via ctx.legacyPreserveOnly in withdrawal-strategy.ts — anders verdampt het surplus in het grootboek; ADR 0014); perpetual/pensioen = need-only. FIRE-selectie (meetsStrategyTarget): legacy is doel-zoekend (ADR 0017) — de vroegste leeftijd waarvan de afbouw-lijn ≥ legacyAmount eindigt (GEEN −2%-tolerantie meer, dus nooit ónder het doel) terwijl het liquide brug-pad richting €0 mag dippen maar nóóit negatief wordt (de buffer zit al in het ingevoerde nalatenschapsbedrag). Omdat het eindvermogen monotoon stijgt in de FIRE-leeftijd levert de vroegste passerende leeftijd automatisch het dichtst-bij-doel-resultaat; geen tweede zoekfase. Resultaatveld legacyTargetUnavoidablyExceeded (alleen legacy) markeert dat zelfs stoppen-nu al ≥ doel eindigt ("je kunt nu al stoppen"). perpetual/deplete-selectie ongewijzigd (minMid>1 + eindvermogen ≥ start×0.99 resp. niet vroegtijdig leeg). De backward V_nodig-referentielijn gebruikte al endVal = legacyAmount, dus de forward-selectie is nu consistent met de referentielijn (geen tolerantie-slack meer). Asset-liquidatie (v2, GENERIEK — niet alleen het eigen huis): elk niet-liquide asset (voertuig/inboedel/overig/deelneming/vastgoed≠eigen_huis) kan binnen het grootboek verkocht worden via UnifiedProjectionInput.assetLiquidations (engine-block 6b) i.p.v. de pot te filteren + de verkoop als inkomen in te spuiten — netto vermogen continu (alleen −verkoopkosten), alléén de liquiditeit verspringt (ADR 0015). Twee bronnen voeden die ene array: (a) het eigen-huis-downsize-pad (buildV2DownsizeHousing) met zijn eigen trigger + gebruiker-instelbare verkoopkosten, en (b) generieke niet-liquide assets via buildGenericAssetLiquidations. SSoT voor of/wanneer verkopen (ADR 0021): assets.sale_config (JSONB, nullable; lib/sale-config.ts) kent drie standen — niet_verkopen (asset blijft altijd in grootboek), vast_moment (verkoop op vaste leeftijd/datum), wanneer_nodig (in-loop on-demand; engine verkoopt zodra liquide tekortschiet — AssetLiquidation.trigger=’on_demand’, via sellNextOnDemand/withdrawWithOnDemand). Default voor alle niet-liquide types (via RESOLVE-DEFAULT in lib/sale-config.ts, geen backfill): wanneer_nodig. Dat wijzigt bestaande prognoses bewust: activa verlaten het grootboek alleen via echte verkoop (mét verkoopkosten, mét schuldaflossing), niet langer “rauw” in de onttrekkingsfase. De “engine 0-diff”-regel (v1→v2-transitiediscipline) is losgelaten per ADR 0016. Verkoopvolgorde = bestaande onttrekkingsvolgorde (meest-liquide eerst, eigen_huis allerlaatst), sort_order als tie-break. life_events.linked_asset_id blijft bestaan als prijs-kalibratie-kanaal: metadata.verkoopprijs moduleert de salePricePct (geclampt [0,2]; ontbrekend → 1.0). Opbrengst = ECHTE engine-asset-waarde (real-grown, “consume don’t recompute”). Verkoopkosten via SALES_COSTS_BY_TYPE (override metadata.verkoopkostenPct). Liquide types + levensverzekering/vordering zijn bewust NIET in scope (die komen als geldstroom binnen). Geen dubbeltelling: lifeEventsToCashflows(events, skipEventIds) onderdrukt UITSLUITEND de opbrengst-portie (one_time_cost + custom metadata.cashflows) van een als liquidatie afgehandeld event; de monthly_cost_change (bv. wegvallend onderhoud) blijft een losse cashflow. Bij stand niet_verkopen wordt een gekoppeld verkoop-event (indien aanwezig) óók aan skipEventIds toegevoegd — opbrengst onderdrukt, maandelijkse gevolgen blijven. Trigger op v2-liquide via buildHorizonInput. Downsize "Verkopen" — spendable + saleManaged verenigd (Fase 2, ADR 0028, jun 2026): de v2-downsize-woning telt vanaf nu AL als besteedbaar/FIRE-eligible liquide vermogen tijdens de OPBOUW (zoals include_full — build-input zet spendableAssetIds óók voor useV2Downsize), waardoor de FIRE-eligibility eerder bereikt wordt. Vóór Fase 2 hield downsize het huis niet-liquide tot de verkoop ("geen overlap in de praktijk"). De woning is nu BÉIDE: spendable (telt mee voor eligibility) ÉN saleManaged (verlaat de pot UITSLUITEND via de downsize-verkoop, nooit rauw onttrokken). Twee voorheen-samenvallende vragen zijn daarom expliciet gesplitst in ÉÉN gedeelde liquiditeit-helper-paar in engine.ts: countsAsEligibilityLiquid (telt mee in liquidValue / liquidSumStart / blendedRealReturnStart / de afbouw-annuïteit / de FIRE-gate — spendable telt, saleManaged-alléén niet) en mayBeRawWithdrawn (mag rauw onttrokken worden in de volgorde — saleManaged NOOIT, ongeacht spendable). Engine-block: de saleManagedIds-set filtert spendable NIET meer weg (anders zou de spendable woning rauw onttrokken mogen worden); build-input markeert het huis spendable voor zowel de grafiek (buildHorizonInput) als de modal-preview (runHousingScenarioProjectionV2). SSoT-invariant (ex-huis-exhaustion): de trigger-resolver resolveDownsizeTriggerV2 meet het liquide vermogen ZÓNDER het te-verkopen huis (de trigger-MEETRUN draait op baseSimInput dat het huis NIET als spendable markeert → countsAsEligibilityLiquid sluit het daar uit → ex-huis); de echte run emit de huis-verkoop als fixed_age-liquidatie op exact die trigger-leeftijd, dus de gerapporteerde verkoop-leeftijd == de leeftijd waarop de engine werkelijk verkoopt. NETTO FIRE-richting is return-mix-afhankelijk: meestal vroeger (realistische persona 62→58, strategie-matrix), maar een groot laag-rendement-huis drukt via blendedRealReturnStart de disconto-voet → V_nodig stijgt mee → FIRE kan in zulke fixtures +1 jr; de eligibility-pot is altijd groter. Tests: test/horizon-downsize-verkopen.test.ts (spendable-pot, continuïteit, SSoT-trigger==verkoop), test/horizon-housing-liquidation.test.ts (her-gebaselined: liquide verspringt NIET meer omhoog bij verkoop want huis zat al in de pot). C5-c voltooid via Optie B (ADR 0016): runUnifiedProjection en runSimulation zijn FYSIEK VERWIJDERD — v2 is de enige FIRE-engine. isHorizonV2Enabled retourneert altijd true; runSelectedProjection is v2-only en negeert de useV2-vlag — er is GEEN v1-arm meer in de selector. Scalar-bridge: runScalarProjectionV2 (lib/horizon-engine/scalar-bridge.ts) biedt een drop-in voor de oude scalar-portfolio-signatuur en routeert die callers (strategie-modal.tsx, de twee hypotheek-vs-beleggen-callers, de event-panes en de horizon-client scenario-overlays) naar v2. Optie B overruled de eerdere parity-behoud-beslissing (D3): de parity-/compare-tooling is VERWIJDERD — compareEngines, de ledger-API (/api/horizon-engine/ledger) en de v1↔v2-vergelijk-inspector /beheer/horizon-tabellen + de parity-tests zijn weg. (Behouden in die dir, niet-parity: ledger-views.tsx + persona.ts, gebruikt door /beheer/horizon-tabellen-mij + /beheer/grafiek-werking.) strategie-modal.tsx is NIET meer uitgesteld; het draait op v2 via de scalar-bridge. B2-parity (13 jun, na de fixes legacy-need-only/recurring-×12/housing-liquidatie): v2 ligt voor spend-down-strategieën later (persona deplete +7 jr, legacy +6 jr) door reëel-vs-nominaal, en gelijk voor behoud (perpetual +1, pensioen 0); eindnetto verschilt navenant. Owner legacy €200k: v1 onbereikbaar, v2 ~74 (haalbaar, eindigt ≥ doel). Review-vervolg (jun 2026, A–D): (A) Cash-voorkeur voor de opbrengst. De pot-regel "verdeling bij toename" (profiles.pot_rules.surplus_group) is TOP-LEVEL config, NIET per-event metadata — hij bepaalt zowel waar surplus als waar liquidatie-opbrengst heen gaat (engine surplusTargets, block 6b). BUG-FIX: potRulesToStrategyOptions gebruikte expandGroupsToAssetTypes([surplusGroup]), dat de volledige 10-type-lijst opleverde (de "vul overige groepen aan"-logica, juist voor een onttrekkings-VOLGORDE maar fout voor een DOEL) → surplusTargets matchten álle assets → opbrengst werd pro-rata over alle potten verdeeld i.p.v. naar de gekozen pot (cash kreeg slechts een fractie). Nu via expandSingleGroupToAssetTypes(group) = uitsluitend de asset-types in díe groep (spaargeld → [cash,savings]) → opbrengst belandt bij cash; default (beleggingen) blijft beleggingen. (B) What-if-baseline single-sourced: de what-if-pagina bouwde zijn baseline-input INLINE (zonder buildHorizonInput) → géén assetLiquidations/skipEventIds → bij een gekoppeld verkoop-event week de baseline af van de /toekomst-grafiek. Nu roept whatif-page-client een gedeelde buildInputForEvents-factory aan die per event-set buildHorizonInput aanroept (zelfde builder als /toekomst + /overzicht); baseline, scenario, pinned-overlays én per-event-impact lopen er allemaal door → baseline == hoofd-grafiek (incl. liquidatie + skipIds). pot_rules wordt nu óók in de what-if-loader gefetcht (defensief, defaults-fallback) zodat de surplus-/opbrengstbestemming gelijk is. (C/M1) De huis-downsize-trigger-meetrun (baseSimInput in build-input) krijgt nu genericLiq.liquidations mee zodat de meetrun dezelfde liquide opbrengsten ziet als de grafiek (huis-verkoop-trigger niet langer te vroeg). (D/M2) buildGenericAssetLiquidations leidt de leeftijd af uit target_date via ageAtDate wanneer target_age ontbreekt (de loaders normaliseren target_age NIET uit target_date). Bekende bredere beperking (gedocumenteerd, niet hier breed gefixt): lifeEventsToCashflows is óók target_age-only, dus de MAANDELIJKSE gevolgen van een datum-only event vervallen daar nog — de builder herstelt enkel de (eenmalige) verkoop-opbrengst. Tests: test/horizon-generic-liquidation.test.ts (A+D), test/horizon-housing-liquidation.test.ts (C), test/whatif-baseline-consistency.test.ts (B). Fractionele FIRE-stip (ADR 0027-addendum, "Grafiek lijn", jun 2026): fireAgeFractional WAS gelijkgezet aan de integer fireAge — daardoor stond de FIRE-stip (in sim-chart.tsx getekend op de nominale netto-vermogenscurve geïnterpoleerd op fireAgeFractional) een vól jaar vermogensopbouw ná het snijpunt, op groot vermogen €100–220k BÓVEN de horizontale deplete-doel-lijn (fireTarget = V_nodig op FIRE × inflatiefactor, nominaal). Fix: crossingAge() (engine.ts) reconstrueert de getekende curve in EXACT dezelfde grondslag als de grafiek (nominale nettoVermogen, gekeyd op leeftijd+1 zoals allPts) en bepaalt de fractionele leeftijd waar die curve de nominale horizontale doel-lijn kruist, GEKLEMD binnen het ADR-0027-detectiejaar [fireAge−1, fireAge] (architect-beslissing optie c). Het terugzoeken naar een eerdere kruising is bewust NIET toegestaan: de doel-lijn is vlak op de lage V_nodig[fireAge]-waarde (V_nodig daalt) terwijl de getekende curve het NETTO vermogen is — bij een include_full-woning kruist die steile netto-curve de lage vlakke lijn jaren eerder, en vrij terugzoeken zou de getoonde vrijheidsleeftijd jaren naar voren trekken en de bewuste ADR-0027-keuze (deplete-FIRE ~51 i.p.v. ~45 over-agressief) stilzwijgend ongedaan maken. Door binnen het detectiejaar te klemmen geldt altijd |fireAgeFractional − fireAge| ≤ 1 en blijft de headline-leeftijd ~de integer FIRE. Bestaat er een in-jaar-kruising (netto ≈ liquide, bv. een gewoon deplete-account zoals het echte account dat firsthand geverifieerd is) → stip exact op de lijn; is de curve aan het jaar-begin al boven het doel (netto ≫ liquide bij een grote include_full-woning) → stip klemt op het segment-begin (t=0, fireAge−1, hooguit ~één jaar opbouw boven de lijn i.p.v. twee). Alleen voor deplete/pensioen (legacy/perpetual hebben een oplopende doel-lijn, geen horizontale crossing-semantiek → fireAgeFractional = fireAge) en niet bij forced-fire. De INTEGER fireAge, requiredFirePortfolioAtFire en liquideAtFire blijven ONGEWIJZIGD — freedomPct/snapshots/doelbedrag schuiven niet (matrix-goldens schoven exact 1 jaar, doelbedrag ongewijzigd). Resterend structureel punt (ADR 0029-kandidaat; ADR 0028 is vergeven aan de downsize-"Verkopen"-herdefinitie, Fase 2): een VLAKKE liquide-doel-lijn onder een NETTO-curve laat bij include_full-woningen een kleine stip↔lijn-afstand staan. Bewaakt door de "Grafiek lijn"-cases in test/horizon-engine.test.ts (echt account: dot-op-lijn binnen 1% + |frac−fireAge|≤1; generiek: stip lager dan de buggy positie, ≤ één jaar opbouw boven de lijn; integer-golden-pins) — red→green geverifieerd. Eigendoms-grondslag (ADR 0027-addendum, jun 2026): de FIRE-eligible engine-waarde van élk asset = current_value × net_worth_inclusion_pct (EIGENDOM, geldt ALTIJD, voor elk asset, in elke strategie — exact gelijk aan het elders getoonde netto vermogen). `include_full` is een ORTHOGONALE as die uitsluitend de FIRE-BEHANDELING bepaalt (het eigen deel ná inclusion_pct telt volledig als liquide/FIRE-eligible/besteedbaar met zijn eigen reële return), NIET de grondslag. assetEngineValue (gedeeld door buildAssets/liquidSumStart/blendedRealReturnStart) past inclusion_pct dus altijd toe; de eerdere forceFull-override (inclusion → 100% voor het besteedbare huis) is verwijderd — daarmee geen desync meer tussen het dashboard-netto-vermogen en de FIRE/afbouw-grondslag. Echt account (huis €1.000.500 @ 50%): netto-start €583.154 i.p.v. >€1M; FIRE 52→59; requiredFirePortfolioAtFire €1.431.390→€1.089.101; liquideAtFire €1.488.016→€1.117.643. De strategie-matrix-persona (inclusion_pct 100) is byte-identiek vóór/na de fix (matrix-goldens schoven niet). Opeethypotheek in het grootboek (Fase 3, ADR 0029, jun 2026): reverse_mortgage is GEEN display-only schaduwschuld + recurring-payout-life-event meer, maar een ECHTE behandeling in runHorizonLedger. Het huis blijft eigen_huis-asset in de ledger (groeit, inclusion_pct-gewogen, NIET spendable, NOOIT rauw onttrokken — countsAsEligibilityLiquid=false, mayBeRawWithdrawn=false op de raw huiswaarde). Op de trigger-leeftijd opent een synthetische, aflossingsvrije RunningDebt "Opeethypotheek" (id reverse-mortgage, isReverseMortgage, onderpand-rente lager dan ongedekt). Het maand-/jaar-tekort wordt — ná de echte liquide pot in de onttrekkingsvolgorde (withdrawWithOnDemand geeft de shortfall terug) — opgenomen als instroom naar liquide (drawReverseMortgage, naar liquidProceedsTargets) MÉT gelijke verhoging van het opeetschuld-saldo; de rente stapelt op het saldo (blok-3-stijl aflossingsvrij, blok 6d). VIERDE liquiditeit-aspect: de leen-RUIMTE (overwaarde × maxLoanPct) telt als expliciete FIRE-eligibility-BIJDRAGE via het nieuwe input-veld collateralBorrowableById (Optie B) — opgeteld in liquidSumStart/blendedRealReturnStart (met de HUIS-return als voet) — NIET via de spendable-boolean (die zou de hele woning eligible maken; reverse_mortgage maakt slechts een FRACTIE eligible). MAX-LTV-CAP (geen oneindig lenen): de opname is gecapt op overwaarde(jaar) × maxLoanPct via reverseMortgageCap(); zowel de opname (eindsaldo na opname ≤ cap/(1+rente) − begin) als de rente-accrual (eindsaldo geklemd op de cap) respecteren de cap → het opeetschuld-saldo overschrijdt NOOIT de LTV-grens, ook niet via gestapelde rente. Daardoor is de NALATENSCHAP (nettoVermogen = totaalAssets − totaalSchuld, automatisch door de echte grootboek-schuld gedrukt) effectief max(0, huiswaarde − opeetschuld) en gegarandeerd NIET-negatief. Tekort-gedreven (monthlyPayout==null) leent REACTIEF ná de onttrekking precies het onbedekte tekort; vaste payout (monthlyPayout!=null) leent PROACTIEF monthlyPayout×12 in de onttrekkingsfase vóór de onttrekkingsberekening (beide gecapt). Een onbedekt tekort boven de cap blijft via de bestaande shortfall-mechaniek staan (FIRE-later/lijn onder doel). RENTE-INTERPRETATIE: interestRate (5,5% nominaal) wordt — consistent met hoe blok 3 gewone schuldrente behandelt (rate direct op het reëel-gegroeide saldo, geen reëel-conversie) — direct op het saldo geaccrued. BOX-3-KEUZE: de V2-engine berekent Box-3-drag uitsluitend op POSITIEVE asset-waarden (blok 2b, Math.max(0, a.value)) en netteert schulden NIET tegen de Box-3-grondslag — gewone hypotheken evenmin. De opeetschuld verlaagt de Box-3-grondslag dus NIET (conservatief, consistent met de bestaande schuldbehandeling; bewust niet over-engineerd). Routering: build-input.ts useV2ReverseMortgage-tak (buildV2ReverseMortgageHousing) zet reverseMortgage-plan + collateralBorrowableById, GEEN V1-payout-life-event, huis blijft in de pot; runHousingScenarioProjectionV2 (modal-preview) deelt dezelfde tak. De display-only schaduwschuld (applyHousingStrategy.shadowDebtAtEndAge en de reverse_mortgage-tak in applyHousingToComposition) is VERVALLEN — één grootboek-waarheid. getFireEligibleNetWorth(reverse_mortgage) gebruikt nu totalNetWorth − overwaarde + reverseMortgageBorrowable(overwaarde, maxLoanPct) i.p.v. totalNetWorth (zie vrijheidsvoortgang-note). Tests: test/horizon-reverse-mortgage.test.ts (a–g: huis-als-onderpand, leen-tegen-lagere-rente, LTV-cap, nalatenschap≥0, life-events, inclusion_pct, tekort-vs-vaste-payout); A-reverse matrix-golden her-baselined (doelbedrag €1.274.081→€1.537.473, +20,67% — de woning telt niet meer 100% mee + geen payout-inkomen meer; include_full/exclude/downsize-goldens ONGEWIJZIGD). Drawdown-grondslag gesplitst (ADR 0028-addendum / Optie B, jun 2026): bij downsize + on_depletion dipte de getoonde lijn ~71, steeg weer ~87 en crashte ~88-90 i.p.v. soepel af te bouwen. Oorzaak: sinds ADR 0028 is de downsize-woning spendable+saleManaged, maar ÉÉN functie liquidValue vervulde drie rollen tegelijk — FIRE-eligibility/display ÉN de deplete-annuïteitsbasis (computeAnnuityBase via currentPortfolio=liquidValue) ÉN de uitputtings-meting. Met de woning spendable telt liquidValue de hele woning (~€500k) mee → de annuïteit denkt ~€1M en onttrekt te weinig; maar mayBeRawWithdrawn sluit de woning uit van rauwe onttrekking → de échte cash loopt vóór de verkoop leeg terwijl liquideVermogen (woning groeit 1,47%) juist stijgt; de trigger (resolveDownsizeTriggerV2) mat op een ex-huis-meetrun die de getoonde run niet beleefde → kruiste de buffer pas ~16 jr te laat → de annuïteit moest de pot in 2 jr naar €0 persen → crash. Fix (engine.ts): twee nieuwe helpers naast liquidValue — withdrawableLiquidValue(assets) = Σ max(0,value) over de mayBeRawWithdrawn-set, en withdrawableRealReturn (waarde-gewogen reële return over diezelfde set; vervangt het oude liquidRealReturn dat enkel de afbouw-annuïteit voedde). De decum-ctx (currentPortfolio, startPortfolio/decumStartLiquide, yearReturn) leest nu deze withdrawable*-grondslag → de annuïteit teert op precies de pot die ze rauw kàn opnemen → geen stille shortfall meer. Elke LedgerRow draagt nu BEIDE potten: liquideVermogen (= liquidValue, eligibility/FIRE-gate — ONGEMOEID) én besteedbaarVermogen (= withdrawableLiquidValue, de RAUW besteedbare pot). De v2-trigger-meetrun (build-input.ts baseSimInput) draait nu MÉT de woning als spendable (exact de FIRE-eligibility-grondslag van de getoonde grafiek → zelfde FIRE-/stop-werk-leeftijd/afbouw) en scant besteedbaarVermogen i.p.v. liquideVermogen → de verkoop vuurt op de leeftijd waarop de getoonde besteedbare daling de verkoopkosten-buffer raakt (~71), niet ~88. De getoonde "liquide, vrij besteedbare"-lijn (views.ts buildChartSeries.vOp + lib/check/build-report.ts buildKruising.vOp + de FIRE-kruising-stip) leest nu besteedbaarVermogen → de echte besteedbare daling vóór de verkoop, niet de eligibility-pot incl. nog-niet-verkocht huis. INVARIANTEN (niet geschonden): de FIRE-gate/opbouw (liquidValue, liquidSumStart, blendedRealReturnStart, meetsStrategyTarget op liquideVermogen, liquideAtFire) blijft de eligibility-pot lezen → de FIRE-leeftijd voor downsize schuift NIET; include_full byte-identiek (een spendable-zónder-saleManaged woning zit in mayBeRawWithdrawn → besteedbaarVermogen == liquideVermogen); reverse_mortgage ongemoeid (huis in geen van beide potten, de leen-ruimte blijft een apart kanaal — geen dubbeltelling); legacy(€>0) ongemoeid (need-only, computeAnnuityBase wordt niet aangeroepen in de floorless engine-tak). Tests: test/horizon-downsize-verkopen.test.ts test (d)/(de meetrun-test) wordt legitiem her-gebaselined (oude buggy ex-huis-meetpad codificeerde de bug). Downsize-trigger accumulatie-fix + valuatie-basis (ADR 0031, jun 2026): TWEE defecten in resolveDownsizeTriggerV2/buildV2DownsizeHousing + één nieuwe config. Bug A — de verkoop-trigger viel in de ACCUMULATIEFASE (bv. 41 terwijl de getoonde run pas op ~60 FIRE bereikt): de trigger-meetrun berekende zijn EIGEN FIRE-leeftijd, maar de spendable woning bevredigt de FIRE-gate (liquideVermogen) al op currentAge terwijl de besteedbare ex-huis pot dat NIET kan dragen → de meetrun "pensioneerde" een nog-werkende gebruiker direct en kruiste de buffer al in de opbouw. Fix = pin de meetrun op een STABIELE FIRE-leeftijd, NIET via een vaste-punt-iteratie (die divergeert: bij een verkoop ver in de toekomst houdt de spendable woning liquideVermogen kunstmatig positief → de FIRE-gate claimt een onmogelijk-vroege FIRE → terugkoppeling oscilleert 41→79→83→41). In plaats daarvan: (1) HOLD-FIRE-anker = runHorizonLedger(baseInput).fireAge, de run waarin de woning spendable ÉN rauw besteedbaar is (geen saleManaged-markering) → besteedbaarVermogen == liquideVermogen → EERLIJKE gate → sale-timing-onafhankelijke, stabiele FIRE (≈ include_full); (2) ALLEEN bij een echte opbouwfase (monthlyIncome > 0 ÉN hold-FIRE > currentAge+1) één verfijning op de getoonde downsize-FIRE (huur+verkoopkosten → iets later dan het anker), geklemd ≥ hold-FIRE en ≤ endAge. Een al-gestopte gebruiker (geen salaris) slaat de verfijning over → de ruwe trigger op het hold-anker is al juist (decumulatie-personas DEP/MID/I1 ongewijzigd). Pensioen-modus (exogene forcedFireAge) wint altijd. Resultaat persona janpaul050486 (firsthand): trigger 41→79 (nu ≥ FIRE 60), include_full-FIRE ONGEWIJZIGD (51), fixed_age ONGEWIJZIGD (verkoopt op de gekozen leeftijd). Bug B — de getoonde metadata.saleProceeds was WOZ-nominaal (buildHousingLifeEventsAtAge → projectEigenHuisValuesAt, groeit nominaal + valt terug op woz_value) en week ~€171k af van de markt-reële engine-verkoop. Fix: resolveDownsizeTriggerV2 retourneert een DownsizeSaleBreakdown (grondslagValueAtTrigger = engine-huiswaarde op trigger vóór salePricePct/kosten, mortgagePayoffAtTrigger = afgelost hypotheeksaldo, saleProceeds = netto), alles basis-bewust uit DEZELFDE meetrun-rij; buildV2DownsizeHousing overschrijft daarmee metadata.saleProceeds en VERRIJKT (H2) het event-metadata met saleValuationBasis + grondslagValueAtTrigger + mortgagePayoffAtTrigger zodat de uitleg-bon (event-pane-view.tsx) consume-don’t-recompute optelt: saleProceeds == grondslagValueAtTrigger × salePricePct × (1−salesCostsPct) − mortgagePayoffAtTrigger. Preview/markers (housing-strategy-section.tsx) tonen per constructie hetzelfde bedrag als de grafiek injecteert. H1 (review): bij ONBEREIKBARE FIRE (onderwater-hypotheek/zeer hoge uitgaven → hold-FIRE = null) pint de meetrun NIET op currentAge (dat zou ’m nu laten stoppen terwijl de getoonde run zelf-zoekt en tot AOW doorwerkt → trigger weer op currentAge = Bug A-variant) maar op undefined (zelf-zoekend, identiek aan de getoonde run). M1 (review): de v2-resolve is DETERMINISTISCH (geen vaste-punt-iteratie); depletion.iterations = werkelijk aantal trigger-meetruns (1 zonder verfijning, 2 met), converged = true betekent louter "definitieve trigger bepaald" (geen convergentietest). Config — saleValuationBasis ’market’|’woz’ (optioneel in het type, default ’market’; parser+DEFAULT_DOWNSIZE_CONFIG zetten ’m altijd) bepaalt de engine-basiswaarde van het huis via applyDownsizeValuationBasis (één bron: getoonde run + meetrun + runHousingScenarioProjectionV2). Geen DB-migratie (housing_strategy_config is JSONB). Tests: test/horizon-downsize-valuation-trigger.test.ts (Bug A trigger ≥ eligibility-FIRE; Bug B exacte saleProceeds==engine binnen €1; woz kleinere huiswaarde; fixed_age-regressie; H1 FIRE-onbereikbaar trigger niet op currentAge; L1 no-salary-brugfase; H2 metadata-grondslag telt op). RESTEREND ARCHITECTUUR-PUNT (architect): de FIRE-gate gebruikt liquideVermogen (incl. de spendable saleManaged downsize-woning), waardoor de zelf-berekende downsize-graaf-FIRE bij een late verkoop een onmogelijk-vroege FIRE kan claimen — de hold-FIRE-anker-aanpak omzeilt dit, maar de onderliggende gate-grondslag (eligibility i.p.v. rauw besteedbaar) blijft een tweede-orde spanning tussen ADR 0028 (spendable woning) en de FIRE-detectie. Pre-existing buiten scope (follow-up): L2 multi-huis (ledgerHouseValueAt/engineSaleAt sommeren alle eigen_huis maar alleen [0] wordt verkocht).',
  },
  {
    ...
    note: 'Eén bron: FIRE-eligible vermogen ÷ benodigde portfolio uit runUnifiedProjection (fallback: strategie-bewust fireTarget op dezelfde grondslag). De fire-pijler van het gezondheidsgetal erft dit percentage via freedomPct, waardoor de live health-score voor huiseigenaren lager (correcter) uitvalt dan oude gepersisteerde snapshot-scores, die nog op het volledige vermogen rusten. Sinds 2026-06-12 delen óók het sovereignty-niveau ("Jouw Pad", dashboard-data-loader → computeSovereigntyLevel) en de widgets vrijheidsvoortgang/vrijheidsmijlpalen + de horizon-hero deze canonieke freedomPct (eigen perspectief: data.freedomPct; huishouden/partner: de override) i.p.v. een eigen som op vol vermogen. De FIRE-prognose-widget erft het ook; de snapshot-historie (app/api/snapshots) en de household-engine houden bewust een eigen, per-rij/per-huishouden consistente definitie. Zie ADR 0009. Downsize "Verkopen" (Fase 2, ADR 0028, jun 2026): de v2-grootboek-ENGINE behandelt de downsize-woning nu als FIRE-eligible (spendable) tijdens de opbouw — net als include_full — waardoor de engine-FIRE voor downsize-gebruikers doorgaans vervroegt (en freedomPct op de engine-grondslag omhoog schuift). OPEN CONSISTENTIE-VRAAG: de DISPLAY-helper getFireEligibleNetWorth blijft (engine-agnostisch, geen useV2-param) de overwaarde van het huis AFTREKKEN voor downsize (mode-switch: exclude_from_fire/downsize → −equity). Daardoor kunnen de dashboard-"belegbaar vermogen"- en freedomPct-oppervlakken die op deze helper rusten (dashboard/core/horizon-loader, AI shared-context, freedom-card, report) de downsize-gebruiker ÓNDER-tellen t.o.v. wat de engine als eligible ziet. Bewust NIET in deze build gewijzigd (cross-surface grondslag-besluit voor de architect/owner). DESYNC BESLECHT voor reverse_mortgage (Fase 3, ADR 0029, jun 2026): getFireEligibleNetWorth(reverse_mortgage) gaf vóórheen totalNetWorth (huis 100% mee) — fout, want het huiseigen geld is niet liquide zonder de lening. Het gebruikt nu de LEEN-RUIMTE-grondslag: totalNetWorth − overwaarde + reverseMortgageBorrowable(overwaarde, maxLoanPct), met reverseMortgageBorrowable als ENE home voor de × maxLoanPct-formule (gedeeld met de engine-eligibility via collateralBorrowableById in build-input). Alle ~8 display-consumers (dashboard/core/horizon-loader, AI shared-context, freedom-card, report) erven dit automatisch — geen tweede × maxLoanPct elders. include_full=totalNetWorth; exclude_from_fire/downsize=totalNetWorth−equity ongewijzigd.',
  },
  {
