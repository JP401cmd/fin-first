...
export const CALCULATIONS: Calculation[] = [
  ...
  },
  {
    ...
    outputs: ['currentAmount € (liquide pot)', 'targetMonths (doel of 6, begrensd op 24)', 'targetAmount €', 'monthsCovered', "source: 'goal' | 'liquid' (ook op de bundel: DashboardData.emergencyFund.source)"],
    formula: "currentAmount = computeLiquidPot(assets, unlinkedCash); targetMonths = min(24, doel.maanden of doel.bedrag/uitgaven) of 6; targetAmount = doel.bedrag of targetMonths × uitgaven; monthsCovered = currentAmount / effectiveMonthlyExpenses; score-target = min(24, max(3, targetMonths))",
    files: ['lib/emergency-fund.ts', 'lib/dashboard-data-loader.ts', 'lib/horizon-data-loader.ts', 'lib/core-data-loader.ts', 'lib/dashboard-wealth-weighting.ts', 'lib/health-score-input.ts', 'app/api/snapshots/route.ts', 'app/api/snapshots/auto/route.ts', 'app/api/snapshots/cron/route.ts', 'lib/check/build-report.ts', 'components/widgets/noodfonds-widget.tsx', 'supabase/migrations/20260729120000_backfill_noodfonds_standaarddoel_marker.sql'],
