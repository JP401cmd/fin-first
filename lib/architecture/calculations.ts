...
export const CALCULATIONS: Calculation[] = [
  ...
  },
  {
    ...
    functions: ['computeHealthScoreFromInputs', 'computeHealthScoreWithTrend', 'buildHealthScoreInput', 'scoreDSTI', 'scoreAssetConcentration', 'computeLargestAssetTypeShare'],
    constants: [
  ...
  },
  {
    ...
    title: 'Vermogensprojectie — v1 (niet-FIRE-rekenbibliotheek)',
    domain: 'Toekomst (FIRE)',
    summary: 'Nominale jaar-voor-jaar vermogensprojectie. Sinds de v2-cutover (ADR 0016) NIET langer de FIRE-bron — FIRE draait op de grootboek-engine v2. v1 blijft als rekenbibliotheek voor niet-FIRE-doelen.',
    inputs: ['netto vermogen', 'sparen (spaarquote × inkomen)', 'life_events', 'FIRE-parameters'],
    outputs: ['nominaal vermogenspad', 'jaar-projectierijen'],
    formula: 'jaarlijks: vermogen × (1+rendement) + sparen − onttrekking',
    files: ['lib/unified-projection.ts', 'lib/horizon-data.ts', 'lib/fire-simulation.ts'],
    functions: ['runUnifiedProjection', 'runSimulation', 'lifeEventsToCashflows'],
    elementIds: ['as-planning', 'fn-toekomstplannen'],
    note: 'NIET-FIRE-utility (cutover C4, ADR 0016): de FIRE-cijfers komen nu uitsluitend uit de grootboek-engine v2 (zie calc "FIRE — grootboek-engine (v2)"). runUnifiedProjection/runSimulation blijven bewust bestaan voor: hypotheek-vs-beleggen (lib/hypotheek-vs-beleggen.ts), de housing-trigger-meetrun (lib/housing-trigger.ts) en de accumulation-only Kern-prognose + de snapshot-/fallback-schrijvers (computeFireProjection). C5-b (engine 2/4): household-projection (lib/household-projection.ts) is gemigreerd — partner- én gecombineerde projectie (itemized én totalen via buildTotalsInput-synthetisch-portfolio) lopen nu flag-bewust via runSelectedProjection(input, useV2); useV2 = de eigen flag van de viewer (isHorizonV2Enabled op het eigen profiel, één boolean voor combined + beide partnerlijnen). C5-b (engine 3/4): fee-analyse (lib/fee-analysis.ts, computeFeeImpactOnFire) is gemigreerd — beide fee-runs (met/zonder fee = return-reductie) lopen flag-bewust via runSelectedProjection op een lokale synthetisch-portfolio-bridge (buildFeeSimInput); useV2 = de eigen flag van de viewer (dashboard-data-loader). Flag-uit = byte-identiek aan de legacy runSimulation-fee-drag. C5-b (engine 4/4): hypotheek-vs-beleggen (lib/hypotheek-vs-beleggen.ts, compareMortgageVsInvest → computeFireImpact) is gemigreerd — de twee FIRE-impact-runs (extra aflossen vs. extra beleggen, alléén verschillend in annualSavings) lopen flag-bewust via runSelectedProjection op de lokale bridge buildHvBSimInput; useV2 = de eigen flag van de viewer (dashboard-data-loader). Flag-uit = byte-identiek aan de directe legacy runSimulation. De aflossing- en beleggen-uitkomsten + breakeven draaien op deterministische amortisatie- en groeischemas (geen FIRE-engine) en zijn flag-onafhankelijk; de flag raakt enkel fireImpactMaanden. NB: de twee UI-callers (hypotheek-vs-beleggen-opbouw + -modal/sectie) hebben nog geen flag in scope en blijven voorlopig v1 (deferred, eigen werkpakket — als strategie-modal.tsx in C5-pre). Geen FIRE-drift meer: alle FIRE-oppervlakken lopen via runSelectedProjection(flag). Verwijderen (C5) is een aparte gated migratie zodra ook de resterende niet-FIRE-consumenten zijn omgezet of bewust buiten scope blijven.',
  },
  {
    ...
    elementIds: ['as-planning', 'fn-toekomstplannen'],
    note: 'Architectuur (invarianten + uitbreidingsregels): docs/architecture/horizon-engine-v2.md. Plan: docs/horizon-tabel-rekenmotor-plan.md. ADR 0013 + 0014 + 0015. Inspecteerbaar op /beheer/horizon-tabellen. Single source of truth = LedgerRow[] (per asset/schuld); intern reëel, de adapter is het ENIGE reëel→nominaal-punt; FIRE = forward doel-zoektocht (geen crossing); strategieën zijn pure plug-ins. Eindstrategie-onttrekking: deplete = spend-down-annuïteit (→ ~€0); legacy = need-only (residu groeit naar de nalatenschap, via ctx.legacyPreserveOnly in withdrawal-strategy.ts — anders verdampt het surplus in het grootboek; ADR 0014); perpetual/pensioen = need-only. Eigen-huis-downsize (v2): huis blijft niet-liquide asset in het grootboek, verkoop = asset-liquidatie op de trigger (UnifiedProjectionInput.assetLiquidations) i.p.v. filteren + inkomen — netto vermogen continu (alleen −verkoopkosten), liquiditeit verspringt; trigger op v2-liquide via buildHorizonInput (ADR 0015). Feature-flag profiles.feature_preferences.horizon_engine_v2 (isHorizonV2Enabled) — **default AAN sinds cutover C3 (ADR 0016); een expliciete `false` is de opt-out**. FIRE-consumenten kiezen via runSelectedProjection. C4 voltooid (ADR 0016 criterium 7 MET): /core + AI-context + freedomPct/gezondheidsscore/sovereignty lezen nu v2 via computeHorizonFireTarget (fire-target-shared.ts) → buildHorizonInput + runSelectedProjection(flag); ook what-if, regel-sim (Voorkeuren-editors) en de AOW/Pensioen-previews (strategy-preview.ts, nu op de volledige UnifiedProjectionInput) draaien flag-bewust. v1 (runUnifiedProjection/runSimulation) blijft als niet-FIRE-rekenbibliotheek (hypotheek-vs-beleggen, housing-trigger-meetrun, Kern-accumulatie). C5-b migreert de niet-FIRE-engines naar flag-bewust: household-projection (engine 2) én fee-analyse (engine 3, computeFeeImpactOnFire — fee = return-reductie, beide runs op de geselecteerde engine via buildFeeSimInput; flag-off = byte-identiek aan de legacy runSimulation-fee-drag) lopen nu via runSelectedProjection met de gebruikersflag. C5 (runUnifiedProjection/runSimulation fysiek verwijderen) bewust aparte gated migratie. Productie-default flippen + legacy verwijderen is een aparte gated stap ná review van de parity-diffs (compareEngines). B2-parity (13 jun, na de fixes legacy-need-only/recurring-×12/housing-liquidatie): v2 ligt voor spend-down-strategieën later (persona deplete +7 jr, legacy +6 jr) door reëel-vs-nominaal, en gelijk voor behoud (perpetual +1, pensioen 0); eindnetto verschilt navenant. Owner legacy €200k: v1 onbereikbaar, v2 ~74 (haalbaar, eindigt ≥ doel).',
  },
  {
