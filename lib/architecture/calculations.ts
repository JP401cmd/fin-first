...
export const CALCULATIONS: Calculation[] = [
  ...
  },
  {
    ...
    note: 'NIET-FIRE-utility (cutover C4, ADR 0016): de FIRE-cijfers komen nu uitsluitend uit de grootboek-engine v2 (zie calc "FIRE — grootboek-engine (v2)"). runUnifiedProjection/runSimulation blijven bewust bestaan voor: fee-analyse (lib/fee-analysis.ts), hypotheek-vs-beleggen (lib/hypotheek-vs-beleggen.ts), household-projection (lib/household-projection.ts), de housing-trigger-meetrun (lib/housing-trigger.ts) en de accumulation-only Kern-prognose + de snapshot-/fallback-schrijvers (computeFireProjection). Geen FIRE-drift meer: alle FIRE-oppervlakken lopen via runSelectedProjection(flag). Verwijderen (C5) is een aparte gated migratie zodra ook deze niet-FIRE-consumenten zijn omgezet of bewust buiten scope blijven.',
  },
  {
