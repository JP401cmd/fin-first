...
export const CALCULATIONS: Calculation[] = [
  ...
  },
  {
    ...
    inputs: [
      ...
      'rows.box1Streams (kernel-bridge, spread-gated weergaveveld): aowNetto, pensioenUitkeringBruto, pensioenOnttrekkingBruto, partnerBatenNetto (gelezen maar bewust nooit verwerkt, ADR 0036)',
      'AOW-leeftijd (HorizonFireSim.aowAgeFractional — enige bron voor de per-rij AOW-vlag)',
    ...
    note: 'ADR 0088 legt de drie besluiten vast die ADR 0040 nog open liet: Box 1 als rapportagelaag NÁÁST de kernel (geen terugkoppeling — zie aandachtspunt box1-buiten-kernel-cashflow), sturen via categorie_prios-overlays i.p.v. WITHDRAWAL_ORDER_PRESETS (die klemmen op Math.min(i+1,4) en zouden "pensioen laatst" en "pensioen-sparen" een IDENTIEKE prio-vector geven — vastgelegd in varianten-sweep.test.ts als "de V1-val"), en scope versmald van "laagste levenslange Box 3" naar de positie van de pensioenpot (alleen die categorie heeft een Box 1-gevolg; spaargeld/beleggingen verplaatsen alleen Box 3, dat dekt katern II van de optimizer al). BEKENDE, BEWUST GEACCEPTEERDE AFWIJKING: computeBox1Tax berekent de arbeidskorting over het volledige grossYearlyIncome, dus óók over AOW/pensioen — fiscaal geen arbeidsinkomen, dus de heffing valt systematisch te LAAG uit (deze reeks is conservatief, onderschat de last). Gemeld en gepind in lib/tax-lifetime/lifetime-tax.test.ts ("BEKENDE AFWIJKING"); de correctie hoort in lib/box1-tax.ts (arbeidsinkomen als eigen invoerveld), niet in deze rapportagelaag — een eigen tariefvariant hier zou een tweede tariefimplementatie zijn. Draait client-side in de bestaande kernel-worker (kind: "taxvarianten", ADR 0054) achter een expliciete gebruikersactie (drie kernel-solves kosten ~466-688 ms); de worker-module is lazy geïmporteerd zodat de kernel-bundel niet in de eerste load van de optimizer-pagina zit, en component-scoped gememoïseerd op variantenSweepInputHash (bewust géén module-level cache — die zou na een accountwissel binnen dezelfde tab de vorige gebruiker kunnen tonen). De server (loadVariantenSweepSnapshot, GET /api/belasting/varianten-sweep) levert uitsluitend de serialiseerbare INVOER — er draait daar geen sweep; computeHorizonFireSim is React-cache()\'d, dus op een oppervlak dat de Horizon-run al deed is de snapshot gratis, op de optimizer-pagina (die loadHorizonData gebruikt, niet de kernel-run) kost hij één horizon-load + één kernel-solve, wat de reden is voor een eigen route i.p.v. aan de page-render hangen (anders zou élke bezoeker die extra kosten dragen voor een sectie die pas op verzoek rekent). Eindvermogen toont bewust ÉÉN grondslag (eindvermogenBelegbaarNominaal, dezelfde als laagsteBuffer, expliciet gelabeld "zonder je huis en ander niet-liquide bezit") — het netto eindvermogen incl. woning is een andere grootheid en staat er nooit naast op dezelfde as (CLAUDE.md). Het vierde optimizer-doel (lib/tax-optimizer/goals.ts#levenslang-minimaal) staat bewust nog op available:false — deze calc leeft als eigen, altijd-zichtbare vierde katern buiten de doel-selector/pickBest-ranking van de calc "Fiscale-strategie-optimizer", niet als vijfde strategie daarin. Wft: geen "je bespaart €X" (het is een verschil tussen twee indicaties), geen gebiedende wijs, geen aanbevolen volgorde — vijf kanttekeningen op het scherm zelf benoemen de aannames (geen bruto-ophoging voor de heffing, gewogen i.p.v. strikte trekking, lijfrente niet apart stuurbaar, partner uitgesloten, schijven van het lopende jaar). Vergrendeld in lib/tax-lifetime/lifetime-tax.test.ts (waaronder de arbeidskorting-afwijking) en lib/tax-lifetime/varianten-sweep.test.ts (waaronder de V1-val, de twee vetorechten en de winnaar-tiebreak).',
  },
  {
    id: 'tax-overview',
