...
export const CALCULATIONS: Calculation[] = [
  ...
  // ── Toekomst (FIRE) ──
  {
    ...
    summary: 'Lost de rendements- en inflatie-aannames op uit het profiel; levert de effectieve veilige onttrekkingsvoet en het marginale IB-tarief.',
    inputs: ['profiles (expected_return, inflation_rate, marginaal_tarief, net_monthly_income)'],
    outputs: ['grossReturn', 'inflationRate', 'effectiveSwr', 'marginaalTarief (jaar-afgeleid uit BOX1_PARAMS)'],
    files: ['lib/fire-params.ts', 'lib/box1-tax.ts'],
    functions: ['resolveFireParams', 'computeEffectiveSwr', 'deriveMarginaalTarief', 'schijfGrensVoor'],
    constants: [
    ...
    note: 'De SWR-formule heeft één home: computeEffectiveSwr (lib/fire-params.ts). Components/widgets dupliceren de formule niet en hardcoden geen 0.04. Het marginale IB-tarief (fireParams.marginaalTarief) is per Arch F1 niet langer een 2024-hardcode (0,3697/0,4950 + schijfgrens €75.518) maar wordt per belastingjaar afgeleid uit BOX1_PARAMS via deriveMarginaalTarief (lib/box1-tax.ts): laag = schijf-1-tarief, hoog = topschijf, drempel = netto-maandinkomen-vuistregel; schijfGrensVoor vervangt IB_SCHIJFGRENS. fire-params delegeert; de /api/parameters-override valideert nu op range i.p.v. een exacte whitelist. Beweegt zo automatisch mee met de jaarlijks vernieuwde beheer-parameters (fiscale-kerngetallen ← BOX1_PARAMS).',
  },
  {
  ...
  },
  {
    ...
    files: ['lib/core-metrics.ts', 'lib/housing-strategy.ts', 'lib/dashboard-data-loader.ts', 'lib/horizon-data-loader.ts', 'lib/core-data-loader.ts', 'lib/ai/context/shared-context.ts', 'lib/ai/context/will-financial-facts.ts', 'lib/ai/local/local-chat-context.ts', 'app/api/share/freedom-card/route.ts', 'app/api/report/route.ts', 'components/app/horizon/horizon-client.tsx'],
