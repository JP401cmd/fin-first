...
export const CALCULATIONS: Calculation[] = [
  ...
  },
  {
    ...
    outputs: ['KernelProjection (maandtabellen A–PT: Bel/CF/Ont/Af/Toename-afname/Verdeling/Bez/S/Prognose/ES/Auto-gebeurtenissen/Geb/PT/Werk-strategie)', 'SolveFireResult (fireAge fractioneel + status + €/mnd-hint)', 'UnifiedProjectionResult (via de bridge — het bestaande consumer-contract, ongewijzigd voor de UI)'],
    formula:
      'Per maand (nominaal, één-maand-lag op saldi van m−1): rendement/pot → Box 3-drag via cashflow → schuldschema (incl. woningblok AY:BE: verkoop-/opeethypotheek-mechaniek) → kasstromen uit de domein-expanders → capaciteit-waterval (prioriteit-gewichten ½^(prio−1) + reserve, tekort → tekort-lening) → onttrekking volgens het profiel. FIRE = maand-bisectie (BepaalFIRE-port): pensioen-eindstrategie kortsluit op de AOW-/pensioenleeftijd (geen bisectie); anders zoekt de bisectie de kleinste maand waar het statusblok (B35 eindleeftijd → B36 doelbedrag → B37 modelwaarde (Prognose!I netto-vermogen bij legacy-nalatenschap, Prognose!J netto-liquide anders) → B38 gap = B37−B36) niet-negatief is.',
  ...
  },
  {
    ...
    note: 'Eén bron: FIRE-eligible vermogen ÷ benodigde portfolio (requiredFirePortfolio uit het UnifiedProjectionResult-contract, sinds FASE 6 stap 5A geleverd door de horizon-kernel via lib/horizon-kernel/bridge.ts — fallback: strategie-bewust fireTarget op dezelfde grondslag). De fire-pijler van het gezondheidsgetal erft dit percentage via freedomPct, waardoor de live health-score voor huiseigenaren lager (correcter) uitvalt dan oude gepersisteerde snapshot-scores, die nog op het volledige vermogen rusten. Sinds 2026-06-12 delen óók het sovereignty-niveau ("Jouw Pad", dashboard-data-loader → computeSovereigntyLevel) en de widgets vrijheidsvoortgang/vrijheidsmijlpalen + de horizon-hero deze canonieke freedomPct (eigen perspectief: data.freedomPct; huishouden/partner: de override) i.p.v. een eigen som op vol vermogen. De FIRE-prognose-widget erft het ook; de snapshot-historie (app/api/snapshots) en de household-engine houden bewust een eigen, per-rij/per-huishouden consistente definitie. Zie ADR 0009. Downsize "Verkopen" — DESYNC BESLECHT door de kernel-migratie (ADR 0032, FASE 6): de verwijderde v2-grootboek-engine behandelde de downsize-woning tijdens de opbouw al als FIRE-eligible ("spendable", ADR 0028), terwijl de DISPLAY-helper getFireEligibleNetWorth (engine-agnostisch) de overwaarde vóór verkoop juist bleef AFTREKKEN (mode-switch exclude_from_fire/downsize → −equity) — dat was de openstaande cross-surface-inconsistentie. De horizon-kernel behandelt het huis bij ELKE housing-modus (dus ook "Verkopen") als niet-liquide TOTDAT de daadwerkelijke verkoopmaand valt (zie calc "Eigen-huis-strategie — verkoop-/opeethypotheek-trigger") — dat is nu HETZELFDE moment als waarop getFireEligibleNetWorth in de mode-switch al aannam. Engine en display zijn dus weer congruent voor downsize/exclude_from_fire (zie de ADR 0028-addendum). reverse_mortgage blijft ongewijzigd op de leen-ruimte-grondslag (ADR 0029): getFireEligibleNetWorth(reverse_mortgage) = totalNetWorth − overwaarde + reverseMortgageBorrowable(overwaarde, maxLoanPct), met reverseMortgageBorrowable als ENE home voor de × maxLoanPct-formule. Alle ~8 display-consumers (dashboard/core/horizon-loader, AI shared-context, freedom-card, report) erven dit automatisch — geen tweede × maxLoanPct elders. include_full=totalNetWorth; exclude_from_fire/downsize=totalNetWorth−equity.',
  },
  {
