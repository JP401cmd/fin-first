...
export function buildArchimateModel(facts: ArchFacts): ArchimateModel {
  ...
  const ENRICH: Record<string, Enrichment> = {
    ...
    't-lokale-ai->app-comp': { payload: 'Lokale categorisatie-voorstellen en chat-antwoorden — de inferentie blijft on-device; alleen een door de gebruiker bevestigd actie-label (géén cijfers) kruist terug naar de eigen server (C2c)', mechanism: 'compute', cadence: 'on-demand' },
