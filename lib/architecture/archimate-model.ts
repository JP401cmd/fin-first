...
export function buildArchimateModel(facts: ArchFacts): ArchimateModel {
  ...
  const nodes: ArchiNode[] = [
    ...
    },
    {
      ...
      lead: 'Profielen, vermogens-snapshots (dag-cadans), de tijdstempel-vermogenshistorie per sync en de actieve dagen per gebruiker (gebruik zonder inhoud, ADR 0146). Sinds ADR 0147 ook de gebruikte app-delen per dag (twaalf vaste modules sinds ADR 0153, geen route of tijdstip — het vijfde app-deel "grip" (tips, fiscale kansen, vergelijking) kwam er 17 sep 2026 bij), de uitnodigingen en promptstaat voor vragenlijsten, en de door beheer gemaakte gebruikersgroepen met hun leden. Waardestromen zijn een globale config-rij in app_settings; ADR 0153 voegt daar /beheer/gebruik aan toe — één security-definer RPC (`admin_gebruik_analyse`) die uitsluitend k=5-onderdrukte tellingen per waardestroom teruggeeft, berekend in de database, nooit een gebruikers-id of tijdstip. ADR 0154 (voorgesteld, nog niet gebouwd) zou daar per-bezoek schermovergangen en gebeurtenissen aan toevoegen — als aparte tellertabellen, nooit gekoppeld aan user_activity_days/_modules.',
      items: ['profiles', 'net_worth_snapshots', 'net_worth_history', 'app_settings', 'user_activity_days', 'user_activity_modules', 'questionnaire_invitations', 'user_groups', 'user_group_members'],
    },
    {
  ...
  const ENRICH: Record<string, Enrichment> = {
    ...
    't-bankconnect->app-comp': { payload: 'Geautoriseerde saldi en transacties (PSD2); vooraf gekozen doelrekening (of "nieuw") reist mee als link-intentie op de pending koppeling, niet in de OAuth-state', mechanism: 'rest', cadence: 'on-demand', contractDomains: ['bank-connect'], note: 'De OAuth-callback legt de koppeling en haalt alleen SALDO op — transacties uitsluitend via de sync-route (ADR 0069). Die sync-route heeft twee aanroepers: de knop op de rekeningdetail (expliciet, ongeremd) en sinds aug 2026 de globale sync-knop, die elke actieve koppeling meeneemt maar ten hoogste één keer per uur per rekening en met drie verzoeken reserve op de dagrem (10/dag/rekening, atomair via public.reserve_bank_sync_slot). Er is géén bank-cron — het pad is dus on-demand, niet dagelijks. Precies daardoor is een verkeerd gelande koppeling tot de eerste sync te verhangen via het correctiemoment op de success-pagina. Welke TriFinity-rekening de data draagt volgt uit een vaste precedentieketen: external_account_id (identiteit) → de expliciete keuze → iban_hash → nieuwe rekening. Het rekeningtype komt van de provider, niet uit een aanname (besluit B3) — dat raakt de vermogensgrondslag via is_liquid.' },
    't-bankimport->app-comp': { payload: 'Geparste MT940/CSV/OFX-boekingen; laag 1 (import_hash|bank_seq) plus laag 2 cross-bron-dedup (datum ±1 dag, bedrag exact, tegenpartij-IBAN of genormaliseerde naam)', mechanism: 'import', cadence: 'on-demand', contractDomains: ['transactions'], note: 'Een gekoppelde rekening importeert verplicht via POST /api/transactions/import (ADR 0074, B7): parsen blijft client-side, opslaan + dedup draait server-side met dezelfde module als de sync-route — één schrijver per gekoppelde rekening. Losse rekeningen houden het clientpad.' },
