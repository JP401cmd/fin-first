...
| # | Voorstel | Cat |
|---|---|---|
| VL-1 ★ | **Nieuw 28 aug 2026 (S2).** Eenvoudig = oordeel vóór lijst: oordeelregel (feit + Nibud-norm, stoplichtwoord uit `LEVERAGE_STATUS_LABEL`) i.p.v. de compacte meter, dan quote-meter + sluipverbruik + top-5 grootste posten; de volle lijst achter DepthSection "Alle {n} posten". Volledig ongewijzigd. Copy-rollen gescheiden: deck = feit, `PageStatusBanner` = handeling. **Herzien 19 sep 2026 (W-017, eigenaarsbesluit op testmelding).** De lijst zelf is wat je hier komt bekijken: in Eenvoudig staat de volle lijst nu DIRECT onder de ene oordeelregel (geen DepthSection), daarna quote-meter + sluipverbruik; de top-5 was daarmee een duplicaat en is vervallen. De S2-les blijft in de oordeelregel-vóór-de-lijst; wat wegvalt is "lijst achter een klik". Tegelijk in béide modi verwijderd: "In vrijheidstijd" (dubbelde het onderschrift van het hoofdcijfer), "Wat als ik opzeg" en de cashflow-kalender (blijft via de Agenda-widget). Volledig deelt nu dezelfde ruggengraat, plus de samenstelling | B |
...
| BEL-9 | *(nagekomen, UR3-29 · 7 sep 2026)* **Één gevolg-zin per box in Eenvoudig.** De belastingpagina’s scoorden bij de beginner het laagst van alle routegroepen: te vlak à la "hier zijn drie boxen", terwijl de één zin die wél werkte ("elke € 1.000 kost € 21") in `box3-detail.tsx` juist ACHTER `HideInSimple` stond — terwijl 3.1, de forfaitaire opbouw-staaf (het rekenmodel), er buiten stond. De beginner kreeg dus het model en niet het gevolg. Nu per box één zin via `SwapInSimple`, Volledig byte-identiek: **box 1** vervangt de figures-strip door "van je bruto inkomen gaat ongeveer N% naar Box 1 — je houdt € X per jaar over" (`box1-kost-zin.tsx`, `simpleFigures` daarmee weg); **box 2** krijgt de staffel als zin onder het hero-bedrag (`box2-schijf-zin.tsx`, harde null-pad op `dividendOnbekend`, H26); **box 3** toont 3.3 als zin zonder gauge (`sentenceOnly`), met de `voetVolBenut`-gate intact — de marginale claim is onwaar voor wie onder de vrijstelling zit. De verdieping blijft waar hij stond: de `i`-knop + de modus Volledig (keuze A1; `DepthSection` viel af omdat dat het afgewezen APP-4 zou heropenen). Geen rekenwijziging — elke zin consumeert `computeBox1Tax`/`calculateBox2`/`calculateBox3`; bewaakt door bron-asserties in `box1-kost-zin.test.tsx`, `box2-schijf-zin.test.tsx` en `box3-heffingsvrij.eenvoudig.test.tsx`. | B |
