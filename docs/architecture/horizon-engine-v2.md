...
Dit respecteert INV-4 (asset-level interventie als data op de input + verwerking in de pure jaar-loop, geen bespoke som buiten het grootboek). Scope = downsize; reverse_mortgage/include/exclude houden voorlopig het v1-model. *Bewaakt door* `test/horizon-housing-liquidation.test.ts`.
