...
13. **Onboarding blijft 11 stappen** — bewust één vraag per scherm, geen actie (A-11).
    > **Herzien op 26 aug 2026 (H13, opdrachtgever) — alleen voor de schulden-sectie.** Het onafhankelijke UX-testpanel telde acht opeenvolgende schermen in sectie 4 ("zeven keer Nee klikken voelt als een ondervraging"). Gekozen is **optie C (hybride)**: de gerichte ja/nee-vragen blijven voor de meest voorkomende schuldsoorten (hypotheek, studielening, persoonlijke lening, autolening), de staart van de catalogus is samengevoegd tot **één aanvinkraster** waarna de wizard één keer per aangevinkt type opent. Resultaat: 4-5 schermen i.p.v. 8. Tegelijk zijn drie defecten hersteld: de duplicaat-vraag (doorlopend krediet én roodstand leverden allebei `revolving_credit` op — `DebtQuickInput` draagt geen `subtype`, dus de roodstand-vraag kon geen enkel record beïnvloeden), de asymmetrische overslag (alleen de hypotheek werd overgeslagen, niet de autolening/DGA-schuld uit `LINKED_DEBT_SUGGESTIONS`) en een A-09-residu (raakzones van 36px op de drempelloze sectie-uitgang, "Annuleren" en de verwijder-knoppen → nu ≥44px). **Besluit 13 blijft gelden voor de overige secties**: één vraag per scherm daar, geen actie. De aangevinkte types worden bewust **niet** gepersisteerd — dat zou de veiligheidskeuze van 3 jul 2026 (gevoelige onboarding-invoer niet opslaan) stilzwijgend omkeren.

    > **Opnieuw herzien op 19 sep 2026 (B-054, opdrachtgever) — H13 vervangen door raster-first (= optie B van destijds), ADR 0164.** Een testgebruiker vroeg de schulden in één keer uit te vragen én meerdere schulden per soort te kunnen opgeven. De vier ja/nee-kopvragen zijn geschrapt; de sectie opent direct op het aanvinkraster met de volledige catalogus, de vier meest voorkomende soorten vooraan onder "Meest voorkomend", al gekoppelde soorten (hypotheek bij je woning, autolening bij je voertuig, RC bij je BV) uitgeschakeld mét herkomst, en ná elke toevoeging "Nog een …?" in de collect-queue. Resultaat: 1 scherm bij "geen schulden" i.p.v. 5. De prijs (volledigheid: elf tegels scannen) is gemitigeerd met de groepskop, de uitgeschakelde gekoppelde soorten en de scherpe uitgang "Ik heb geen schulden". Het herhaalbare blok is de bestaande QuickAddWizard, nooit een eigen inline formulier. Besluit 13 blijft gelden voor de bezittingen-sectie.
