...
4. **Geen kapotte afgeleide metrics** — de consumenten van de FIRE-output (`computeFreedomProgress`/freedomPct, gezondheidsscore, sovereignty, horizon-hero, `net_worth_snapshots`, briefing, AI-context) blijven correct met v2's `requiredFirePortfolio`/`fireAge`. *Status: ✓ geverifieerd (13 jun, Fase D). Sterker nog: de cutover **corrigeert** een live v1-bug. `runUnifiedProjection.requiredAt` rekent voor perpetual met `verifyEndAge = candidateFireAge + 100` (≈ leeftijd 189) en crediteert de **levenslange** AOW/pensioen (`toAge: null`) over dat hele venster → het benodigd vermogen klapt in (gemeten op één en hetzelfde profiel: ruim een factor zes lager bij levenslange AOW dan bij AOW-tot-90; exacte bedragen buiten git, zie ADR 0111). Daardoor is v1's freedomPct (= FIRE-eligible vermogen ÷ requiredFirePortfolio) voor perpetual+AOW-gebruikers — de gangbare pensioensituatie — structureel te hoog. v2 (backward `V_nodig` tot de échte eindleeftijd; AOW 67→90) geeft een plausibel benodigd vermogen in dezelfde orde als de AOW-tot-90-variant → freedomPct wordt realistischer (meestal lager). De verschuiving bij de flip is dus een correctie, geen regressie. v1 wordt niet apart gefixt (gemoot door C5).*
