...
- De cutover (Fase C: C1 consumenten op v2 + /core-divergentie ✓, C2 afgeleide metrics ✓, C3 default flippen ✓, C4 FIRE-oppervlakken op de flag-selector ✓ [fire-target-shared, what-if, regel-sim, strategy-preview], C5 legacy fysiek verwijderen — bewust aparte gated migratie) blijft **onomkeerbaar en gated**. v1 (`runUnifiedProjection`/`runSimulation`) blijft voorlopig bestaan als niet-FIRE-rekenbibliotheek (fee-analyse, hypotheek-vs-beleggen, household-projection, housing-trigger-meetrun, Kern-accumulatie).
