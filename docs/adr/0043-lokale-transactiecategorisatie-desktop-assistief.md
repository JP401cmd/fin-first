...
- **Eén manier om te categoriseren blijft geborgd.** De lokale resolver implementeert exact het bestaande `aiResolver`-contract (`(batch) => Promise<CombinedAiResult[]>`) en hergebruikt `buildCategorizeSystemPrompt` + de `resolveSlug`-slugvalidatie — geen tweede promptvariant, geen parallelle validatie. `runCombinedCategorization` blijft ongewijzigd; de resolver is het enige omschakelpunt.
