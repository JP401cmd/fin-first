...
| `assets.has_woonbalans_tracking`, `has_rental_tracking`, `monthly_maintenance_cost`, `vva_fee`, `vacancy_log` | Opgevraagd door `ASSET_CLIENT_COLUMNS` in `lib/asset-data.ts` en gebruikt door migratie `20260802190000`. Zonder deze kolommen faalt **elke** bezittingen-query met `column assets.has_rental_tracking does not exist`. Het symptoom is verraderlijk: geen foutmelding, maar overal €0 — netto vermogen wordt gelijk aan minus je schulden, en widgets zeggen "voeg je eerste bezitting toe" terwijl de tabel vol staat |
| Unieke constraint op `net_worth_snapshots (user_id, snapshot_date)` | `app/api/snapshots/auto` doet `upsert(..., onConflict: 'user_id,snapshot_date')`, maar `20260504000001` legt daar alleen een **gewone** index. Zonder unieke index geeft die route 500 `no unique or exclusion constraint matching the ON CONFLICT specification`. **Of productie deze constraint out-of-band heeft, is uit de repo niet af te leiden** — het is dus óf een zesde geval van drift, óf een live bug. Dat je dat niet kunt zien zonder productie te bevragen, is precies het probleem |
