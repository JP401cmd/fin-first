...
3. **WhatIf-baseline** — ✅ **gedaan**: [`buildBaselineOverrides`](../lib/whatif-overrides.ts) accepteert `effectiveSavingsRatePct` (heette tot 19 sep 2026 `savingsRate6m`); /toekomst geeft het server-getal door (`healthScoreInput.effectiveSavingsRatePct`). De standalone WhatIf-pagina bestaat niet meer (ADR 0144, 14 sep 2026 — het lab op de tijdas is de enige wat-als-verkenning), dus `buildBaselineOverrides` heeft nog één productie-aanroeper (horizon-client).
