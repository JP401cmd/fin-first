...
| **E** | Server-side foutlogging bedraad | Grep `logError`-callers | ✅ | **Bijgewerkt 2026-09-26.** Afgevangen 500's komen nu op `/beheer/errors`. `serverError()` (`lib/api/respond.ts`, gebruikt in ~213 route-bestanden) schrijft via `after()` fire-and-forget naar `error_logs`, via `captureServerError` → `lib/log-error.ts#logError`. Er is geen tweede pad, en de response wacht nooit en faalt nooit op de log. Opgeslagen: tag, foutklasse, pg-code, status en een gemaskeerde melding (max 300 tekens; `details`/`hint` nooit) plus alleen de stack-frames. Geen user_id, url of body. Throttle: 20 rijen per tag per minuut per instantie. Onafgevangen fouten liepen al via `onRequestError` (`lib/observability/request-error.ts`). De retentie-cron dekt `error_logs` (12 mnd). Security-gate GO; zijn 🟡 is in dezelfde wijziging opgelost: `/api/web-vitals` weigert nu controletekens, anders was een uitgelogde NUL-payload een herhaalbare schrijfroute. Client-side capture via `/api/log-error` is ongewijzigd. *Oude stand (juli): `logError` had nul callers.* |
...
- ~~**[P1] Server-side foutlogging bedraden**~~: gedaan op 26 sep 2026. `serverError()` schrijft gemaskeerd naar `error_logs` (zie rij E).
