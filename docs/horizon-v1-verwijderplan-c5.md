...
- **C5-a — ✅ gepusht (63d975d90):** `isHorizonV2Enabled` retourneert altijd true → v2 geforceerd in productie; toggle-UI + GET/PUT-route verwijderd. De selector houdt beide armen (parity gebruikt de v1-arm via `false`).
- **C5-c (engine-deletie) — GEBLOKKEERD door de parity-keuze (D3):** `runUnifiedProjection`/`runSimulation` blijven nodig zolang `compareEngines` + de ledger-API + `/beheer/horizon-tabellen` bestaan. v1 leeft dus voort **uitsluitend** voor de parity/compare-tooling (+ de uitgestelde UI-surfaces). Volledige fysieke verwijdering vergt eerst het schrappen van de parity-tooling (D3 herzien).
- **Uitgesteld (eigen werkpakketten, default v1 = veilig):** `strategie-modal.tsx` + de twee hypotheek-vs-beleggen UI-callers (geen flag in scope); de nu-dode flag-off-armen in de vier engines (onbereikbaar in productie, onschadelijk); `runSimulationUnified` (live consumenten: scenario-overlays + 4 regressie-suites + parity-test).
- **Doc/curatie nog te syncen:** ADR 0016 (§8 "toggle per gebruiker" achterhaald), concern `horizon-engine-v2-duaal`, `calculations.ts`-noten, `horizon-engine-v2.md` — via `architecture-docs-keeper`, zodra de parallelle sessie de tree met rust laat.
