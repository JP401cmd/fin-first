...
| V21 | Onmogelijke FIRE-uitkomsten uit het statusblok (bevinding M6, UX-testpanel 24 aug 2026). Bij een structureel tekort-profiel toonde /toekomst "VRIJHEIDSLEEFTIJD 100,0 jaar" en "DOELBEDRAG €−11.328.971 benodigd" als gewoon resultaat. Mechanisme: **B93** toetst `Prognose!J(0) ≥ B36` vóór `B38 < 0`. Bij B36 = 0 (deplete/pensioen/legacy-0) is die toets triviaal waar en MASKEERT hij de horizon-parkeerstand (B16 = 100, gap < 0); bij eindstrategie **perpetual** wordt B36 zelfs negatief (J@FIRE < 0) — de gap op fireAge = 100 is dan per constructie exact 0, dus de horizon-check grijpt niet in en de bisectie parkeert op 100. `bridge.ts` leidt uit `status !== 'unreachable_within_horizon'` vervolgens `fireReachable = true` + `fireAgeFractional = 100` af, en `requiredFirePortfolio` (= J@FIRE) is dan het negatieve bedrag. | **Besluit eigenaar (26-08-2026): BEIDE — kernel-fix én UI-vangrails.** Kernel: optionele, inert-by-default `KernelInput.reachedNowVereistBereikbaarDoel` — `reached_now` mag niet vallen op B36 < 0 of B38 < 0 ⇒ `unreachable_within_horizon`. **App-pad AAN** (adapter), **parity-/fixture-pad UIT** (`input-from-fixture` zet 'm níet → alle fixtures byte-groen tegen Excel v5, géén herijking). De bridge markeert daarnaast de eind-horizon-terugval expliciet (`requiredFireIsEndOfHorizonFallback`) i.p.v. de geprojecteerde eindstand stil als "benodigd vermogen" door te geven. Weergave (tweede linie, `lib/horizon/outcome-guard.ts`): leeftijd ≥ horizonplafond of doelbedrag ≤ 0/terugval ⇒ "We missen gegevens" i.p.v. een getal, op /toekomst (KPI-tegels, kassabonnen, sim-widget, grafiek-uitleg) én /overzicht (Vrijheid-strip, mini-vermogensgrafiek). Vangnet: `lib/horizon-kernel/onmogelijke-uitkomst.test.ts` + `lib/horizon/outcome-guard.test.ts`; ADR 0108. De kern-scoping mag níet "richting oracle" worden weggefixt zonder nieuw eigenaar-besluit. |
| V22 | Hypotheek-aflossing als VAST bedrag (P1-bug, drain-run R4, 27 aug 2026; firsthand bevestigd met bug-reporter + SQL op het eigenaarsaccount). `adapter/potten.ts#plannedMonthlyAflossing` bevroor de aflossingscomponent van vándaag (`computeRenteAflossingsSplit().currentAflossing`) als constant jaarbedrag, en `tables/s.ts#regularSlot` paste die constant toe over de hele horizon — conform het oracle, dat élke reguliere slot als "annuïteit met vaste maandaflossing" modelleert. Bij een échte annuïteit is het aflossingsdeel juist het deel dat GROEIT terwijl de rente over een dalend saldo krimpt. Live: hypotheek €249.278,39 @4%, maandlast €1.193,54, einddatum juni 2056 (358 mnd) → bevroren aflossingsdeel €362,61 ⇒ ~687 maanden aflossen, saldo pas ~€0 rond leeftijd 103 i.p.v. 76. Zichtbaar als rode Schulden-balken tot 90+ in de Opbouw-grafiek; gevolg-keten `S!AF/AG` → `Prognose!totaalSchulden`/`nettoVermogen` → netto vermogen te laag en FIRE-leeftijd te laat voor iedere hypotheekhouder. Géén regressie: gedocumenteerd oracle-gedrag (`tables/s.ts` slot-rollen, ADR 0032). | **Besluit eigenaar (27-08-2026): ECHTE annuïteit in de kernel, via het M6-patroon.** Optionele, inert-by-default `KernelInput.echteAnnuiteitAflossing` + een optionele pot-`DebtPot.annuiteitMaandlast` (constante totale maandlast, rente + aflossing). Met beide gevuld herrekent `tables/s.ts#plannedMonthlyAt` de split per maand: `aflossing(m) = CLAMP(maandlast − saldo(m−1)·rente/12, 0, saldo(m−1))`, zodat het saldo op de werkelijke einddatum exact €0 raakt. **App-pad AAN** (`adapter/index.ts`), **parity-/fixture-pad UIT**: `input-from-fixture` zet de vlag níet én vult `annuiteitMaandlast` niet → **alle oracle-fixtures byte-groen tegen Excel v5, géén golden-herijking** (geverifieerd: 53 bestanden / 1201 tests, 0 gewijzigde fixtures). De adapter vult de maandlast alléén voor daadwerkelijk annuïtair aflossende schulden: `repayment_type` = `lineair`/`aflossingsvrij`, een handgezette `custom_aflossing_amount` en een maandlast ≤ rente krijgen 'm bewust NIET (aflossingsvrij mag niet ineens gaan aflossen). Schaalt mee met `inclusion_pct`, net als `startwaarde`/`aflossingEur`. **Bewust BUITEN dit besluit** (bekende, begrensde restpost): de payoff-vrijval `CF!G` (`tables/cf.ts#geplandeMaandAflossing`) blijft de BEVROREN `aflossingEur/12` vrijgeven i.p.v. de aflossing op het payoff-moment — een eigen conventie (ADR 0020-inverse) die conservatief is (onderschat de vrijval, dus het voordeel) en een apart eigenaar-besluit vergt. Vangnet: `lib/horizon-kernel/annuiteit-aflossing.test.ts` (18 tests: per schuldvariant + inert-by-default byte-identiteit + doorwerking naar totaalSchulden/nettoVermogen); ADR 0032-addendum. De kern-extensie mag níet "richting oracle" worden weggefixt zonder nieuw eigenaar-besluit. |
| V24 | Schuld→bezit-overloop verdampt bij surplus "Schulden aflossen" (UAT WF-TOEK-25-bug1, 2 sep 2026; persona "compleet": enkel `surplus_group` wisselen van 'beleggingen' naar 'schuld_aflossen' verschoof de vrijheidsleeftijd 42 → 53 en het afbouw-startbedrag €1,17M → €2,24M). Mechanisme: `adapter/prio-overgang.ts#toenamePrio` zet bij `schuld_aflossen` ÁLLE bezit-categorieën op reserve-prio 5 → `wBezitToename` (`verdeling/weights.ts#halveningWeights`) all-zero; de schuld-categorieën zijn STATISCH op t=0 gevuld/gewogen. Zodra de aanwijsbare schuld is afgelost (caps 0) lekt het volledige aflos-budget als `aflossing.onbenut`, en de HC:HH-overloop `onbenut · wBezitToename = onbenut · 0` liet het maandoverschot voor de rest van de horizon uit het grootboek verdampen. Zelfde bugklasse als V17 (toename) en de afname-/onttrekking-degeneratie. Het oracle kent deze configuratie niet: `schuld_aflossen` is een app-specifieke prio-mapping; de schuld-prio-fixture heeft gevulde bezit-toename-prio's (wBezitToename > 0). (V23 = markt-risicofactor/ADR 0117, buiten dit register.) | **Kernel-extensie, inert-by-construction (3 sep 2026; zelfde patroon als V17 — géén `KernelInput`-vlag nodig).** `computeVerdeling` valt voor een POSITIEF lek (`onbenut > ε`) bij een all-zero `wBezitToename` terug op de gedeelde instroom-ladder `verdeling/weights.ts#bezitInstroomFallbackWeights` (prio 1..4 zonder gevuld-eis → prio-5-reserve gelijk gewogen → all-nul), voorgecomputeerd in `VerdelingWeights` (`bezitToenameDegeneraat`, `wBezitToenameFallback`). Poort is op de 19 fixtures nooit waar → parity-suites (tabellen + engine + solver + band + MC) byte-groen. Dezelfde ladder voedt voortaan óók de V17-toename-degeneratie (`tables/toename-afname.ts#toenameGewichten`; één formule, één plek) en is **pot-bewust** (`bezitInstroomDoelMask`): alleen categorieën met ≥ 1 niet-woning-pot zijn instroom-doel — Bez deelt per stuk door `aantal` (0 → alsnog verdamping, bv. "Naar beleggingen" zonder beleggingspot) en de eigen woning is bij "Meerekenen" wel TS-liquide maar geen spaarpot voor kasoverschot. Bewuste semantiek ná aflossing: gelijk over de liquide reserve-potten (geen tweede bestemming opgegeven; expliciete categorie-prio's (V5) sturen de overloop via het reguliere pad). Nul/negatief lek + niet-degenerate pad byte-identiek. Vangnet `lib/horizon-kernel/aflossing-overloop-verdamping.test.ts` + concern `horizon-kernel-bekende-afwijkingen` (punt 8); niet "richting oracle" wegfixen zonder nieuw eigenaar-besluit. |
| V25 | Niet-geïndexeerde posten groeiden tóch mee met inflatie (Notion 3daf9e8d, 17 sep 2026; bevestigd met kernel-test: €1.000/mnd niet-geïndexeerd vanaf +10j gaf +10j €1.219 en +20j €1.486 = ×1,02^t; in euro van vandaag vlak €820 i.p.v. dalend). Twee vindplaatsen: (a) eigen gebeurtenissen — `adapter/events.ts#cashflowToGebPost` deelde één keer naar de startmaand, CF!H/Af!D indexeerden daarna élke maand (exact voor Eenmalig, structureel fout voor Periodiek); Excel-Geb kent géén indexatievlag per handmatige post → app-uitbreiding BUITEN het oracle-domein, geen oracle-afwijking. (b) eigen pensioen "Geïndexeerd = Nee" — `tables/auto-gebeurtenissen.ts#derivePensioenPot` (Auto-geb M) is de gedocumenteerde Excel-structuur (vooraf gede-indexeerd naar de ingang, daarna centraal geïndexeerd), maar onbeproefd (geen fixture met H="Nee") en intern strijdig met PT!K (partnerpensioen vlak nominaal bij niet-geïndexeerd). Live: 24/55 niet-geïndexeerde periodieke events (12 gebruikers) + 15/18 pensioen-events. | **(a) Kernel-extensie inert-by-default (`GebPost.nominaalVast`, ADR 0167, 19 sep 2026):** CF!H en Af!D tellen vaste posten in een aparte som zónder `idx(m)`; zonder vlag is die som 0 en blijft `H = baten·idx` letterlijk → 19 fixtures byte-groen. De adapter deelt niet meer en zet de vlag (Eenmalig én Periodiek). **(b) Bewuste oracle-afwijking (eigenaarsbesluit 19 sep 2026, optie B1):** `KernelInput.pensioenNominaalVast` — app-pad AAN (`adapter/index.ts`), fixture-pad UIT (`input-from-fixture` zet 'm nooit). Vangnet `lib/horizon-kernel/geb-nominaal-vast.test.ts` (abs. tol. €0,01) + concern `horizon-kernel-bekende-afwijkingen` (punt 10); niet "richting oracle" wegfixen zonder nieuw eigenaar-besluit; definitieve borging = Excel v6 met indexatievlag per post + fixtures herextraheren. |
