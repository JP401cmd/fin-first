...
| ☑ 2026-09-11 | **Datacorrectie (eigenaar-testaccount)**: dubbele rekening "credit card" (ICS, inactief) verwijderd — 172 transacties die een exacte kopie waren van de PayPal-upload (zelfde dag, bedrag en tegenpartij; −€7.601,73), de rekening zelf, en de lege cash-bezitting gedeactiveerd (zelfde stappen als `delete_bank_account`). 4 bevestigde terugkerende regels die er onterecht aan hingen verplaatst naar de rekening waarop hun betalingen staan. Eén transactie met telcontroles; nameting groen. | Melding V-001 (HBO niet gedetecteerd): dezelfde upload in twee rekeningen liet uitgaven dubbel tellen (~€6.095 in 12 mnd) en de abonnementsdetectie struikelen. Eigenaar koos 11-9 "via SQL, eerst preview". | Transacties staan ongewijzigd op de rekening "PayPal" — geen informatie verloren. Rekening terughalen: alleen via de dagelijkse Supabase-back-up of opnieuw aanmaken + uploaden. Regels terugzetten: `update recurring_transactions set account_id = <oude id>` voor de 4 rijen. Bezitting: `update assets set is_active = true`. |
