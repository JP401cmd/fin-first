...
| Na fase 2 (19 jul, deploy b0ae1c331) | **3,5 s (−53% cum.)** | 0,73 s / **LCP 0,82 s** | 4,2 s (stabiel; TTFB 2,0→0,85-2,9 s) | 0,10 mob / 0,03 desk | Streaming: blok 1 flusht vóór de dashboard-batch; gedeelde-tabel-queries 43→8; afkap-bugfix + spaarquote-grondslag-gelijktrekking (gebruikersbesluit); 9,0 s-uitschieter /toekomst = LCP-herpak-variantie (herhaalmeting 4,2 s) |
| Na fase 3 (19 jul, merge f537d09b5) | n.v.t. (geen LCP-doel) | n.v.t. | n.v.t. | n.v.t. | Doelen waren API-latency + bundels: ~100 read-GET's zonder auth-roundtrip (ADR 0052, ES256 bewezen); /login first-load 220,7→157,4 kB gzip (−28,7%, build-gemeten, review-gecorroboreerd); schulden-pilot live bevestigd (0 pagina-client-fetches); ratchet-CI actief (report-only tot Linux-baseline); prod-smoke schoon — ON CONFLICT-burst in logs = PRE-EXISTING budgets-upsert-bug (onConflict 'user_id, slug' vs echte index met COALESCE(parent_id), getriggerd door parallelle-sessie-testrun, niet door deze release) |
| Na fase 4 wave 1 (20 jul, deploy 09a1400cd) | **2,5 s (−67% cum.)** | LCP desk 0,56 s (−32%) | **1,4 s (−89% cum.)** | zie opm. | Shell enkelvoudig (ADR 0053: children 1× ge-SSR'd/gehydrateerd, één `<main>` live bevestigd) + kernel→web-worker (ADR 0054): /toekomst forced-reflow **3,55 s → 109 ms** — kernel aantoonbaar van de main thread af; /login LCP 0,222 s, CLS 0,00. CLS-/overzicht-meting (0,94 mob) is op dit account VERVUILD door de welkomstgids-banner (0/4 afgevinkt) — shift-cluster valt exact samen met banner-mount, niet met de shell (attributie 2× onafhankelijk vastgesteld; baseline-0,94 had vermoedelijk dezelfde bron); hermeten op account zonder openstaande onboarding. Nieuw aandachtspunt: report-only CSP-melding 'unsafe-eval' alléén op /toekomst (worker-bootstrap) — geen impact, volgen. |
| Na fase 4 T4.3 (20 jul, deploy ac8495254) | 1,16 s | — | 1,6–1,7 s (TTFB-ruis) | ongewijzigd | **React Compiler aan (ADR 0055).** Kernresultaat = interactie: slider op /toekomst triggert re-render-cascade over 4+ componenten (wat-als-lijn/dekkingsradar/guardrail-kompas/dekkingsgraad-strook) → **INP 159 ms (Good-band)**, soepel, geen drift/errors. Spike: healthcheck 1517/1517, 169 veilige bails/103 bestanden; suite 7669 groen nul drift; build-compile +12,7 s (Babel). /login desk 0,172 s (−22% vs wave-1). LCP-swings (/overzicht desk +20%, /toekomst +22%) = **TTFB-ruis** (2 opeenvolgende runs 605→1246 ms server-side, vóór client-code) — geen compiler-regressie; Speed-Insights-veld 3–7 dagen als tie-breaker. Handmatige memo's blijven; nieuwe componenten hoeven geen defensieve memo's. |
