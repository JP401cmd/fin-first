...
- **De mobiele TopBar-kleur is óók instelbaar, als chrome — geen identiteit, geen status** (ADR 0174 D3). Eén waarde in `profiles.topbar_color` (lowercase `#rrggbb`, `null` = leisteen `DEFAULT_TOPBAR_COLOR`), gekozen in `TopbarColorPicker` en geschreven via `PUT /api/appearance`. Hij voedt uitsluitend de vier `--topbar-*`-tokens via `topbarColorVars()`, die zelf de voorgrond (wit of inkt) kiest; er komt geen `--color-topbar-50..950`-palet en hij telt niet als vijfde accent. Kleur de balk dus nooit met een accent- of statusklasse, en lees de keuze in client-code via `useTopbarColor()` (bv. voor `ThemeColorSync`). De picker waarschuwt, blokkeert nooit, bij een middentoon (naam < 4,5:1) of een lichte balk (groene/oranje kompaspunten < 3:1).
...
## Plan-review-conventie — de wizard beweegt mee (verplicht bij instellingen)

De wizard "Je voorkeuren voor je plan instellen" op /toekomst (ADR 0142, aanvulling TPR-15) is een **extra ingang** naar instellingen die ook op hun eigen scherm staan. Hij mag nooit stil achterlopen:

- **Eén body, twee hosts.** Een instelling die de review behandelt, heeft één editor-body die zowel het bestaande scherm als de wizard rendert (`PLAN_REVIEW_EDITORS` / `PLAN_REVIEW_LAAG2_EDITORS`, host-contract `RegelEditActionsState`). Bouw nooit een tweede formulier in de wizard; een nieuw veld hoort in de body.
- **Optie-kopij als `Record<Union, …>`** — een nieuwe optie compileert pas met keuze · effect · waarom-kopij.
- **Veld-register per schrijfroute.** Geef je een schrijfroute van de review (`/api/fire-settings`, `/api/withdrawal-strategy`, `/api/pot-rules`, `/api/housing-strategy`, `/api/parameters`, `/api/retirement-aspirations`, `/api/assets/[id]/sale-config`, `/api/assets/[id]/expected-return`, `/api/life-events/strategie`) een veld, dan beslis je in dezelfde PR in `lib/plan-review/veld-register.ts` of de wizard het toont (stap/laag 2 + body) of niet (`buitenWizard` + reden). `veld-register.test.ts` scant de bron en wordt rood bij een ongeregistreerd veld. Een nieuw `KernelInput`-blok hoort daarnaast in `lib/plan-review/register.ts`.
- **Live effect = de kern met een override** (`runRegelProjection` + `RegelSimOverride` op de kolommen die de adapter leest) — nooit een eigen som in de wizard.
