-- Fix: source-vs-production drift for save_onboarding_data RPC.
--
-- The original migration 20260319000002_create_save_onboarding_rpc.sql created
-- save_onboarding_data as SECURITY DEFINER *without* SET search_path. At some
-- point in production the function was updated (likely via dashboard or a
-- lost migration) to SET search_path = '' — evidenced by the commit message
-- of 968d6b9e ("fix(db): resolve onboarding-blocking search_path bug and
-- household RLS recursion"), which states:
--
--   "trigger function fn_auto_link_bank_account_asset() inherited an empty
--    search_path when called from save_onboarding_data (which has SET
--    search_path = '')"
--
-- That empty search_path lives only in the production database; it never
-- made it back into source control. Consequences:
--   1. Reading the migrations alone no longer tells you how the function is
--      actually defined.
--   2. A fresh `supabase db reset` on a local DB would NOT reproduce the
--      production behavior (no SET search_path at all).
--   3. Latent bug: anyone who re-runs or rewrites the function from source
--      could accidentally restore the broken empty-search_path definition.
--
-- This migration re-declares save_onboarding_data with an explicit
--   SET search_path = public
-- and schema-qualifies every table and user-defined function reference in
-- the body with `public.`. Rationale: identical pattern to the earlier fix
-- in 20260407000002_fix_auto_link_bank_account_asset_search_path.sql, which
-- chose `public` (not '') precisely so that inner trigger functions inherit
-- a valid search_path and so local + production behave identically.
--
-- Source = truth after this migration: the body below IS the production
-- definition. No logic changes — only SET search_path and `public.` prefixes.
--
-- Deploy with: `npx supabase db push` (or equivalent).

CREATE OR REPLACE FUNCTION public.save_onboarding_data(payload JSONB)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  LOOP
    ...
    PERFORM public.ensure_companion_cash_asset(v_bank_id, true, true);
  END LOOP;
  ...
  -- 9. AOW life event
  DELETE FROM public.life_events WHERE user_id = v_user_id AND event_type = 'aow';
