-- =============================================
-- Migration: Household Support Foundation
-- Feature #239: Household database foundation
-- Reference: docs/plan-household-support.md
-- Date: 2026-02-18
-- =============================================
...
-- 4. Add ownership and household_id to existing data tables
ALTER TABLE assets ADD COLUMN IF NOT EXISTS ownership TEXT NOT NULL DEFAULT 'personal' CHECK (ownership IN ('personal', 'shared'));
ALTER TABLE assets ADD COLUMN IF NOT EXISTS household_id UUID REFERENCES households(id) ON DELETE SET NULL;

ALTER TABLE debts ADD COLUMN IF NOT EXISTS ownership TEXT NOT NULL DEFAULT 'personal' CHECK (ownership IN ('personal', 'shared'));
ALTER TABLE debts ADD COLUMN IF NOT EXISTS household_id UUID REFERENCES households(id) ON DELETE SET NULL;

ALTER TABLE budgets ADD COLUMN IF NOT EXISTS ownership TEXT NOT NULL DEFAULT 'personal' CHECK (ownership IN ('personal', 'shared'));
ALTER TABLE budgets ADD COLUMN IF NOT EXISTS household_id UUID REFERENCES households(id) ON DELETE SET NULL;

ALTER TABLE transactions ADD COLUMN IF NOT EXISTS ownership TEXT NOT NULL DEFAULT 'personal' CHECK (ownership IN ('personal', 'shared'));
ALTER TABLE transactions ADD COLUMN IF NOT EXISTS household_id UUID REFERENCES households(id) ON DELETE SET NULL;

ALTER TABLE bank_accounts ADD COLUMN IF NOT EXISTS ownership TEXT NOT NULL DEFAULT 'personal' CHECK (ownership IN ('personal', 'shared'));
ALTER TABLE bank_accounts ADD COLUMN IF NOT EXISTS household_id UUID REFERENCES households(id) ON DELETE SET NULL;

ALTER TABLE net_worth_snapshots ADD COLUMN IF NOT EXISTS ownership TEXT NOT NULL DEFAULT 'personal' CHECK (ownership IN ('personal', 'shared'));
ALTER TABLE net_worth_snapshots ADD COLUMN IF NOT EXISTS household_id UUID REFERENCES households(id) ON DELETE SET NULL;

ALTER TABLE valuations ADD COLUMN IF NOT EXISTS ownership TEXT NOT NULL DEFAULT 'personal' CHECK (ownership IN ('personal', 'shared'));
ALTER TABLE valuations ADD COLUMN IF NOT EXISTS household_id UUID REFERENCES households(id) ON DELETE SET NULL;

ALTER TABLE recurring_transactions ADD COLUMN IF NOT EXISTS ownership TEXT NOT NULL DEFAULT 'personal' CHECK (ownership IN ('personal', 'shared'));
...
CREATE POLICY "Members can view own household" ON households FOR SELECT TO authenticated USING (id IN (SELECT household_id FROM household_members WHERE user_id = auth.uid()));
CREATE POLICY "Owner can update household" ON households FOR UPDATE TO authenticated USING (id IN (SELECT household_id FROM household_members WHERE user_id = auth.uid() AND role = 'owner'));
CREATE POLICY "Authenticated users can create households" ON households FOR INSERT TO authenticated WITH CHECK (created_by = auth.uid());
CREATE POLICY "Owner can delete household" ON households FOR DELETE TO authenticated USING (created_by = auth.uid());

-- 7. RLS policies for household_members
CREATE POLICY "Members can view household members" ON household_members FOR SELECT TO authenticated USING (household_id IN (SELECT hm.household_id FROM household_members hm WHERE hm.user_id = auth.uid()));
CREATE POLICY "Can insert household members" ON household_members FOR INSERT TO authenticated WITH CHECK (household_id IN (SELECT hm.household_id FROM household_members hm WHERE hm.user_id = auth.uid() AND hm.role = 'owner') OR user_id = auth.uid());
CREATE POLICY "Owner can update household members" ON household_members FOR UPDATE TO authenticated USING (household_id IN (SELECT hm.household_id FROM household_members hm WHERE hm.user_id = auth.uid() AND hm.role = 'owner'));
CREATE POLICY "Members can leave or owner can remove" ON household_members FOR DELETE TO authenticated USING (user_id = auth.uid() OR household_id IN (SELECT hm.household_id FROM household_members hm WHERE hm.user_id = auth.uid() AND hm.role = 'owner'));

-- 8. RLS policies for household_invitations
CREATE POLICY "Members can view household invitations" ON household_invitations FOR SELECT TO authenticated USING (household_id IN (SELECT household_id FROM household_members WHERE user_id = auth.uid()) OR invited_email IN (SELECT email FROM auth.users WHERE id = auth.uid()));
CREATE POLICY "Owner can create invitations" ON household_invitations FOR INSERT TO authenticated WITH CHECK (invited_by = auth.uid() AND household_id IN (SELECT household_id FROM household_members WHERE user_id = auth.uid() AND role = 'owner'));
CREATE POLICY "Owner or invitee can update invitations" ON household_invitations FOR UPDATE TO authenticated USING (invited_by = auth.uid() OR invited_email IN (SELECT email FROM auth.users WHERE id = auth.uid()));
CREATE POLICY "Owner can delete invitations" ON household_invitations FOR DELETE TO authenticated USING (invited_by = auth.uid());
