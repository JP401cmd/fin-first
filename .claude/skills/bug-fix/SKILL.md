...
### 7. Review — één gebundelde fork-review (+ `security-specialist` alleen bij de harde triggers)
Eén **fork-subagent** (`subagent_type: "fork"` — níet de `code-review`-agent, zie de gedeelde conventies) beoordeelt de fix op correctheid, neveneffecten en kwaliteit, en krijgt in dezelfde opdracht de **UI-consistentie-lens** (bij een UI-bug) en de **security-lens** mee — geen drie aparte review-spawns (zie de gedeelde conventies). Geef hem de blast-radius-regel uit stap 5 mee (welke consumers het gewijzigde veld/symbool lezen) — valideren is goedkoper dan reconstrueren. Eén uitzondering blijft hard: raakt de fix auth, RLS, een migratie, een route met datatoegang of partner-/huishouddata — of wás de bug zelf een lek — dan draait de `security-specialist` zijn ship-gate-checklist als aparte run vóór afronding.
...
**Een fix op een branch is nog niet uit de lucht.** Draaide het defect live op productie (master = productie: de push ís de deploy — zie de release-skill) en staat de fix op een feature-/worktree-branch, dan is de bug pas afgerond wanneer hij op master staat. Sluit een run daarom nooit af zonder die vraag expliciet te beantwoorden: (a) de fix nú los naar master brengen — een zelfstandige bugfix hoeft niet te wachten tot de rest van de branch af is — of (b) hem laten meeliften met de branch, en de gebruiker er dan bij vertellen dát productie tot dat moment blijft falen, en hoe lang dat naar verwachting duurt. Optie (b) stilzwijgend kiezen is de fout: "de test is groen" bewijst dat de code klopt, niet dat de gebruiker er geen last meer van heeft. Les 3–4 aug 2026: `5afee541c` fixte een 42703 die élke `/overzicht/cashflow`-load stil liet leeglopen, maar bleef 32 uur op `worktree-perf-cashflow` staan; productie produceerde die fout door tot de merge van 4 aug 16:53 — zichtbaar in de Postgres-logs, door niemand opgemerkt.
