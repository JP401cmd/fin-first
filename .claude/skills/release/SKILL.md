...
### 11. Versie & vrijgavenotitie
Vóór de ship, want de versie en de notitie reizen mee in de deploy. **Eén versiebron:** `package.json` (gelezen via `lib/app-version.ts`; `/api/version`, `/beheer/versie`, `/beheer/releases` en de architectuurplaat tonen die). De bovenste notitie in `lib/release-notes.ts` draagt exact die versie — `lib/release-notes.test.ts` dwingt dat af, samen met `major === 0` (1.0.0 pas bij het formele go-besluit livegang) en de kort/gebruikersvriendelijk-norm.
1. **Bump-regel (eigenaar, 22 sep 2026):** het tweede getal (minor) gaat **maximaal 1× per maand** omhoog — `0.92` → `0.93` kan pas als er een kalendermaand tussen zit. Toets dat vooraf: de `date` van de oudste notitie met het huidige minor-getal in `lib/release-notes.ts` (de eerste `0.92.x`) moet ≥ 1 maand geleden zijn; anders is een minor-bump niet toegestaan, ook niet bij nieuwe functionaliteit. **Elke andere release telt de patch door**: `0.92.1`, `0.92.2`, … — ongeacht fix of functionaliteit. Is de maand om én is er nieuwe of uitgebreide functionaliteit, dan mag `0.93.0` (patch terug naar 0). Nooit major. Alleen een run van deze skill bumpt — `checkpoint`-commits en losse pushes niet.
   **Weergave vs. bron:** gebruikers zien de patch met drie cijfers (`0.92.001`, `0.92.002`) via `APP_VERSION_DISPLAY` / `formatVersionForDisplay` in `lib/app-version.ts`; `package.json` en `RELEASE_NOTES[].version` blijven geldige semver (`0.92.1`) — `npm version` weigert leading zeros. Schrijf de gepadde vorm dus alleen in tekst voor mensen (commit-titel, gebruiker), nooit in `package.json` of `version:`.
2. `npm version <nieuw> --no-git-tag-version` — raakt `package.json` én `package-lock.json`; beide stagen.
3. **Vrijgavenotitie** als nieuw `RELEASE_NOTES[0]` in `lib/release-notes.ts`: kort, in gewone taal, gegroepeerd op wat de gebruiker kent (Bezittingen · Schulden · Budget · Je plan · Fin · Platform — alleen de groepen die iets kregen), zonder bestandsnamen, paden, routes of ADR-nummers. Per item: wat kan iemand nu wat eerst niet kon, of wat werkt er nu wél goed? Alleen intern (refactor, migratie, tooling)? Dan één item onder Platform: "geen zichtbare verandering" — korter dan een verzonnen voordeel. Toon volgt `merkstem`, die de canonieke bron aanwijst — schrijf hier geen eigen toonlijst. **Doet de tekst een nieuwe belofte over veiligheid, opslag of rendement, dan is het een claim** → `compliance-check` vóórdat de notitie meegaat.
4. **Poort:** `npx vitest run lib/release-notes.test.ts` (via PowerShell) groen — rood betekent versie ≠ package.json, major ≠ 0, of een notitie die de norm schendt. De in-app suite `beheer.informatief` bewaakt hetzelfde op de server-runner.
5. Draaide stap 6 al: herdraai `npm run arch:diagram`, de plaat leest de versie uit `package.json`.
6. **Versie zichtbaar in de release:** de commit heet `chore(release): <versie>`, en de vrijgavenotitie op `/beheer/releases` draagt de versie. Voor gebruikers staat hij onderaan `/mijn` ("TriFinity versie 0.92.001", uit `APP_VERSION_DISPLAY`) — controleer bij de echte verificatie (stap 9) dat dat getal het nieuwe nummer toont.

Dit is géén tweede UAT-oordeel: stap 7 vraagt of een getest oppervlak geraakt is en schrijft naar `lib/uat/**`; deze stap vraagt wat een mens ervan merkt. De twee antwoorden mogen verschillen ("geen UAT-impact" naast een zichtbare verbetering, of andersom) — dat is geen tegenspraak. Niet publiceren op `/nieuws`: dat is de financiële-nieuwsfeed, geen productchangelog. De notitie landt op `/beheer/releases`; een oppervlak voor het testerspanel is een later, apart besluit.
...
### 13. Deploy-verificatie & rollback-pad
...
3. **Rollback-pad bij een kapotte deploy**: `git revert` van de commit(s) + push (geen force-push, geen reset op gedeelde history). Het schema blijft staan — migraties zijn forward-only; idempotente, additieve migraties (stap 4) zijn er precies zodat de oude code naast het nieuwe schema blijft werken. Meld de gebruiker wat er terugrolde en waarom. Rolt de release terug, dan rolt de versiebump (stap 11) mee terug — de revert raakt `package.json` en de notitie samen.
...
| "De deploy zal wel goed gaan, lokaal bouwde het" | Vercel-build ≠ lokale build (env/NFT/regio) — stap 13 bestaat omdat dit hier eerder misging. |
| "De versie bump ik wel bij de volgende release" | Dan draait prod op een versie waar geen notitie bij hoort en klopt `/beheer/versie` niet meer — stap 11 zit vóór de ship omdat versie en notitie mee moeten in dezelfde deploy. |
