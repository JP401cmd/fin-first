...
Vergelijk de huidige bron-DNA (`lib/ai/dna/base.ts` + `lib/ai/dna/wil.ts`) met de opgeslagen baseline in `lib/ai/local/parity-manifest.json` door **`npm run parity:scan`** te draaien — dat script leest hetzelfde gecommitte manifest en meldt per bron of de hash nog klopt. **Bereken die hash nooit met de hand.** De checker normaliseert CRLF→LF vóór het hashen; een handmatige hash op rauwe bytes doet dat niet, en levert op een Windows-checkout drift-meldingen op die puur regelinde-artefact zijn. Zo kwamen op 7 sep 2026 vier van de zestien baselines als drift binnen terwijl de inhoud identiek was. Twijfel je of een gemelde drift echt is? `npm run parity:rebaseline` classificeert het verschil expliciet als `in-sync`, `crlf-artefact` of echte drift. Klopt alles → geen drift, klaar. Wijkt een hash af → bepaal wélke regels in de bron-DNA veranderd zijn (filosofie, REGELS/COMPLIANCE, TOON) en of dat betekenisdragend is **voor elk artefact dat die bron noemt — loop ze één voor één langs, want een bron kan in het ene artefact al gecondenseerd zijn en in het andere niet.** De scanner meet per BRONBESTAND-hash, niet per artefact: hij meldt dát `base.ts` veranderde, maar niet wie hem had moeten overnemen. Op 7 sep 2026 ging dat mis — dezelfde commit die `base.ts` aanscherpte condenseerde de regel mee naar `LOCAL_CHAT_DNA`, terwijl briefing, rapport, nieuws en aanbevelingen op de oude formulering bleven staan; de scan meldde daarna elf dagen "drift" op een bron die voor één artefact allang verwerkt was. Een groene chat-DNA is dus géén bewijs voor de rest.
