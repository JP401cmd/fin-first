...
- [ ] **Een module-accent draagt geen grondslag en geen hiërarchie.** De vier accenten zijn gebruikersinstelbaar en bewust laag-chromatisch: ze zeggen "dit hoort bij deze module", nooit "dit is het belangrijkste getal" of "dit is de liquide grootheid". Een secundair KPI-bedrag staat dus in `--ink-2`, niet in `--module-active-*`; de hiërarchie loopt via grootte, gewicht en inkt-niveau, en de grondslag staat in wóórden naast het getal. Let op de stille variant: een accent dat vandaag met één grootheid samenvalt (bv. "de tweede regel is altijd het liquide doel") wordt door de eerste volgorde-wijziging betekenisloos, zonder dat een test of compiler dat merkt. Precedent: de Doelbedrag-KPI op /toekomst, rechtgezet 19-09-2026.
- [ ] Geen pure zwart (#000) of pure wit (#fff) als achtergrond?
- [ ] **WCAG 1.4.1 — kleur is nooit de énige drager.** Draagt een verschil betekenis (gedempt/actief, in/uit een selectie, een andere grondslag, een status), dan staat die betekenis óók zonder kleur op het scherm: tekst, een patroon/arcering (SVG `<pattern>`, `strokeDasharray`), een vorm of een icoon. **Dekking/opacity telt hier als kleur**, niet als tweede drager — ze schuift alleen de lichtheid op. Let op gestapelde grafieken met een monochrome ladder: daar is "lichter" al de betekenis van een ándere categorie, dus een gedempt segment leest als een buur. Draagt een legenda of tooltip de sleutel, dan moet de swatch hetzelfde patroon tonen als het vlak. Op smalle elementen (een jaarstaaf van 2–4px op mobiel) is tekst de primaire drager; rangschik de dragers bewust en leg die volgorde vast in de UAT. Precedent: `components/app/horizon/wealth-composition-chart.tsx` (buiten-doel-markering) en de gestippelde "andere grondslag"-lijn in `sim-chart.tsx`.
...
- [ ] Tests die getoonde bedragen pinnen: assert `formatCurrency`-output met een regex (bv. `/€\s*1\.234/`) of via `formatCurrency(x).replace(/ /g, ' ')` — de nbsp (` `) tussen `€` en het bedrag matcht níet tegen testing-library's default whitespace-normalizer, en de foutmelding ("Unable to find … normalized from …") wijst precies de verkeerde kant op (kostte in jul 2026 twee extra iteraties en in aug 2026 opnieuw een verspilde run). Typ het bedrag nooit met de hand over — dat verbreekt de koppeling met de formatter.
...
- [ ] **Twee vastgelegde uitzonderingen op de 44px-norm** (besluit eigenaar 26-08-2026, bevinding M19). Beide zijn bewuste compromissen, geen slordigheid — breid de lijst niet uit zonder eigenaarsbesluit:
  - **Pagina-header-controls op 28×28** (`h-7 w-7`): de `PageInfoButton` en het statuspunt rechtsboven op elke pagina. (De `InsightToggleButton` hoorde hier ook bij; die is 12-09-2026 met de inzicht-kaarten verwijderd — melding B-047.) Ze zitten met vaste offsets naast elkaar in de pagina-hoek (zie CLAUDE.md §Pagina-header-controls); 44px zou de kop-zone domineren en raakt ~46 bestanden. 28×28 haalt de WCAG-2.5.5-AA-ondergrens (24×24) ruim, de TriFinity-eigen 44px-norm niet.
