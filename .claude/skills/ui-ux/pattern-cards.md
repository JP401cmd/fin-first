...
- **Component**: `PageVerdictOpening` (`components/editorial/page-verdict-opening.tsx`), met `PageVerdictSentence` (zin) en `PageVerdictSuffix` ("Naam | oordeel") voor een oordeel dat nakomt.
- **Twee vormen.** Op de hefboompagina's en de hub van Toekomst (`/overzicht/bezittingen`, `/schulden`, `/budget`, `/belasting` en `/toekomst`) is de kop een **zin** (ADR 0174 D6). Alle andere subpagina's houden **"Naam | oordeel"** (zie hieronder); de omzetting daarvan is een vervolgkaart.
- **De zin** (`sentence` / `sentenceSlot`): "Je bezittingen zijn *goed gespreid*." Eén zin met de paginanaam als onderwerp, het oordeel als `<em className="font-normal italic">` in de stoplichtkleur, en een punt erachter (buiten de em). **Dezelfde zin op álle breakpoints**: de naam zit in de zin zelf en staat op mobiel ook in de TopBar, dus er is geen `hidden lg:inline`-naam en geen scheider.
  - **Zinnen komen uit één bron**: `HEFBOOM_OORDEELZIN` in `lib/hefboom-oordeelzin.ts` (via `loadHefboomPageVerdict(...).sentence`) en `resolvePlanVerdictSentence` in `lib/horizon/plan-status.ts`. Nooit een zin in de pagina schrijven. `HEFBOOM_VERDICT` blijft de korte vorm voor tegels en rondleiding.
  - **Vast onderwerp per pagina** (`HEFBOOM_ONDERWERP`, `PLAN_ONDERWERP`): elke `voor` begint ermee. Dat is het streamingcontract. Bij een stromende kop (`sentenceSlot={{ subject, rest }}`) staat het onderwerp in de eerste byte en levert `<PageVerdictSentence subject=…>` achter een `<Suspense>` de rest; de kop groeit dan van "Je budget" naar "Je budget is *op koers met sparen*." Zie `/overzicht/budget` (`budget-verdict.tsx`).
  - **Toon**: constaterend, zonder imperatief (Wft), zonder koop-/verkoopmetafoor (ADR 0165), zonder het woord "oordeel" (B-071). De "mogelijk"-hedge op belasting blijft.
  - **Geen oordeel** (`sentence={null}`): de kale paginanaam, zoals hieronder.
- **"Naam | oordeel"** (`verdict` / `verdictSlot`): **elke overige app-pagina-opening** onder `/overzicht/**` en `/toekomst/**`, met één uitzondering (de hub `/overzicht` zelf, zie onder). De titel bestaat uit twee delen — paginanaam + oordeel:
...
- **Deck**: twee korte zinnen, samen ~20 woorden. Eerste zin: wat de pagina is. Tweede zin: waar de kop naar kijkt, in gewone taal en zonder het woord "oordeel" ("Hoe meer soorten bezittingen je hebt, hoe beter je spreiding."). **Beweer niet meer dan de score meet**: noem alleen wat de status echt bepaalt (de bezittingenscore telt soorten, geen weging; de belastingstatus is alleen Box 3). Uitleg die daar niet in past (een rekenregel, een weging) gaat naar de `werking` van PAGE_INFO, niet in de deck.
...
- **Referentie-implementaties**: zin — `app/(app)/overzicht/belasting/page.tsx` (direct) en `app/(app)/overzicht/budget/` (stromend, `budget-verdict.tsx`); "Naam | oordeel" — `app/(app)/overzicht/budget/transacties/page.tsx` (direct oordeel), `app/(app)/overzicht/budget/vaste-lasten/` (stromend oordeel), `components/future/toekomst-subpage-shell.tsx` en `components/overview/belasting-box-page-header.tsx` (gedeelde shells, één `route`-prop).
...
- **Opbouw** (in deze volgorde, container `<header className="relative space-y-3">`):
  ...
  2. **Narratieve Playfair-H1**: `font-bold leading-tight tracking-[-0.02em] text-[28px] sm:text-[36px] md:text-[44px]` in `var(--font-playfair, serif)`, geformuleerd als korte vraag/zin in "Geld levert tijd op"-geest, met precies één `<em className="font-normal italic">`-accentwoord in `var(--module-active-700)`. **Let op**: dit is bewust de líchtere maatvoering — niet `EditorialHeadline` (font-black/leading-0.95) hergebruiken.
