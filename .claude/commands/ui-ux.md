...
- **Volle viewport-hoogte**: pane reikt van `top: 0` tot `bottom: 0`. In de new-navigation-shell (achter feature-flag `new_navigation_shell`) wordt geen `AppHeader` gerenderd; de pane mag dus tot bovenaan de viewport doorlopen — dat geeft visuele ruimte voor een prominente close-affordance bovenin én voorkomt een lege strook waar voorheen `top: var(--header-height)` werd gebruikt.
