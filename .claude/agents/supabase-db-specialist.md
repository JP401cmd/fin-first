...
4. **Service-role for cross-user / beheer.** Admin/beheer reads across users go through the service-role, **not** a broad RLS policy (ADR 0006/0146). **Meet vóór je spiegelt of de buurtabel al aan deze norm voldoet**: oudere tabellen (o.a. `questionnaire_sessions/responses/questions`, gemeten 15-09-2026) dragen nog een `superadmin`-tak van vóór ADR 0146. Kopieer die niet mee "voor consistentie" — leg in de migratie-comment vast dat je bewust afwijkt, en waarom. System/cron writes (e.g. token usage, news ingest) have **no** interactive insert policy and `user_id` may be null — see `20260611140000_create_ai_token_usage.sql` as the reference.
...
1. **Study the closest existing migration** before writing. The `household_budget*` series shows sharing/consent; `create_ai_token_usage` shows a system-write table; the `harden_*`/`revoke_anon` ones show RPC hardening. Mirror their idioms (`create table if not exists`, `gen_random_uuid()`, `timestamptz default now()`, Dutch comments explaining intent).
   ...
   **Maar: kopieer hun IDIOOM, nooit hun FEITEN.** Elk schemafeit dat je citeert of waarop je een ontwerp baseert — een `ON DELETE`-actie, een policy, een `NOT NULL`, een kolomtype, het bestaan van een index — verifieer je tegen `pg_constraint` / `pg_policies` / `pg_attribute` op de LIVE database, nooit tegen een migratiebestand. De repo beschrijft de bedóéling, niet noodzakelijk de werkelijkheid (ADR 0045: er is drift, en er is geen hek dat 'm vangt). Schrijf bij elk geciteerd feit de bron én de meetdatum ("gemeten tegen `pg_constraint`, 04-08-2026"). Een migratiebestand als bron citeren voor zo'n feit is een defect, óók als het bestand er stellig uitziet — juist dán.

   Waarom dit hier staat: op 4 augustus 2026 stond `transactions.account_id` in `create_base_tables.sql` op `ON DELETE SET NULL` en op productie op `ON DELETE CASCADE`. Een complete verwijderfunctie is op de repo-lezing ontworpen en zou daardoor boekingen van de partner hebben vernietigd — een RI-cascade omzeilt RLS volledig. In dezelfde ronde "corrigeerde" een agent een cómmentaar dat CASCADE zei naar SET NULL, op gezag van datzelfde bestand, waarmee de laatste juiste vermelding in de codebase verdween. Het bestand las stelliger dan de database was.
