...
5. **Verwacht gedrag** — what *should* happen, ideally tied to the canonical source (e.g. "beide schermen consumeren de horizon-kernel-uitvoer (`lib/horizon-kernel`), dus de datum hoort identiek te zijn").
...
1. **Reproduce or characterize.** Establish a deterministic repro from a clean state; if you can't, document exactly what's non-deterministic and the observed frequency. Never report a guess as a fact. **Verify a suspected root cause against the real engine / representative data before asserting it** — re-implementing or eyeballing the logic to predict the outcome is itself a bug source. Explicitly distinguish *correct-but-surprising for these inputs* (engine right, inputs unusual) from a genuine defect; and for data-specific symptoms, identify **which metric/code-path actually governs the observed number** (e.g. the line plots `netto vermogen` while the gate steers `liquide vermogen` — fixing the gate would be inert). A wrong root cause sends the whole pipeline down the wrong path. **Bij een API-fout met de generieke 500-envelope (bekende catch-tag, `serverError(err, 'tag:METHOD')`): zoek éérst server-side bewijs — Supabase `get_logs` en/of directe SQL-reproductie van de verdachte query — vóórdat je browser-repro inzet; browser-repro is daarna alleen nog nodig om de exacte klikvolgorde/UI-symptomen te bevestigen, niet om de servercrash aan te tonen.** Logs + SQL zijn sneller, sluitender en token-goedkoper, en botsen niet op een gedeelde/gelockte browserresource.
2. **Trace to the source.** Read the real code paths. For number bugs, follow the single-source-of-truth (`lib/horizon-kernel` for FIRE/Horizon — `lib/unified-projection.ts` is only its consumer type contract —, `lib/budget-utils.ts`, `lib/format.ts`, etc.) and check whether two surfaces read different places — that's the classic TriFinity bug. For data/visibility bugs, check RLS and ownership (gebruiker/huishouden). For AI bugs, check the prompt DNA, context builders, sanitize/PII, and the kill-switch. Bij "instelling werkt niet"-bugs (een toggle/keuze die niet doorwerkt, maar via een ander scherm wél): zet het werkende en het niet-werkende schrijfpad regel-voor-regel naast elkaar en benoem expliciet de side-effect die het werkende pad wél doet — typisch de recompute/sync van een afgeleide gate-kolom die de leespaden lezen. Bij bugs in **externe referentie-/statistiekdata** (CBS/Nibud/DNB e.d., bv. de benchmark/"Spiegel"): onderscheid expliciet of de bron een **mediaan of gemiddelde** rapporteert, en of het **gestandaardiseerd (equivalised) of ruw** huishoudinkomen/-vermogen betreft — beide onderscheidingen zijn materieel en kunnen de conclusie omdraaien (een equivalised cijfer is per definitie al herrekend naar een alleenstaande, dus een extra huishoud-correctie er bovenop is dubbeltelling).
3. **Scope it.** Determine who is affected, on which surfaces, under which data conditions. Distinguish "always" from "only when X". **Bij scherm-/oppervlak-inventarisaties: volg import-ketens, scan niet alleen `app/(app)/**`.** Een `BottomSheet`/`ShellOverlay`/pane die vanuit een `(app)`-route wordt geopend erft dezelfde shell-context (bv. de zwevende nav-pill, layout-vars) ook al leeft het component in `components/**`. Trace minimaal één niveau diep via de callers (bv. grep op `ShellOverlay`/pane-/sheet-openers) zodat getroffen componenten buiten de routemap niet worden gemist.
