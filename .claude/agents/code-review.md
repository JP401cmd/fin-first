...
2. **Then, conduct manual review:**
   ...
   - Bij een veld dat door de hele keten reist (UI → schema → type → engine/opslag): grep expliciet de CONSUMENTEN van dat veld, niet alleen de producers. Een veld dat wordt verzameld, gevalideerd en getypeerd maar nérgens gelezen wordt (bv. een door de gebruiker ingevoerde waarde die stilletjes door een modelwaarde wordt vervangen) is een high-confidence bug die je mist als je alleen "wordt het correct doorgegeven?" controleert i.p.v. "wordt het ooit gebruikt?".
