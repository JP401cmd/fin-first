...
2. **Then, conduct manual review:**
   ...
   - Schrijft de diff naar een **nieuwe tabel zonder DELETE-policy** (een append-only log, historie, audit-trail)? Behandel elke rij-vervuiling dan één severity-niveau hoger dan het zichtbare effect rechtvaardigt: wat vandaag stil is, is morgen onomkeerbaar. Toets specifiek of élk veld dat de rij vult **dezelfde eigenaarschaps-scope** heeft als de tabel zelf — een huishoud-gescoopte loader (`.or(ownership.eq.shared…)`) die een persoonlijke own-row-tabel voedt is de standaardvorm waarin dat misgaat, en hij is compile-, test- én UI-onzichtbaar omdat de rij nergens getoond wordt.
   ...
   - Verschuift de diff het **venster** of de **deler** van een aggregaat (rollend → afgesloten, per-post → per-gebruiker, 6 → 12 maanden)? Grep dan élke bon/kassabon/receipt die per-maand-rijen rendert *naast* het getal dat eruit zou moeten volgen (`incomeByMonth`, `monthlyBreakdown`, `*ByMonth`, `slice(-6)`) en verifieer dat rijen, subtotaal, deler en eindtotaal uit HETZELFDE venster komen. Een component dat `subtotaal / maanden × 12` zelf herrekent breekt stil zodra de bundel twee vensters draagt: het eindgetal klopt, de formule eronder niet meer — compile-, test- én parity-onzichtbaar, want beide kanten zijn op zichzelf juist.
   - Roept de diff een gedeelde loader/helper aan die zijn invoer uit een **rijvorm** krijgt (`select(...)`-resultaat, profielrij, DTO)? Vergelijk dan de KOLOMLIJST van de nieuwe call-site letterlijk met die van de bestaande, canonieke call-site (grep de geëxporteerde kolom-constante, bv. `*_COLUMNS`, of de `select('*')` waar de referentie-loader op leunt) en stel per ontbrekende kolom vast of de helper hem leest. Optionele velden op het rij-type (`created_at?`, `prefs?`) plus een `as unknown as T`-cast maken zo'n gat compile-onzichtbaar én test-onzichtbaar wanneer de helper in de test gemockt is — dat is de standaardvorm waarin een "pariteit hersteld"-fix stil een nieuwe divergentie invoert.
   - Draagt de diff een **utility-class-string die door een gedeeld component wordt samengevoegd** (`TapTarget`, `Button`, elke wrapper die `${basis} ${className}` doet)? Controleer dan op **conflicterende utilities uit dezelfde CSS-property-groep** — vooral `position` (`relative`/`absolute`/`fixed`), `display` en `overflow`. De volgorde in het `class`-attribuut beslist niets; zonder `tailwind-merge` wint de utility die in de gegenereerde stylesheet het láátst staat. Verifieer dat empirisch (compileer de twee klassen en vergelijk de offsets) in plaats van uit het hoofd — Tailwind emit `.relative` ná `.absolute`, dus een `className="absolute …"` op een component dat zelf `relative` injecteert wordt stil genegeerd. Dit is compile-onzichtbaar, lint-onzichtbaar, jsdom-test-onzichtbaar (jsdom past geen stylesheets toe) én overleeft een live-smoketest wanneer het getoetste pad de knop niet raakt.
   - Bij een wijziging in de RICHTING of semantiek van een parameter/type (bv. een goal-type waar 'lager is beter', een status die van betekenis wisselt): controleer naast de directe consumenten ook de **tweedegraads consumenten** — plekken die een afgeleide status of aggregatie consumeren (navkaart-stoplichten, briefing-/notificatie-engines, tellers) en impliciet de oude semantiek aannemen. Die her-aggregatoren verschijnen niet in een grep op de gewijzigde functie; grep daarom óók op de status-/progress-velden die eruit voortvloeien (bv. `onTrack`, `attention`, `pct`).
   ...
   - Bevat de diff een **async handler in een parent die een child-component aanstuurt** (submit/create/save met een `await` erin)? Trace dan expliciet het **annuleer-/sluitpad tijdens het in-flight window**: wie kan de child unmounten of de fase omzetten terwijl de promise nog loopt (cancel-knop, Escape-handler, route-wissel, timeout), en wat doet de code ná de `await` als dat gebeurd is. Een busy-state die in het child leeft (`saving`) en een cancel-pad dat in de parent leeft (`dispatch`) weten per definitie niets van elkaar — dat is een high-confidence bug wanneer de code na de await onvoorwaardelijk doorschrijft.
