...
5. **Verify like it ships.** Run `npx tsc --noEmit`, `npx eslint <de bestanden die je wijzigde> --quiet`, and the relevant `npm run test:run` paths after multi-file changes. Never report done while checks are red. **Twee omgevingsvallen op dit platform, geen van beide zegt iets over jouw code:** (a) repo-breed `npm run lint` is permanent rood (honderden warnings + een handvol pre-existing errors) en is dus géén gate — toets per bestand met eslint; (b) draai vitest via de **PowerShell**-tool, nooit via Bash: onder Git Bash faalt het met een misleidende `Vitest failed to find the current suite`, ongeacht de testinhoud, en `--reporter=basic` bestaat niet in vitest 4.x. Schrijf bronbestanden ook nooit met PowerShell (`Set-Content`/`Out-File`) — die round-trip mangelt niet-ASCII (é, —, ±) tot mojibake; gebruik de Edit-tool, ook voor een tijdelijke bijt-proef heen én terug. **Blijft een bijt-proef groen, dan is dát de bevinding:** je test bewaakt iets anders dan je denkt. Zoek uit welke onbedoelde eigenschap hem groen hield en herontwerp code of test tot de proef bijt — voer 'm nooit af als "kennelijk niet nodig". Staat `tsc` rood in een gedeelde working tree, controleer dan eerst met een unieke-bestandslijst of de fouten in jóúw bestanden zitten voor je gaat repareren. For risky changes, ensure the `tester` agent has coverage proving it. **Let op — twee testlagen:** de in-app regression suites (`lib/regression-tests/suites/*`, geregistreerd via `registerTest`/`registerCategory`) draaien NIET in vitest maar via de server-runner — UI: `/beheer/regressietest`, of headless: `POST /api/regression/run` tegen een lopende `npm run dev` (dev-only; auth via `REGRESSION_TEST_EMAIL`/`_PASSWORD` + seed). `npm run test:run` dekt ze dus niet. Voor FIRE/horizon-kernel-drift is er wél een puur-vitest gouden matrix (`lib/regression-tests/horizon-strategie/matrix.test.ts`) die `npm run test:run` al draait — gebruik die als snelle headless engine-check en regenereer de goldens bewust.
6. **Eindreview zonder fork.** Schrijven de pijplijn-conventies een fork-review voor (`subagent_type: "fork"`) maar is dat type in jouw sessie niet beschikbaar (sub-agents kunnen niet forken), dispatch dan de `code-review`-agent mét een expliciete bestandslijst (de gewijzigde paden + de reviewvraag en de drie lenzen), zodat hij niet zelf de hele diff hoeft te zoeken. Meld in je rapport dat de fork-route niet beschikbaar was.
7. **Communicate decisions.** Explain the why, the trade-offs considered, what you delegated, what you verified, and what's next. Be direct about residual risk.
