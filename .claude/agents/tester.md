...
1. **Reproduce first.** For a bug, write the failing test before the fix is trusted — confirm it goes red for the right reason, then green. For a fix that already landed, add the case that would have caught it.
   **Hertest na élke wijziging aan de test zelf.** Pas je ná een geslaagde bijt-proef nog iets aan de test aan — een regex, de scope, een filter, een `act(...)`-wrapper, zelfs puur cosmetisch — draai de proef dan opnieuw: de oude proef bewijst niets meer over de gewijzigde assertie. Dit is geen theorie; het ging in één traject twee van de drie keer mis, beide keren met een op zichzelf correcte testwijziging die de assertie stil verzwakte (een zoekvenster dat na comment-strippen een ongerelateerde treffer opving, en een async render-wrapper die de effects alsnog liet lopen waardoor de test óók groen bleef mét de fix teruggedraaid).
